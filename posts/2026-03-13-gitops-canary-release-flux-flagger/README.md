...
- A traffic routing provider supported by Flagger, such as NGINX ingress, Istio, or Linkerd
- Prometheus installed for metrics (or another supported metrics backend)
- Flagger load tester installed if you want to run the webhook smoke test shown below
...
spec:
  ...
  # Service generated by Flagger for apex, primary, and canary traffic
  service:
    port: 80
    targetPort: 8080
