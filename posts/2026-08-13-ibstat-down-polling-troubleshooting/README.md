...
$ dmesg -T | grep -iE 'mlx5|firmware|infiniband|rdma|module'
~~~
...
Use the read-only form of NVIDIA's physical-layer tool where it supports the device. For an adapter, current MFT documentation says to address the appropriate MST device; switch ports use the port selector:
...
`mlxlink` reports link and cable information supported by that device and firmware. `--show_module` and `--show_counters` add module and physical counter data. Do not use its port-state, speed, FEC, PRBS, or transmitter commands during an initial read-only investigation; those are configuration or test operations and can take the link down.
...
Do not clean optics, bend fiber, or use loopbacks outside the vendor's handling procedure. Active copper and optical cables contain electronics; treating them as passive wire can miss power, firmware, or module compatibility faults.
...
When the fabric has another active management path, `ibportstate` can query switch/HCA port speed and width relative to its peer. For a completely down link, use the local endpoint and switch-local tools instead.
...
The SM matters after physical state reaches `LinkUp`. Then a port commonly remains logical `Init` while waiting for SM discovery and LID assignment. NVIDIA's troubleshooting documentation explicitly separates that initializing condition from driver/device failures.
...
- `Init/LinkUp`: diagnose the Subnet Manager, its reachability, and fabric sweep.
...
- [NVIDIA MFT: `mlxlink` utility](https://networking-docs.nvidia.com/mftswum/426135lts/mlxlink-utility)
- [NVIDIA MFT 4.30 documentation: `mlxfwmanager` query tool](https://docs.nvidia.com/networking/display/nvidia-firmware-tools-mft-documentation-v4-30-0.0.pdf)
