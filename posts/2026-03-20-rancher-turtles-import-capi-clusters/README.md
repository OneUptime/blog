...
Rancher Turtles integrates Cluster API (CAPI) with Rancher to provide a unified, declarative approach to Kubernetes cluster lifecycle management. Importing an existing CAPI cluster is done by adding the `cluster-api.cattle.io/rancher-auto-import` label to the cluster or its namespace. Rancher Turtles waits until the cluster reports `ControlPlaneAvailable=True`, then creates the Rancher `clusters.management.cattle.io` resource and installs the `cattle-cluster-agent`.
...
# Monitor progress
kubectl get clusters.cluster.x-k8s.io example-cluster -n default --watch
```
