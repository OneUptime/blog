...
## Strategy 2: Resource Exclusions and Diff Customization

If you cannot restructure your manifests immediately, you can tell ArgoCD to exclude a resource kind from discovery and sync. Resource exclusions are configured globally in `argocd-cm` and match by API group, kind, and cluster, not by individual resource name, so use them carefully.

```yaml
# In argocd-cm ConfigMap, exclude a resource kind globally
apiVersion: v1
kind: ConfigMap
metadata:
  name: argocd-cm
...
For per-application drift caused by fields that another controller changes, use the `ignoreDifferences` field. This ignores selected fields during comparison; it does not remove the resource from the application or solve ownership conflicts by itself.
...
For resources generated by tools outside the application's desired manifests, you can keep them from affecting the application's overall sync status using the `argocd.argoproj.io/compare-options` annotation.
...
This only affects sync status. If a resource is still visible to ArgoCD and you need to prevent pruning, combine it with the `Prune=false` sync option:
...
| Temporary sharing during migration | Diff customization, then move to a single owner |
...
- Use `IgnoreExtraneous` only for extraneous generated resources that should not affect sync status
