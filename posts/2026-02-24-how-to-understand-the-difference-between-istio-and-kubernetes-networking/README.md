...
When something calls `my-service:8080`, CoreDNS resolves `my-service` to the ClusterIP, and kube-proxy or the cluster's data plane routes the request to one of the backing pods. The exact load-balancing behavior depends on the implementation.
...
Kubernetes Services provide basic L4 load balancing across pods that match a label selector. A Service cannot route based on HTTP headers or split service-to-service traffic by percentage. Kubernetes Ingress and Gateway API can route external HTTP traffic by host or path, but Services themselves do not provide mesh-wide L7 traffic management.
...
This routes requests with the `x-version: beta` header to v2, and splits remaining traffic 90/10 between v1 and v2. This assumes a DestinationRule defines the `v1` and `v2` subsets. Kubernetes Services cannot do this natively.
...
Istio can automatically use mTLS for service-to-service traffic between mesh workloads, and a `PeerAuthentication` policy can require it:
...
No application changes needed. The sidecars handle encryption and certificate management. In `STRICT` mode, workloads must connect with mTLS.
...
This says: only the web-app service account from the frontend namespace can make GET requests to /api/products on workloads labeled `app: products-api`. You cannot express this with Kubernetes NetworkPolicy.
...
Kubernetes has readiness and liveness probes, but Services do not provide circuit breaking, retry policies, or request timeouts for service-to-service traffic at the infrastructure level.
...
| Load balancing | Basic L4 balancing, implementation-dependent | Round-robin, least request, random, consistent hash |
| Traffic splitting | No | Yes (weighted, header-based, URI-based) |
| Encryption | No (by default) | Yes (automatic mTLS) |
| Network policies | L3/L4 NetworkPolicy | L7 AuthorizationPolicy in addition to Kubernetes NetworkPolicy |
...
| Canary deployments | Basic rollout primitives, but no native weighted L7 split | Yes |
...
3. **Istio sidecars** intercept traffic after the application sends it and before it reaches the destination in sidecar mode
...
- **kube-proxy + Istio**: When Istio is active in sidecar mode, Envoy can handle service routing for mesh traffic instead of relying on kube-proxy's Service load-balancing path. The proxy is configured with Kubernetes service and endpoint information, and kube-proxy rules still exist for traffic that is not intercepted by the sidecar.
