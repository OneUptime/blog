...
- Host endpoints configured for the target nodes with matching labels
...
spec:
  ...
  preDNAT: true
  applyOnForward: true
  selector: host-endpoint == 'production-node'
  ingress:
    - action: Allow
      protocol: TCP
      source:
...
calicoctl get globalnetworkpolicy -o wide | grep pre-dnat
