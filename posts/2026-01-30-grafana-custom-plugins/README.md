...
# Go to the new plugin directory and install dependencies
cd <your-plugin>
npm install
```

You also need a running Grafana instance for testing. The easiest approach is the Docker Compose setup generated by `create-plugin`:

```bash
# Build the plugin frontend
npm run dev

# Start Grafana with the generated Docker Compose configuration
docker compose up
```
...
export class MyDataSource extends DataSourceApi<MyQuery, MyDataSourceOptions> {
   ...
   * Uses Grafana's backendSrv. For authenticated HTTP APIs, configure
   * Grafana data proxy routes so secrets stay on the server.
   */
   ...
   */
  getDefaultQuery(_app: CoreApp): Partial<MyQuery> {
    return {
...
// Register the app plugin with its root component
...
```bash
# Export a Grafana Cloud access policy token
export GRAFANA_ACCESS_POLICY_TOKEN=<YOUR_ACCESS_POLICY_TOKEN>

# Sign a private plugin for one or more Grafana root URLs
npm run sign -- --rootUrls https://your-grafana-instance.com
```

Submit to the Grafana plugin catalog from Grafana Cloud under Org Settings > My Plugins. The submission asks for a packaged plugin ZIP URL, source code URL, SHA1 hash, and testing guidance.
