...
You can add OpenTelemetry resource attributes to spans generated by the mesh through resource detectors. For example, the environment detector reads attributes from `OTEL_RESOURCE_ATTRIBUTES`:
...
You can also set custom span tags through the Telemetry API:
...
One of the biggest advantages of the OTel Collector is sending traces to multiple backends. With `DD_API_KEY` set in the collector's environment, update the collector config:
...
kubectl logs deploy/otel-collector -n observability | grep "Traces"
...
kubectl run -n my-app tmp-curl --rm -i --restart=Never \
  --image=curlimages/curl -- \
  curl -sv http://otel-collector.observability:4318/
