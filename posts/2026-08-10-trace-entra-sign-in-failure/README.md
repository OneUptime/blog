...
The AADSTS code is a category, not a complete root cause. The correlation ID helps group requests from the same sign-in attempt, the request or trace ID identifies a specific request, and the UTC timestamp narrows the search window. The authoritative investigation record is the sign-in event in the tenant that processed the request.
...
| --- | --- | --- |
| AADSTS code | Find the current error category and Microsoft guidance | Codes and text can become more granular; do not build application control flow around the prose |
...
| Timestamp | Bound the search and disambiguate reused client/user combinations | Convert to UTC and allow for log ingestion delay |
...
- **Interactive user sign-ins** for browser or other user-present authentication;
- **Non-interactive user sign-ins** for silent token acquisition and background refresh on behalf of a user;
...
Looking only at Interactive user sign-ins is a common reason an engineer concludes that “Entra did not log the failure.” Match the log type to the OAuth flow and identity.

## Filter Precisely

Use a narrow UTC time range around the captured timestamp, then add the **Correlation ID** or **Request ID** filter. The portal also supports filters for status, user, application, resource, IP address, client application, and Conditional Access.
...
6. confirm the request reached Microsoft Entra rather than failing in DNS, TLS, a proxy, or the application's callback before authentication.
...
- **What:** client application, target resource, resource tenant, resource service principal, and requested access context.
...
- **Authentication details** for the sequence of authentication methods and which step succeeded or failed;
- **Conditional Access** for evaluated policies, grant controls, and report-only outcomes;
- **Device info** for managed, compliant, join, browser, and operating-system state;
- **Location** for the source IP and interpreted geography; and
- **Status** for the failure reason, error code, and additional details.
...
From a failed sign-in's Activity Details panel, launch **Sign-in diagnostic**. It analyzes the selected event and offers scenario-specific findings and remediation.
...
- “invalid audience” usually means the client requested a token for a different resource than the API validating it; and
- a Conditional Access error requires the policy result and authentication details, not just the AADSTS text.

Applications should handle standards-based OAuth fields such as `error` and `error_description` appropriately, but should not hard-code business behavior around mutable human-readable AADSTS descriptions.

## Query Sign-Ins with Microsoft Graph

For repeatable investigations, Microsoft Graph exposes `auditLogs/signIns`. Programmatic retrieval requires a tenant with Microsoft Entra ID P1 or P2. For delegated access, the client needs `AuditLog.Read.All` and the signed-in user needs a supported directory role such as Reports Reader. App-only access uses the `AuditLog.Read.All` application permission and has no signed-in user role. For a user sign-in, filter by correlation ID:
...
Treat the returned data as sensitive security telemetry. Limit access, protect exports, and follow the organization's retention and privacy rules. For investigations beyond the built-in retention window, configure diagnostic settings in advance to route sign-in logs to Azure Monitor, a storage account, an event hub, or an approved SIEM.
...
Do not log token bodies. If token validation fails, log the validation category—issuer, audience, signature key, lifetime, nonce, or state mismatch—and only safe identifiers.
