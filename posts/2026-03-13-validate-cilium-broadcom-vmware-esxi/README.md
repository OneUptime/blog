...
Running Cilium on Kubernetes clusters hosted on VMware ESXi introduces specific considerations around eBPF kernel requirements, tunnel configuration, and network interface compatibility. ESXi's virtual networking layer must be configured to pass traffic correctly to allow Cilium's datapath to function, particularly for features like VXLAN or Geneve tunneling and kube-proxy replacement.

Validating Cilium on ESXi requires checking the Linux kernel version on guest VMs, verifying the relevant vSwitch or distributed virtual switch (DVS) security policies for your networking mode, and confirming that eBPF programs load successfully on all nodes.
...
- Linux kernel 5.10+ on guest VMs, or a Cilium-supported equivalent such as RHEL 8.10's 4.18 kernel
...
ESXi may require specific vSwitch settings for networking modes that use non-default MAC behavior or packet capture.

```bash
# From an ESXi host or vCenter, verify port group security settings
# The following settings may be required depending on your node and network design:
# - Promiscuous Mode: Accept (required for packet capture or nested virtualization use cases)
# - Forged Transmits: Accept (required if a VM sends frames with a source MAC different from its assigned vNIC MAC)
# - MAC Address Changes: Accept (required if a VM needs to receive traffic for a changed guest MAC)
...
- Use Cilium's native routing mode with BGP if your upstream network can route pod CIDRs - it avoids encapsulation overhead
