...
jobs:
  snyk:
    ...
    steps:
      ...
      # Fail the job after SARIF upload if Snyk found blocking vulnerabilities
      - name: Fail on blocking Snyk findings
...
on:
  # Trigger from external automation that dispatches vulnerability alerts
  repository_dispatch:
    ...
    steps:
      ...
      - name: Audit and identify fixes
        ...
        run: |
          ...
          fs.appendFileSync(process.env.GITHUB_OUTPUT, `has_fixes=${fixes.length > 0}\n`);
          fs.appendFileSync(process.env.GITHUB_OUTPUT, `fix_count=${fixes.length}\n`);
          fs.appendFileSync(process.env.GITHUB_OUTPUT, `fixes<<JSON\n${JSON.stringify(fixes, null, 2)}\nJSON\n`);
          EOF
      ...
      - name: Apply security fixes
        ...
        run: |
          # Define a branch name for the pull request action
      ...
      - name: Create Pull Request
        ...
        with:
          ...
          commit-message: "fix(security): auto-remediate vulnerable dependencies"
...
def get_vulnerabilities() -> list[Vulnerability]:
    ...
    for name, data in audit_data.get("vulnerabilities", {}).items():
        ...
        cve_id = "N/A"
        for advisory in data.get("via", []):
            if isinstance(advisory, dict) and advisory.get("cve"):
                cve_id = advisory["cve"]
                break
...
const latestCountsByRepository = new Map();

function observeSeverity(result, severity) {
  for (const [repository, counts] of latestCountsByRepository.entries()) {
    result.observe(counts[severity], {
      repository,
      scanner: 'trivy',
    });
  }
...
criticalVulnsGauge.addCallback((result) => observeSeverity(result, 'critical'));
highVulnsGauge.addCallback((result) => observeSeverity(result, 'high'));
mediumVulnsGauge.addCallback((result) => observeSeverity(result, 'medium'));
lowVulnsGauge.addCallback((result) => observeSeverity(result, 'low'));
...
async function reportScanResults(scanResults) {
  ...
  // Update values observed by the registered gauge callbacks
  latestCountsByRepository.set(repository, counts);
