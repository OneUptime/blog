...
Look for the configured event name, source type, and a successful publication containing an event ID. A Kafka producer acknowledgment or an HTTP `2xx` generated by an upstream proxy proves only that the event reached that boundary. Argo's webhook handler returns `2xx` only after a successful dispatch to the EventBus, so a provider delivery record showing that response is evidence of publication when the response came from the EventSource itself. The EventSource success log and `argo_events_events_sent_total` provide the clearest in-cluster confirmation.
...
- if `argo_events_action_failed_total` increases, conditions were met but post-condition trigger processing failed, such as parameterization, execution, or policy evaluation;
