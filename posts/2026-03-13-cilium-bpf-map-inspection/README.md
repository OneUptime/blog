...
Cilium implements its data plane with eBPF, and much of its state - routing decisions, connection tracking, load balancer backends, policy rules, NAT translations - is stored in eBPF maps in the Linux kernel. When something is wrong with Cilium's behavior, the root cause is often visible in these maps. Cilium exposes inspection commands for its BPF maps through the in-agent `cilium-dbg bpf` subcommand family, giving you direct visibility into the kernel data structures that govern packet processing.
...
# cilium_lb4_services_v2    - IPv4 load balancer service VIPs
# cilium_lb4_backends_v3    - IPv4 load balancer backends
# cilium_snat_v4_external   - IPv4 SNAT/masquerade table
...
kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg bpf ct list global | grep "10.1.0.5"
...
kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg bpf lb list | grep "10.96.0.100"
...
ENDPOINT_ID=$(kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg endpoint list | grep my-pod | awk '{print $1}')

# Get endpoint details, including the policy map path
kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg endpoint get ${ENDPOINT_ID}

# Dump policy BPF maps
kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg bpf policy get --all

# Sample output:
# POLICY   DIRECTION   IDENTITY   PORT/PROTO   PROXY PORT   AUTH TYPE   BYTES   PACKETS   PREFIX
# Allow    Ingress     1234       ANY          NONE         disabled    10240   100       0
# Deny     Ingress     5678       ANY          NONE         disabled    0       0         0
```
...
kubectl exec -n kube-system cilium-xxxxx -- \
  cilium-dbg bpf nat list | grep "203.0.113"

# Check egress gateway BPF entries
kubectl exec -n kube-system cilium-xxxxx -- \
...
# Compare with max: cilium-dbg config get bpf-ct-global-tcp-max and bpf-ct-global-any-max
...
```mermaid
flowchart TD
    A[Packet Arrives] --> B[cilium_lb4_services_v2\nService VIP lookup]
    B --> C[cilium_lb4_backends_v3\nSelect backend]
    C --> D{cilium_policy_<id>\nCheck policy}
    D -->|Allow| E[cilium_ct4_global\nCreate/update conntrack]
    D -->|Deny| F[Drop]
    E --> G[cilium_snat_v4_external\nApply masquerade if needed]
    G --> H[Forward to backend]
```

## Conclusion

BPF map inspection gives you the ground truth about Cilium's data plane state - no interpretation required, just the actual kernel data structures that govern packet processing. Use `cilium-dbg bpf ct list` when debugging connection failures (look for missing or stale conntrack entries), `cilium-dbg bpf lb list` when load balancing isn't working (check if the right backends are registered), and `cilium-dbg bpf policy get --all` when a policy appears correct in YAML but traffic is still being dropped (check what was actually compiled into the policy map). This level of visibility is a key strength of Cilium's eBPF-based datapath.
