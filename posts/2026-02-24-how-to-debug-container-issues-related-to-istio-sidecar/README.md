...
### Step 3: Test Connectivity from the Application Container

```bash
kubectl exec -it my-app-xyz -c my-app -- curl -v http://other-service.default.svc.cluster.local:8080
```

If this works from the same container but your application code still fails, the issue is likely in how your app constructs requests. Avoid relying on `curl` inside `istio-proxy`: many proxy images do not include debugging tools, and traffic generated by the proxy container does not always follow the same capture path as traffic from the application container.
...
### Check if the Pod Is in the EndpointSlices

```bash
kubectl get endpointslice -l kubernetes.io/service-name=my-service
```

If your pod's IP isn't listed as a ready endpoint, it's not ready (check readiness probes).

### Check the Inbound Listener

```bash
istioctl proxy-config listener my-app-xyz --port 8080
```
...
If your application uses any of these ports, change your application's port or configure the relevant Istio proxy port where Istio supports doing so. Excluding a port from capture does not free a port that the proxy or agent already binds. For ordinary application ports that should bypass inbound capture, use:
