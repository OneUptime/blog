...
You need an AKS cluster running Kubernetes 1.19 or newer with kubectl configured, Helm 3 installed, NGINX Ingress Controller 1.0.2 or newer, and basic familiarity with Kubernetes deployments and services. I will assume you already have an AKS cluster. If not, create one with `az aks create`.
...
The `controller.metrics.enabled=true` flag is important because Flagger needs Prometheus metrics from NGINX to make promotion decisions. The Prometheus scrape annotations let Flagger's Prometheus add-on discover the ingress controller metrics endpoint.

## Step 2: Install Flagger and Its Prometheus Instance

Flagger needs a metrics source to evaluate canary health. Its Helm chart can install a Prometheus add-on that scrapes NGINX metrics.
...
Flagger will create the ClusterIP services for the application from the Canary resource. The apex service will be named `podinfo`, and Flagger will also create `podinfo-primary` and `podinfo-canary`.

```yaml
# Generated by Flagger from the Canary service spec
apiVersion: v1
kind: Service
metadata:
  name: podinfo
...
Apply the deployment and ingress with kubectl.

```bash
# Apply the deployment and ingress
...
spec:
  ...
  ingressRef:
    ...
    kind: Ingress
    name: podinfo
  # Maximum time in seconds for the canary deployment to make progress
...
Let me break down the analysis section. Flagger will check the canary every 30 seconds. It starts by sending 10% of traffic to the new version, then increases by 10% each step up to 50%. At each step, it checks that the success rate stays above 99% and latency stays below 500ms. If 5 metric checks fail during the analysis, Flagger rolls back the canary. If the canary reaches the maximum weight without hitting that failure threshold, Flagger promotes it to primary.
...
If the canary version starts failing, Flagger will detect it through the metrics and automatically roll back. You can simulate a failure by deploying a new version and sending requests that return 500 responses through the ingress.

```bash
# Deploy a new version
kubectl set image deployment/podinfo \
  podinfo=ghcr.io/stefanprodan/podinfo:6.1.4

# Generate 500 responses during the analysis
