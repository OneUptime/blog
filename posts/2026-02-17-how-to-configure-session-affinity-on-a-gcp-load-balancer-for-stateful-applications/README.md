...
GCP load balancers support several session affinity types, depending on the load balancer family:

| Type | How It Works | Best For |
|------|-------------|----------|
| NONE | No explicit affinity; the default backend selection uses packet/request hashing | Stateless applications |
| CLIENT_IP | Hash based on the client/source IP address (and destination IP for most load balancers) | Simple stickiness, L4 load balancers |
| CLIENT_IP_PROTO | Hash of source IP, destination IP, and protocol | Protocol-specific routing |
| CLIENT_IP_PORT_PROTO | Hash of source IP, source port, protocol, destination IP, and destination port | Fine-grained L4 affinity |
| GENERATED_COOKIE | Load balancer sets a cookie | HTTP(S) load balancers |
| HEADER_FIELD | Hash of a specific HTTP header | Custom session identification |
| HTTP_COOKIE | Hash of a named HTTP cookie, generated by the load balancer if absent | Application-named affinity cookies |

## Configuring Generated Cookie Affinity

Generated cookie affinity is a common option for HTTP(S) load balancers. The load balancer sets a cookie on the first response, and subsequent requests with that cookie are routed to the same backend while the cookie is valid and the backend remains eligible.
...
3. Response includes a generated affinity cookie (`GCLB` for global external and classic Application Load Balancers, `GCILB` for regional external and internal Application Load Balancers)
4. Client includes the affinity cookie in subsequent requests
...
# Expected for a global external or classic Application Load Balancer: Set-Cookie: GCLB=...; Path=/; ...
```
...
Apply this as a `PATCH` request to the backend service API, or use Terraform:
...
HTTP cookie affinity uses a named cookie to determine routing. If the client does not provide that cookie, the load balancer generates it and returns it in a `Set-Cookie` header:
...
This is useful when you want the affinity cookie to have an application-specific name, such as `JSESSIONID`.
...
Internal passthrough Network Load Balancers support IP-based affinity options:
...
For internal HTTP(S) Application Load Balancers (Envoy-based):
