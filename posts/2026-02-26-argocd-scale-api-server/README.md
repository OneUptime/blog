...
With SSL passthrough, TLS terminates at the ArgoCD API server so it can handle both HTTPS and gRPC on the same port. If you want Nginx to enforce Layer 7 features such as request rate limiting, terminate TLS at Nginx and use separate ingress rules for HTTP and gRPC.
...
metadata:
  ...
  namespace: argocd
data:
  # Limit concurrent webhook requests
  server.webhook.parallelism.limit: "50"

  # Limit Kubernetes API client traffic generated by the API server
...
```yaml
# Nginx Ingress annotations for request rate limiting
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: argocd-server
...
Running the API server in insecure mode (without TLS) and terminating TLS at the ingress controller can reduce TLS work on each API server pod.
