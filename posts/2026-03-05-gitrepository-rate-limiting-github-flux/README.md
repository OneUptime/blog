...
When running Flux CD at scale with many GitRepository resources pointing to GitHub, you may encounter throttling or rate limiting. GitHub enforces rate limits on API calls and may throttle excessive Git operations, and Flux's polling behavior can quickly increase the number of Git fetches. This guide explains how to recognize rate limiting issues, adjust your Flux configuration to stay within limits, and use authentication to increase your allowance.
...
- **GitHub App installations:** 5,000 requests per hour by default, scaling up to 12,500 requests per hour for larger non-Enterprise installations, or 15,000 requests per hour for GitHub Enterprise Cloud organizations

Flux performs Git clone and fetch operations, and GitHub App authentication may also use the GitHub API. Each GitRepository reconciliation triggers at least one Git operation.
...
The single most effective measure is to authenticate all GitRepository resources. For GitHub API requests made with a personal access token, this raises your limit from 60 to 5,000 requests per hour.
...
spec:
  ...
  ref:
    branch: main
  secretRef:
    # API requests made with this token get 5,000 req/hour instead of 60
    name: github-token
```

## Step 2: Increase the Reconciliation Interval

An interval of 1 minute is aggressive for many use cases. Increasing it significantly reduces the number of fetches.

Estimate your fetch volume based on the number of GitRepository resources:

```bash
# Formula: fetches_per_hour = (60 / interval_minutes) * number_of_repos
# Example: 20 repos at 1m interval = 1,200 fetches/hour
# Example: 20 repos at 10m interval = 120 fetches/hour
# Example: 20 repos at 30m interval = 40 fetches/hour
```
...
First, create a GitHub App in your organization settings, install it on the repositories Flux needs to read, and generate a private key. Create the secret with the App credentials:
...
stringData:
  githubAppID: "123456"
  # Provide exactly one of githubAppInstallationOwner or githubAppInstallationID
  githubAppInstallationOwner: "your-org"
  githubAppPrivateKey: |
...
Reference the secret from a GitRepository and set `provider: github` so Flux uses GitHub App authentication:
...
Flux can also generate this secret with `flux create secret githubapp`. GitHub App installation tokens expire after one hour, but when you use Flux's GitHub App authentication, the source-controller obtains fresh installation tokens from the App credentials.

## Step 4: Use Webhook Receivers Instead of Polling

Instead of frequent polling, configure GitHub webhooks to notify Flux when changes occur. This reduces unnecessary scheduled Git fetch operations.
...
SSH-based Git operations do not consume GitHub REST API rate limits. They use a separate authentication and connection mechanism, although excessive Git traffic may still be throttled by GitHub.
