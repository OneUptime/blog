...
Description: Learn how to use the Flux Alert eventMetadata.summary field to add custom contextual messages to your notifications for faster incident response.

---

## Introduction

When a Flux alert fires, the default notification contains the event message, resource kind, name, and severity. While informative, these defaults lack organizational context. The `eventMetadata.summary` field in a Flux Alert resource lets you add a custom message to every notification, providing immediate context about what the alert means, who owns it, and what action to take.

This guide shows you how to use the `eventMetadata.summary` field effectively, from simple labels to structured messages that accelerate incident response.

## Prerequisites

Before you begin, ensure you have:

- A Kubernetes cluster running a currently supported Kubernetes minor version
