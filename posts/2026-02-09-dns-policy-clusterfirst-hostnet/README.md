...
Pods running with `hostNetwork: true` use the host's network namespace. With the default `ClusterFirst` DNS policy, Kubernetes treats host network pods like the `Default` policy, so they use the node's DNS configuration. This causes issues when these pods need to resolve Kubernetes service names. The `ClusterFirstWithHostNet` DNS policy solves this by providing cluster DNS resolution while maintaining host network capabilities.
...
- Use the node's DNS configuration unless `dnsPolicy: ClusterFirstWithHostNet` is set
...
- `ClusterFirst`: Use cluster DNS first unless `hostNetwork: true` is set
...
Ingress controllers can be configured to use host network when they need to bind directly to node ports:
...
CNI components commonly need host network access; use `ClusterFirstWithHostNet` when they also need to resolve cluster services:
    ...
    spec:
      ...
      - name: calico-node
        ...
        - name: KUBERNETES_SERVICE_HOST
          value: "kubernetes.default.svc.cluster.local"  # Requires working cluster DNS
  ...
  template:
    ...
    spec:
      ...
      - name: config-reloader
        ...
        # Can resolve a cluster Service name
        env:
        - name: CONFIG_SERVICE
...
Example resolv.conf with ClusterFirstWithHostNet:
...
Combine ClusterFirstWithHostNet with dnsConfig. Kubernetes merges these values with the base DNS configuration generated by the DNS policy:
...
1. Use ClusterFirstWithHostNet for pods with hostNetwork: true when they need Kubernetes DNS
