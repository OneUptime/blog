...
- `cilium` CLI, Hubble CLI, and access to run `cilium-dbg` in Cilium agent pods
...
kubectl get ciliumendpoints -n <NAMESPACE> <POD_NAME> -o json | \
...
kubectl get ciliumidentities <IDENTITY_ID> -o yaml
```
...
- **Policy changes not propagating**: Check the endpoint state and agent logs for regeneration failures before restarting affected workloads.
