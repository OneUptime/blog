...
- **BPF maps are full**: Check connection tracking entries with `cilium-dbg bpf ct list | wc -l` and map capacity with `cilium-dbg map list`. If approaching limits, increase the `bpf.ctTcpMax`, `bpf.ctAnyMax`, or `bpf.mapDynamicSizeRatio` Helm values.
- **Performance issues after configuration change**: Check if BPF program count or runtime has changed. Use `bpftool prog show` on the node to see loaded programs and available runtime statistics.
