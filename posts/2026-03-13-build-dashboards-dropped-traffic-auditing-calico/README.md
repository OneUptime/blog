...
Calico provides multiple observability mechanisms: Felix Prometheus metrics (port 9091), flow logs for connection-level visibility, and integration with Grafana for metrics dashboards. This guide covers how to configure and use these capabilities effectively.
...
# Enable flow logs API and Whisker in Calico Open Source operator or Helm installs
kubectl apply -f - <<'EOF'
...
Calico observability requires enabling Felix Prometheus metrics, enabling flow logs for connection-level data, and building dashboards that surface actionable signals. The three most important operational signals are Felix dataplane failures (indicates dataplane programming errors that will be retried), denied traffic in flow logs or policy metrics (indicates policy misconfiguration or security events), and IPAM utilization (indicates capacity issues). Configure dashboards and alerts for these signals from day one in production clusters.
