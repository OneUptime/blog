...
Both input and output tokens are billed, but at different rates. Azure OpenAI pricing changes over time and varies by model, deployment type, and region, so always check the official Azure OpenAI pricing page before doing financial planning. As an illustration, older GPT-4 and GPT-3.5-era rates were commonly modeled like this:
...
Notice that in this example, GPT-4 output tokens cost twice as much as input tokens. Also notice that smaller models can be dramatically cheaper than larger models. These cost differences are the foundation of most optimization strategies.
...
In the Azure Portal, navigate to your Azure OpenAI resource. Click "Diagnostic settings" and create a new setting that sends metrics to a Log Analytics workspace.

Once enabled, you can query token metrics with KQL (Kusto Query Language):

```kql
// Query prompt and completion token metrics over the last 7 days
AzureMetrics
...
Output tokens often cost more than input tokens for larger models. Setting a `max_completion_tokens` limit prevents the model from generating unnecessarily long responses.

```python
# Set a reasonable max_completion_tokens limit based on your use case
response = client.chat.completions.create(
    model="gpt4-production",
    messages=messages,
    max_completion_tokens=300,  # Limit output to roughly 225 words
    temperature=0.5
)
```
...
Not every request needs your largest model. Many tasks - simple Q&A, text classification, formatting, data extraction - work well with smaller models such as GPT-4o mini, GPT-4.1 mini, or GPT-4.1 nano at a fraction of the cost.
...
def route_request(query, complexity="auto"):
    ...
    Uses a smaller model for simple tasks and a larger model for complex ones.
    """
    if complexity == "auto":
    ...
    model = "gpt4o-mini-deployment" if complexity == "simple" else "gpt4-production"
...
A more sophisticated approach would use a lightweight classifier to determine which model to use, or use a smaller model as a first pass and only escalate to a larger model when confidence is low.
...
def cached_completion(messages, model="gpt4-production", max_completion_tokens=500):
    """
...
If you are processing large volumes of data (like summarizing thousands of documents), use the Azure OpenAI batch API instead of making individual real-time requests when your workload can tolerate asynchronous processing. Global batch processing has separate quota, a 24-hour target turnaround, and is priced at 50% less than global standard processing for supported models.
...
In the Azure AI Foundry portal, go to Deployments, select your deployment, and adjust the TPM allocation. Start conservative and increase as needed.

## Cost Comparison Example

Let us compare costs for a typical chatbot scenario processing 50,000 requests per day with an average of 800 prompt tokens and 200 completion tokens per request, using the illustrative legacy rates from earlier in this post.
...
The difference is staggering. By routing 80% of simple queries to a smaller model and only using GPT-4-class models for complex ones, you could bring that monthly cost down substantially while still delivering high-quality answers where it matters.
