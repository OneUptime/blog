...
kubectl apply -n cb-test -f https://raw.githubusercontent.com/istio/istio/release-1.30/samples/httpbin/httpbin.yaml
...
This says: if an instance returns 3 consecutive 5xx errors, eject it for at least 30 seconds. Up to 100% of instances can be ejected.
...
After the first base ejection time (30 seconds in our config), the ejected instance should be returned to the pool. If it is still unhealthy, it will get ejected again quickly.
...
You should see the ejection count go to 1, then back to 0 after the first base ejection time, then back to 1 when the bad pod gets ejected again.
...
This simulates traffic that sees both delays and errors. The injected aborts are generated by the proxy rather than by a specific upstream pod, so use this test with the bad pod still running if you want to validate outlier ejection. The connection pool should prevent request pile-up from the delays.
...
4. Ejected instances are returned to the pool after the base ejection time for the first ejection
