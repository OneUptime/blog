...
graph LR
    ...
    D -->|200 OK| C
    D -->|Error / Retry| B
    B -->|After max retries| E[Dead Letter Queue]
```
...
You can find the queue URL in the Elastic Beanstalk console under your worker environment's configuration, or programmatically with the AWS CLI.

```bash
# Get the worker queue URL from the environment configuration
aws elasticbeanstalk describe-configuration-settings \
    --application-name my-app \
    --environment-name worker-env \
    --query "ConfigurationSettings[0].OptionSettings[?Namespace=='aws:elasticbeanstalk:sqsd' && OptionName=='WorkerQueueURL'].Value" \
    --output text
```
...
option_settings:
  aws:elasticbeanstalk:sqsd:
    ...
    # How long messages are valid and actively processed
    RetentionPeriod: 345600
    # Error visibility timeout (seconds)
    ErrorVisibilityTimeout: 60
    # How many times to retry before sending to a configured dead-letter queue
    MaxRetries: 3
```
...
If you use the autogenerated queue, Elastic Beanstalk enables a dead-letter queue by default. If you configure the worker environment to use an existing SQS queue, configure the DLQ on that queue in SQS. When a message exceeds MaxRetries and the source queue has a DLQ configured, the message moves to that dead-letter queue. You can monitor this queue and set up alerts.
...
Worker environments can scale with the same Elastic Beanstalk Auto Scaling trigger settings as web environments. The built-in trigger metrics are EC2 and load balancer metrics such as CPU utilization, so use a metric that reflects worker pressure for your workload.

```yaml
# .ebextensions/scaling.config - Auto scaling based on CPU utilization
option_settings:
  aws:autoscaling:asg:
    MinSize: 1
    MaxSize: 10
...
This configuration adds 1 instance when average CPU utilization stays above 70% for the breach duration and removes 1 when it drops below 30%. If you want queue-depth-based scaling, create a CloudWatch alarm or scaling policy that uses the SQS metric for the worker queue, such as `ApproximateNumberOfMessagesVisible`. For more on auto scaling configuration, see our guide on [configuring Elastic Beanstalk auto scaling](https://oneuptime.com/blog/post/2026-02-12-configure-elastic-beanstalk-auto-scaling/view).
...
Worker environments are one of the most underused features in Elastic Beanstalk. They let you offload heavy processing from your web tier without setting up a separate queue consumer infrastructure. The SQS daemon handles the polling, retries, and configured dead-letter routing - you just write an HTTP endpoint that processes messages.

Start with a single worker instance, tune your timeout and concurrency settings based on actual workload, and scale out as needed. The combination of worker auto scaling and periodic tasks via cron.yaml covers most background processing needs.
