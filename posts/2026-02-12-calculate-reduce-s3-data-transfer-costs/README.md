...
S3 storage is cheap. S3 data transfer is where the bill gets painful. A terabyte of Standard storage costs about $23 per month, but transferring that same terabyte out to the internet costs roughly $80-$90 after the monthly free transfer tier. And if you're not paying attention to how your applications access S3, you could be paying transfer charges you don't need to. Let's break down exactly where the costs come from and how to reduce them.
...
| Data out to internet (first 100GB/month, aggregated across most AWS services and Regions) | Free |
| Data out to internet (next 10TB/month) | $0.09/GB |
...
| Data transfer between regions | Often $0.01-$0.02/GB, depending on the regions |
| Data transfer via S3 gateway VPC endpoint (same region) | No additional endpoint charge |
...
aws ce get-cost-and-usage \
  ...
  --filter '{
    "And": [
      ...
      {"Dimensions": {"Key": "USAGE_TYPE_GROUP", "Values": ["S3: API Requests - Standard", "S3: Data Transfer - Internet (Out)", "S3: Data Transfer - Region to Region (Out)"]}}
    ]
  }' \
  --group-by Type=DIMENSION,Key=USAGE_TYPE
```

You can also get a more detailed view using CloudWatch request metrics. S3 request metrics must be enabled for the bucket first, and they are billed at standard CloudWatch rates.
...
billable_gb = max(total_gb - 100, 0)  # first 100GB/month is free across most AWS services
estimated_cost = billable_gb * 0.09  # $0.09/GB for the next 10TB tier
...
This is the single most impactful optimization. Data transfer from S3 to CloudFront is free. CloudFront's data transfer pricing depends on geography and whether you use pay-as-you-go or a flat-rate pricing plan, but it is often cheaper than S3 direct for web delivery. Cached content doesn't even hit S3 at all.
...
If your EC2 instances or Lambda functions access S3 through an internet gateway or NAT gateway path, you might be paying avoidable network processing charges even though same-region S3 data transfer to AWS services is free. An S3 gateway VPC endpoint keeps same-region S3 traffic on the endpoint route with no additional endpoint charge.
...
Cross-region data transfer is commonly around $0.01-$0.02/GB, depending on the source and destination Regions. If your EC2 instances are in us-east-1 but your S3 bucket is in eu-west-1, every byte transferred costs money.
...
Amazon S3 Select is no longer available to new customers, but existing S3 Select customers can continue to use it. Instead of downloading entire objects and filtering client-side, use S3 Select to retrieve only the data you need. This reduces the amount of data transferred.
...
flowchart TD
    ...
    G -->|Partial data needed| H[S3 Select for existing customers]
...
**Before optimization:**
- 50TB internet transfer: first 100GB free, 10,000 GB x $0.09, 39,900 GB x $0.085 = $4,291.50/month
...
- CloudFront transfer: pricing depends on geography and pricing plan
...
- On pay-as-you-go pricing at $0.085/GB, 20,000 GB is $1,700/month; eligible CloudFront flat-rate plans can be lower
- **Total: ~$1,750/month on this pay-as-you-go assumption**

**Savings: ~$2,590/month (60%)**
...
5. Use S3 Select for filtered queries on large files if your account already has access
