...
Tags: Developer Onboarding, Platform Engineering, Developer Experience, Deployment, Metric
...
Publish cohort median and p75 or p90 when they are estimable, not an individual leaderboard. The distribution is usually skewed, and an average is sensitive to a few long onboardings.
...
When a cohort includes censored observations, estimate quantiles from a survival curve rather than from completed cases alone. Report a quantile as not reached when the observation window is too short to estimate it.
...
Statistics: Kaplan-Meier median and p75 when estimable, deployed-within-14-days, censored count
