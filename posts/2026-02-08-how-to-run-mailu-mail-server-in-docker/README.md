...
- At least 1GB of RAM and 1GB of swap without ClamAV, or 3GB of RAM and 1GB of swap with ClamAV enabled
- Ports 25, 80, 110, 143, 443, 465, 587, 993, 995, and 4190 available
...
Mailu provides an online configuration generator at `https://setup.mailu.io/2024.06/`. It creates Docker Compose and environment files based on your answers. However, you can also set things up manually.
...
# Admin interface
# Automatically create the initial admin user if it does not already exist
...
# Antivirus: clamav or none (clamav needs additional RAM)
ANTIVIRUS=none
...
```bash
# Export the DNS records, including DKIM
docker compose exec admin flask mailu config-export --dns domain
...
docker run --rm -v mailu_mailu-dkim:/data -v "$(pwd):/backup" alpine tar czf /backup/mailu-dkim-$(date +%Y%m%d).tar.gz /data
docker compose up -d

# Restore from backup
docker compose down
docker run --rm -v mailu_mailu-mail:/data -v "$(pwd):/backup" alpine tar xzf /backup/mailu-mail-20260201.tar.gz -C /
docker run --rm -v mailu_mailu-data:/data -v "$(pwd):/backup" alpine tar xzf /backup/mailu-data-20260201.tar.gz -C /
docker run --rm -v mailu_mailu-dkim:/data -v "$(pwd):/backup" alpine tar xzf /backup/mailu-dkim-20260201.tar.gz -C /
docker compose up -d
```
