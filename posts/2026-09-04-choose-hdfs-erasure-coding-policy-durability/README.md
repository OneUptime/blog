...
If every shard location independently has probability `p` of being unavailable during the same vulnerability window, the block-group unavailability probability is:

```text
P(EC loss) = sum from i=m+1 to n of C(n,i) * p^i * (1-p)^(n-i)
where n = k + m
```
...
These equations are useful for checking intuition, not for forecasting production durability. They assume identical, independent failures. Real outages are correlated by rack power, top-of-rack switches, storage-controller batches, software defects, maintenance, and operator actions. An object also spans many block groups, multiplying its exposure.
...
Hadoop requires at least `k+m` DataNodes for a full stripe. For rack tolerance, its guide recommends enough racks that no rack holds more internal blocks than the parity count:
...
Replication can normally restore one lost block by reading one surviving full copy. Reconstructing one RS internal block requires reading at least `k` surviving internal blocks, decoding, and sending the recovered block to a target. Hadoop performs those reads in parallel and accounts EC recovery against reconstruction scheduling with an `xmits` weight.
...
Benchmark effective throughput under contention. Network bisection bandwidth, source-disk queues, CPU, coder implementation, and throttling can dominate. `hadoop checknative` shows whether the intended ISA-L acceleration is available to clients and DataNodes.
...
After assigning the candidate policy, write new canaries and query the files:
