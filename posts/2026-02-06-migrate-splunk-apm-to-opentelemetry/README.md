...
Splunk APM has actually been built on top of OpenTelemetry for a while now. Splunk's tracing libraries are distributions of the upstream OpenTelemetry SDKs with Splunk-specific defaults and exporters baked in. This is good news for migration because it means you are already halfway there. The work involved is mostly about removing the Splunk-specific layer and switching to the vanilla OpenTelemetry SDK with standard exporters.
...
Because these are distributions of OpenTelemetry components, the migration is less about rewriting instrumentation and more about unwrapping it. You are peeling off the Splunk layer to get to standard OpenTelemetry underneath.
...
The Splunk Java agent (`splunk-otel-javaagent.jar`) is a wrapper around the standard OpenTelemetry Java agent with Splunk-specific defaults and features. Replacing it is straightforward because the auto-instrumentation coverage largely overlaps.
...
const sdk = new NodeSDK({
  ...
  // Auto-instrumentation covers a similar set of libraries as Splunk's distribution
  instrumentations: [
...
This configuration provides common host metrics functionality using standard components.

```yaml
# Standard OpenTelemetry Collector configuration
# Replaces the Splunk OTel Collector for common OTLP and host metrics pipelines
receivers:
  otlp:
    protocols:
      grpc:
...
exporters:
  ...
  # Splunk's OTLP/HTTP ingest endpoints accept standard traces and metrics
  otlphttp/splunk:
...
service:
  pipelines:
    traces:
      ...
      exporters: [otlphttp/splunk, otlphttp/new]
    metrics:
      receivers: [otlp]
      processors: [memory_limiter, batch]
      # Both exporters receive the same metric data
      exporters: [otlphttp/splunk, otlphttp/new]
```
...
**Log Observer Connect**: If you use Splunk's log correlation, configure OpenTelemetry log instrumentation or your logging framework to include trace ID and span ID in your logs. This gives you the same trace-to-log correlation.
