...
The detector runs on a schedule, aggregates data for each interval, feeds it to the RCF model, and produces an anomaly grade and confidence score. A non-zero anomaly grade means the data point was flagged as anomalous, with higher grades indicating more severe anomalies.
    ...
    "feature_attributes": [
        {
            ...
            "aggregation_query": {
                "error_count": {
                    "sum": {
                        "script": {
                            "source": "doc['level.keyword'].size() != 0 && doc['level.keyword'].value == 'ERROR' ? 1 : 0"
                        }
                    }
                }
            }
        ...
        {
            "feature_name": "max_response_time",
            "feature_enabled": true,
            "aggregation_query": {
                "max_response_time": {
                    "max": {
...
curl -XPOST "https://search-domain.us-east-1.es.amazonaws.com/_plugins/_anomaly_detection/detectors/results/_search" \
    -H "Content-Type: application/json" \
    -d '{
    "size": 20,
    "query": {
    ...
    "triggers": [
        {
            ...
            "condition": {
                "script": {
                    "source": "ctx.results[0].aggregations.max_anomaly_grade.value != null && ctx.results[0].aggregations.max_anomaly_grade.value >= 0.7",
                    "lang": "painless"
                }
            },
            "actions": [
                {
                    ...
                    "message_template": {
                        "source": "Anomaly detected! Max grade: {{ctx.results[0].aggregations.max_anomaly_grade.value}} in the last 5 minutes."
                    }
                }
            ]
        }
