...
```bash
kubectl exec -n production authorized-test-pod -- ping -c 3 <target-pod-ip>
echo "Result: $?"
```
...
Configure ICMP Rules policies in Calico requires attention to policy ordering, selector accuracy, and rule coverage for the traffic direction you want to control. Follow the patterns in this guide to ensure your ICMP Rules policies are correctly configured, tested, and monitored. Always validate in staging before applying to production, and maintain comprehensive logging for visibility into policy decisions.
