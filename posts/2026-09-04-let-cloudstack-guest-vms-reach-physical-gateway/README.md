...
ip neigh
ping -c 3 192.0.2.1
sudo arping -I GUEST_INTERFACE -c 3 192.0.2.1
traceroute -n EXTERNAL_TEST_IP
```

Replace `EXTERNAL_TEST_IP` with a reachable external destination approved for testing; documentation ranges are not Internet test targets.

If the network uses CloudStack-managed DHCP, the offered gateway must match the network record. If addressing is external, verify that only authorized DHCP servers answer and that any redundant servers coordinate allocations. On a CloudStack-managed shared network, a static guest address must match the address allocated to its NIC in CloudStack; on an L2 network with external IPAM, reserve the address there before use.

## Locate a Layer-2 Break

If ARP for the gateway fails, capture the same request at successive points. These commands assume Linux bridges with a VLAN subinterface attached to a per-VLAN guest bridge. Use the tap and source bridge reported by `virsh domiflist`, and replace `VM_MAC` with the test NIC's MAC address:

```bash
# KVM host: identify the guest interface first
sudo virsh domiflist GUEST_DOMAIN

sudo tcpdump -eni GUEST_TAP 'ether host VM_MAC and (arp or icmp)'
sudo tcpdump -eni VM_SOURCE_BRIDGE 'ether host VM_MAC and (arp or icmp)'
sudo tcpdump -eni PHYSICAL_UPLINK 'vlan 210 and ether host VM_MAC and (arp or icmp)'
```

The per-VLAN bridge carries untagged frames; the physical trunk carries VLAN 210. For VLAN-aware bridges or Open vSwitch, identify the actual forwarding path and adjust the capture points and filters. VLAN offload can affect tag visibility in host captures, so verify the capture setup before treating missing packets as proof of a network failure.

Interpretation:

- Request absent on the tap: check the guest NIC/configuration and that the correct tap and filter are selected.
...
For a new network that fails, stop the test VM, remove it and the unused network through CloudStack, then remove the VLAN from trunks. For an existing production network, restore the previous switch/bridge mapping instead of deleting the network; revert an offering change only through a supported CloudStack offering update. A VLAN assigned to an isolated network remains associated for its lifecycle when explicitly assigned.
