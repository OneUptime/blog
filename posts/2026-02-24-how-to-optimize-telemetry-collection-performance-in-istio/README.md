...
Many Prometheus setups use a 15-second scrape interval, which is aggressive for many workloads. If your dashboards show 5-minute rate queries, you do not need 15-second granularity.
...
If you are running Prometheus 3.0 or newer with native histogram support enabled, you can reduce the number of time series generated by classic histogram metrics by converting them to native histograms at scrape time:

```yaml
apiVersion: monitoring.coreos.com/v1
kind: PodMonitor
metadata:
  name: istio-proxies-native-histograms
  namespace: monitoring
spec:
  selector:
    matchLabels:
...
Native histograms store the count, sum, and buckets in one histogram sample instead of separate bucket time series, which can reduce histogram-related time series by 10-20x. For Prometheus 3.0 through 3.7, you also need to enable the `native-histograms` feature flag on the Prometheus server.
...
Apply them in that order. Start with the changes that have the biggest impact and lowest risk. The Telemetry API changes can be applied without restarting any pods, while ProxyConfig and sidecar resource changes require workloads to be restarted before they take effect.
