...
-- Identity syntax varies by database. For example, MySQL uses AUTO_INCREMENT
-- and SQL Server commonly uses IDENTITY.
...
-- Exact MERGE syntax varies by database; this example assumes the
-- surrogate key is generated by an identity/sequence/default.
...
WHEN MATCHED AND (
    ...
    target.address_line1 IS DISTINCT FROM source.address_line1 OR
    target.city          IS DISTINCT FROM source.city          OR
    target.state         IS DISTINCT FROM source.state         OR
    target.country       IS DISTINCT FROM source.country
) THEN UPDATE SET
...
PostgreSQL 15 and later support `MERGE`. For PostgreSQL, `INSERT ON CONFLICT` is still a concise UPSERT pattern for Type 1 SCD:
...
WHERE
    dim_customer.first_name    IS DISTINCT FROM EXCLUDED.first_name    OR
    dim_customer.last_name     IS DISTINCT FROM EXCLUDED.last_name     OR
    dim_customer.email         IS DISTINCT FROM EXCLUDED.email         OR
    dim_customer.phone         IS DISTINCT FROM EXCLUDED.phone         OR
    dim_customer.address_line1 IS DISTINCT FROM EXCLUDED.address_line1 OR
    dim_customer.city          IS DISTINCT FROM EXCLUDED.city          OR
    dim_customer.state         IS DISTINCT FROM EXCLUDED.state         OR
    dim_customer.country       IS DISTINCT FROM EXCLUDED.country;
```
...
SELECT
    ...
    email AS new_email,
    phone AS new_phone,
    address_line1 AS new_address_line1,
    city AS new_city,
    state AS new_state,
...
WITH source_customers AS (
    ...
    {% if is_incremental() %}
    WHERE modified_date > (
        SELECT COALESCE(MAX(source_modified_at), CAST('1900-01-01' AS TIMESTAMP))
        FROM {{ this }}
    )
    {% endif %}
),
  ...
  - name: dim_customer
    ...
    columns:
      ...
      - name: source_modified_at
        description: Timestamp when the source record was last modified
        tests:
          - not_null
```
...
WHEN MATCHED AND (
    ...
    target.{{ col }} IS DISTINCT FROM source.{{ col }}{{ ' OR' if not loop.last else '' }}
    {% endfor %}
) THEN UPDATE SET
    {% for col in columns_to_update %}
...
ALTER TABLE dim_customer ADD row_checksum VARBINARY(32);
...
SELECT
    ...
    -- SQL Server HASHBYTES returns varbinary; SHA2_256 avoids deprecated algorithms.
    HASHBYTES('SHA2_256',
        CONCAT_WS('|',
            COALESCE(first_name, ''),
            COALESCE(last_name, ''),
...
WHEN MATCHED AND (
    ...
    OR target.row_checksum IS NULL
)
THEN UPDATE SET
    target.first_name    = source.first_name,
    target.last_name     = source.last_name,
...
USING (
    ...
    WHERE ABS(CHECKSUM(customer_id)) % 4 = 0
) AS source
ON target.customer_id = source.customer_id
...
USING (
    SELECT * FROM stg_customer
    WHERE ABS(CHECKSUM(customer_id)) % 4 = 1
) AS source
ON target.customer_id = source.customer_id
WHEN MATCHED AND /* change detection */ THEN UPDATE SET /* ... */
WHEN NOT MATCHED THEN INSERT /* ... */;
...
-- Syntax varies by database; INCLUDE is supported by SQL Server and PostgreSQL.
...
```sql
-- SQL Server example: wrap MERGE in transaction for atomicity
BEGIN TRY
    BEGIN TRANSACTION;
