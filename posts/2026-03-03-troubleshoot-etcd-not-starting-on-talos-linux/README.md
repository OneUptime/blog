...
## Failure: etcd Data Directory Problems

etcd stores its data on disk. On Talos Linux, this is typically at `/var/lib/etcd`. If the disk is missing, corrupted, locked by another process, contains data from an old cluster, or was wiped during a reset, etcd will fail to start:
...
talosctl -n <cp-ip> list /var/lib/etcd/member/
```
...
```bash
# Reset the node and reboot it back into maintenance mode
...
If the database is over its quota or heavily fragmented, check the etcd alarm and database status, then defragment it on a healthy control plane node:

```bash
# Check for a NOSPACE alarm
talosctl -n <cp-ip> etcd alarm list

# Check database size and in-use size
talosctl -n <cp-ip> etcd status

# Defragment the database on the node
talosctl -n <cp-ip> etcd defrag

# Clear the NOSPACE alarm after fixing the space issue
talosctl -n <cp-ip> etcd alarm disarm
```
...
If certificates are expired or invalid, rotate the affected certificates or regenerate your cluster configuration from the same secrets and re-apply it. Make sure all control plane nodes get updated configurations at the same time.

## Failure: Network Issues Between Control Plane Nodes

etcd uses port 2379 for client requests and port 2380 for peer communication. The peer port must be reachable between all control plane nodes; if nodes cannot reach each other, the cluster will fail to maintain quorum.
...
Verify that your firewall rules or security groups allow peer traffic on port 2380 between all control plane nodes.
...
```bash
# First, wipe the EPHEMERAL partition on all control plane nodes and reboot them
talosctl -n <cp-1> reset --graceful=false --reboot --system-labels-to-wipe=EPHEMERAL
talosctl -n <cp-2> reset --graceful=false --reboot --system-labels-to-wipe=EPHEMERAL
talosctl -n <cp-3> reset --graceful=false --reboot --system-labels-to-wipe=EPHEMERAL
