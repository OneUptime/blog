...
rule [family="ipv4|ipv6"] [priority="priority"]
  ...
  [audit]
  [accept [limit value="rate/duration"]|reject [type="type"] [limit value="rate/duration"]|drop [limit value="rate/duration"]|mark set="mark[/mask]"]
```
...
This allows the first three new SSH connection attempts per minute to match the accept action. Make sure there is no broader SSH allow rule in the same zone, or later zone behavior could still allow the traffic.
...
# Log and drop everything else (for auditing) after the allow rules have run
firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" priority="32767" log prefix="DB-DROPPED: " level="info" limit value="10/m" drop' --permanent
...
```bash
# Reject with the default firewalld reject type
...
# Block ping from everywhere else after the allow rule has run
firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" priority="32767" icmp-type name="echo-request" drop' --permanent
