...
- `cilium` CLI, `cilium-dbg`, and Hubble CLI available
...
kubectl get cnp,ccnp --all-namespaces -o json | \
  jq '.items[] | {scope: (.metadata.namespace // "cluster"), name: .metadata.name, kind: .kind}'
```
...
kubectl get ciliumendpoints --all-namespaces -o json | \
  ...
  ) | {namespace: .metadata.namespace, name: .metadata.name, identity: .status.identity.id}'
```
...
kubectl get cnp,ccnp --all-namespaces -o json | jq -r '.items[] | "\(.kind)/\(.metadata.namespace // "cluster")/\(.metadata.name)"'
```
...
- **Inconsistent data across nodes**: Ensure all Cilium agents are running the same version with `cilium status` and `cilium-dbg version` inside the agent pods.
