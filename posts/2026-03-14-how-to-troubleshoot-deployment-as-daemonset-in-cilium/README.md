...
- `cilium` CLI, Hubble CLI, and access to `cilium-dbg` in the Cilium agent pods
...
- **Policy changes not propagating**: Check whether endpoint regeneration has completed with `kubectl -n kube-system exec <CILIUM_POD> -- cilium-dbg policy wait <POLICY_REVISION>`.
