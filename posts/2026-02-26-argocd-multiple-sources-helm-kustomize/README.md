...
Helm and Kustomize serve different purposes. Helm packages applications as reusable charts with templating, while Kustomize patches existing YAML without templates. Combining them in a single ArgoCD application lets you use Helm for the base application and Kustomize to add environment-specific companion resources without forking the chart.

## Why Combine Helm and Kustomize

Here are real scenarios where this combination shines:

- You deploy a third-party Helm chart but need to add companion resources with annotations or labels that the chart does not create
- You want to add separate sidecar-related resources, such as ConfigMaps, Secrets, or RBAC, alongside a Helm-deployed application
- You need to create policy or monitoring resources beyond what the chart's values expose
...
The Helm chart source renders templates into manifests. The Kustomize source provides additional resources, or patches resources defined within that same Kustomize source. ArgoCD combines all manifests and applies them together.
...
spec:
  ...
  sources:
    ...
    # Git repo providing values files
    - repoURL: https://github.com/your-org/k8s-configs.git
      targetRevision: main
...
If you need to patch Helm output, you have three alternatives:
...
**Option 3:** Deploy the Helm chart as a Kustomize base using `helmCharts` in the kustomization.yaml. This allows Kustomize patches on Helm output but requires a single-source approach. In ArgoCD, this also requires enabling Helm support for Kustomize with `kustomize.buildOptions: --enable-helm` or a config management plugin:
