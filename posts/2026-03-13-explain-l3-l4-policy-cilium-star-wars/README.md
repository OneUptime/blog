...
The key data structures involved are the endpoint policy map (keyed by identity, port, protocol, and traffic direction) and the connection tracking (CT) table. The policy map determines whether a new connection is permitted based on the peer identity and L4 attributes. Once a connection is allowed, entries in the CT table permit subsequent packets in that flow without re-evaluating the policy map. This is how Cilium achieves O(1) policy lookup regardless of the number of rules.
...
kubectl exec -n kube-system ds/cilium -- cilium-dbg endpoint list | grep $XWING_IP
```
...
graph LR
    ...
    CM -->|New connection| PM[Policy Map Lookup<br/>identity + port + protocol + direction]
    PM -->|Allow entry found| NEW[Create CT Entry + Forward]
    PM -->|No entry| DROP[Drop Packet]
```

```bash
# Inspect policy BPF maps
...
At L3/L4, the policy decision is based on security identities and L4 attributes such as destination port and protocol, with connection tracking maintaining state for the flow tuple. It does not look at the TCP payload. This means:

- A permitted connection (`tiefighter` to `deathstar:80`) allows any HTTP method and path
- `PUT /v1/exhaust-port` is allowed by the same L3/L4 rule as `POST /v1/request-landing`
...
Cilium's L3/L4 policy enforcement is technically elegant: an eBPF lookup against a compiled policy map determines whether a new TCP connection is permitted. The mechanism is fast, scalable, and identity-aware. But it is fundamentally limited to connection-level decisions. Understanding this limitation - that TCP connection permission does not imply HTTP path permission - is what makes the progression to L7 policy in the Star Wars demo both inevitable and instructive.
