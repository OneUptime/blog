...
The `datetime()` query is a preliminary screen for SQLite-supported date/time text or Julian day values, not a strict timestamp validator. Unix seconds require `datetime(created_at, 'unixepoch')`; other epoch units need explicit scaling. Validate the intended format separately because SQLite also accepts time-only values and `now`.
...
| --- | --- |
| `INTEGER PRIMARY KEY` | `bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` or preserve external IDs with a primary-key constraint |
...
Load into PostgreSQL staging tables with `COPY`, then transform into final tables with explicit casts and validation. Server-side `COPY FROM` with a filename reads paths visible to the database server and requires appropriate privileges; client-side `\copy` in `psql` reads from the client. Choose deliberately and protect exported data.
...
For a smaller pause, establish durable change capture before taking the base snapshot, coordinate its log position with the snapshot, and replay subsequent committed changes in order and idempotently. SQLite does not provide a built-in logical replication stream equivalent to PostgreSQL, so the application must own and test that change log.
