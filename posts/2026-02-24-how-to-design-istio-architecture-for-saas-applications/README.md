...
The shared services namespace needs to reach tenant namespaces. Istio `Sidecar` hosts use `namespace/dnsName` entries, so tenant namespace names need to be listed explicitly or generated by your onboarding automation.
...
For coarse local rate limiting at the API gateway sidecar level:
