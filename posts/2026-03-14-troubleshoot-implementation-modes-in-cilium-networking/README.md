...
- **BPF maps are full**: Check map pressure with `cilium-dbg metrics list | grep bpf_map_pressure` or conntrack entries with `cilium-dbg bpf ct list global | wc -l`. If approaching limits, increase conntrack table size in Helm values.
- **Performance issues after configuration change**: Check whether BPF map pressure or drop counters increased. Use `cilium-dbg metrics list | grep -iE "bpf_map_pressure|drop_count|mtu"` to inspect related metrics.
