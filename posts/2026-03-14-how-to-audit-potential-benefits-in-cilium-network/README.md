...
kubectl get ciliumendpoints --all-namespaces -o json | \
  jq '[.items[] | .status.policy.realized] | length'

# Identify endpoints without any policy
kubectl get ciliumendpoints --all-namespaces -o json | \
  jq '.items[] | select(
    .status.policy.realized."policy-enabled" == "none" or
    .status.policy.realized."policy-enabled" == null
  ) | {
...
kubectl -n kube-system get pods -l k8s-app=cilium -o name | while read pod; do
  ...
  kubectl -n kube-system exec "$pod" -c cilium-agent -- \
    sh -c 'for key in enable-policy policy-audit-mode enable-l7-proxy enable-hubble; do cilium-dbg config get "$key" 2>/dev/null || true; done'
done
```
...
COVERED=$(kubectl get ciliumendpoints --all-namespaces -o json | \
  jq '[.items[] | select(
    .status.policy.realized."policy-enabled" != "none" and
    .status.policy.realized."policy-enabled" != null
  )] | length')
...
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg identity list | head -30
```
...
- **Policy count mismatch**: Some policies may be in a failed state. Check with `kubectl describe cnp -A | grep -E "Status|Valid|Error"`.
