...
Validating IPAM release operations requires confirming three outcomes: the specific IP is no longer assigned in the IPAM database, the overall IPAM consistency check still passes in Calico environments that provide `calicoctl ipam check`, and the IP becomes available for new pod allocation. Skipping post-release validation is the most common source of ongoing IPAM inconsistencies.
...
kubectl get endpointslices --all-namespaces -o yaml | grep "${IP}"
...
for ip in $(cat release-candidates.txt); do
  if kubectl get pod --all-namespaces -o wide | grep -q "${ip}" || \
     kubectl get endpointslices --all-namespaces -o yaml | grep -q "${ip}"; then
