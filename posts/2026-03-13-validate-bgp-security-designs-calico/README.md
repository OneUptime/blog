# How to Validate BGP Security Designs in Calico

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, BGP, Security, Networking

Description: Validate BGP security configurations in Calico by testing authentication, verifying filter effectiveness, and auditing route advertisements.
...
Key BGP security controls include: MD5 session authentication, prefix length limits (rejecting unusually short or long prefixes), import/export route filters (rejecting routes from unexpected sources or peers), and RPKI (Resource Public Key Infrastructure) validation for public internet routing. For internal Kubernetes clusters, the first two are most relevant.
...
Ensure the `calico-node` ServiceAccount can read the secret in the namespace where `calico-node` runs:
