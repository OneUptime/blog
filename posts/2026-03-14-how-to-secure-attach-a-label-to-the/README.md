...
Securing traffic to and from labeled Kubernetes nodes is essential for maintaining a robust Kubernetes network security posture. Cilium leverages eBPF technology to provide deep visibility and control over network traffic, making it possible to enforce fine-grained security policies at the kernel level.

This guide focuses on practical steps to harden your label-based node policies using CiliumClusterwideNetworkPolicy host policies. You will learn how to create policies that restrict access, implement defense-in-depth strategies, and verify that your security controls are working as intended.
...
- Cilium host firewall enabled when enforcing policies with `nodeSelector`
- Basic understanding of Kubernetes networking concepts

## Understanding the Security Model

Before implementing security controls, it is important to understand how Cilium uses node labels in host policies. Cilium does not control who can attach or change Kubernetes node labels; protect that operation with Kubernetes RBAC and admission controls.
...
Apply a CiliumClusterwideNetworkPolicy to restrict traffic to and from nodes selected by label.
...
kubectl get ciliumclusterwidenetworkpolicy node-label-policy
```

### Hardening with Default-Deny

Implement a default-deny baseline for workloads in the `production` namespace to ensure no traffic flows unless explicitly allowed:
...
hubble observe --output json --last 500 | \
  jq -r '.flow
    | select(.source.namespace? and .destination.namespace?)
    | select(.source.namespace != .destination.namespace)
    | [
        .source.namespace,
        .destination.namespace,
        (.l4.TCP.destination_port // .l4.UDP.destination_port // "unknown")
      ]
...
kubectl get ciliumclusterwidenetworkpolicy node-label-policy -o yaml
```
...
- **Policy not taking effect**: Verify node labels match the host policy selector with `kubectl get nodes --show-labels`.
...
Securing label-selected node traffic in Cilium requires a layered approach: implement default-deny baselines, create specific allow policies for legitimate traffic, and continuously monitor with Hubble. By following the steps in this guide, you have established strong security controls for your node-selected workloads. Remember to regularly review and update your policies as your application architecture evolves, and always test changes in a staging environment before applying them to production.
