...
Deploy the Flagger load testing service used by the rollout webhook.
...
Deploy your application with a Deployment. Flagger will create the ClusterIP services used for traffic shifting.
...
## Configuring the HTTPRoute

Flagger creates and manages the HTTPRoute that routes traffic through the Gateway to your application. The route settings are configured in the Canary resource with `hosts` and `gatewayRefs`.

```yaml
# Example of the HTTPRoute generated by Flagger
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: podinfo
...
Create the Canary resource that configures HTTPRoute traffic management.
  ...
  analysis:
    ...
    webhooks:
      - name: load-test
        ...
        metadata:
          cmd: "hey -z 1m -q 10 -c 2 -host app.example.com http://podinfo-canary.test:9898/"
```
