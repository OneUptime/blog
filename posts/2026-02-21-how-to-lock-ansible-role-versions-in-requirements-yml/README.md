...
Version ranges are supported for collections, but not for roles.
...
collections:
  ...
  # Version range (install any 3.x version)
...
For even more control, adopt a two-file pattern similar to `package.json` / `package-lock.json`. Ansible does not generate a role lock file for you, and role version ranges are not supported, so keep role versions exact in both files:

```yaml
# requirements.yml - human-edited, specifies exact role versions
roles:
  - name: geerlingguy.docker
    version: "6.1.0"
  - name: geerlingguy.nginx
...
```yaml
# requirements.lock.yml - exact versions used by CI
roles:
  - name: geerlingguy.docker
    version: "6.1.0"
  - name: geerlingguy.nginx
...
```bash
# Install the exact versions from requirements.yml
...
- name: Install Ansible role dependencies
  run: |
    ansible-galaxy role install -r requirements.yml --force -p roles/
