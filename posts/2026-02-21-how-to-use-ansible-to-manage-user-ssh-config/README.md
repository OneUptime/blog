...
The `mode: '0600'` keeps the config readable only by the target user, which is a good default if the file contains private hostnames, usernames, or key paths.
...
flowchart TD
    ...
    C -->|"Host prod-db"| D[Apply host-specific settings first]
    C -->|"Host *"| E[Apply wildcard defaults later]
    D --> F[Merge with defaults]
    E --> F
    F --> G["Connect to db1.prod.internal:22 as postgres"]
```

Host entries are processed in order. The first matching value wins for each parameter, so put more specific host entries near the beginning of the file and wildcard (`Host *`) defaults near the end.
...
For system-wide defaults, use `/etc/ssh/ssh_config` or `/etc/ssh/ssh_config.d/` on systems that include that directory from the main SSH client config:
...
- name: Configure SSH for CI/CD
  ...
  tasks:
    ...
    - name: Deploy CI/CD SSH config
      ansible.builtin.template:
        src: templates/cicd_ssh_config.j2
...
The template file (`templates/cicd_ssh_config.j2`):
...
1. **Set permissions to 0600** on SSH config files when they contain private hostnames, usernames, or key paths. SSH is stricter about private key permissions, but keeping user config private is still a good default.
...
7. **Use `/etc/ssh/ssh_config.d/` for system-wide defaults** on systems that include that directory from `/etc/ssh/ssh_config`. User-specific config is read first, so per-user values can still take precedence.
