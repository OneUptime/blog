...
# Remove a host key
argocd cert rm git.internal.example.com --cert-type ssh
```
...
```bash
# Open a shell in the repo server pod
kubectl exec -it -n argocd deployment/argocd-repo-server -- sh

# Test SSH connectivity
ssh -T -o StrictHostKeyChecking=yes git@github.com
# If the host key is trusted, the connection gets past host key verification.
# Authentication may still fail if the matching private key is not available.
