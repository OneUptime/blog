# How to Enable ESXi PCI Passthrough and Fix Devices Stuck at `Needs Reboot`
...
- vMotion;
- snapshots while the VM is powered on;
- suspend and resume;
- Fault Tolerance.
...
- install OEM-supported BIOS and device firmware, and use a supported ESXi image and host-driver combination;
...
Never pass through the controller that contains the ESXi boot device, the only management NIC, or another device required by VMkernel. Passing through a boot storage controller can make the boot device unavailable to ESXi after startup and prevent configuration changes from persisting.

Multi-function devices can have reset dependencies. For reset methods with bus-level granularity, Broadcom supports VMDirectPath I/O only when all PCI functions on the same bus are assigned collectively to the same VM. If functions within a multi-function device have a functional dependency, all dependent functions must also be assigned together. Do not configure or assign only one function of a GPU, NIC, or controller until the device and OEM documentation confirm the supported grouping.
...
If the card was moved, reseated, replaced, or rediscovered after firmware or BIOS work on an ESXi 7.0 or 8.0 host matching Broadcom KB 445893, compare its live Device Layer Bus Address with the stored ConfigStore record:
...
If this scenario also left stale SR-IOV virtual-function records for the same device, identify and delete each confirmed VF record separately with its own `-i` value.
...
- **Device disappears when a passthrough VM powers on:** Broadcom says an ESXi 8.0 hotplug-reset issue is resolved in ESXi 8.0 Update 3g (`80P06`). Upgrade to that release or a later ESXi 8.0 Update 3 patch instead of repeatedly re-adding the device.
...
- [GPU shows Shared Direct or Basic and will not enter passthrough on ESXi](https://knowledge.broadcom.com/external/article/445893)
...
- [Bootbank pointing to /tmp due to storage adapter/device being marked as passthrough device](https://knowledge.broadcom.com/external/article/373820)
