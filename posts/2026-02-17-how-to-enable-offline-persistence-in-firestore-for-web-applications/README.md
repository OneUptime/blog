...
When your app writes data while offline, Firestore queues the operation locally and updates the local cache right away. Snapshot listeners see that local change immediately, while the write promise resolves after the server acknowledges the write. This means your UI can stay responsive without waiting for the network.
...
function saveNote(noteId, content) {
  ...
  // Local listeners update immediately, even offline. The promise resolves
  // after the write has been acknowledged by the backend.
  setDoc(noteRef, {
    content: content,
    updatedAt: new Date(),
  ...
  console.log('Note queued locally if offline');
}

// Snapshot listeners work with cached data too
// They fire immediately with cached data, then update when server data arrives
const unsubscribe = onSnapshot(doc(db, 'notes', 'note-1'), { includeMetadataChanges: true }, (snapshot) => {
...
Firestore provides a way to listen for when your snapshot listeners have all observed the same change. This does not mean the snapshots are in sync with the server, so combine it with snapshot metadata and browser APIs for a complete picture.

```javascript
// Monitor when active snapshot listeners are in sync with each other
// This helps keep listener-driven UI state consistent
import { onSnapshotsInSync, enableNetwork, disableNetwork } from 'firebase/firestore';

// Listen for when all active snapshot listeners have processed the same change
const unsubscribe = onSnapshotsInSync(db, () => {
  console.log('All active snapshot listeners are in sync with each other');
});
...
onSnapshot(doc(db, 'notes', 'note-1'), { includeMetadataChanges: true }, (snapshot) => {
...
Offline persistence in Firestore relies on IndexedDB and is officially supported on Chrome, Safari, and Firefox. Safari in private browsing mode has limited IndexedDB support. Some older or unsupported browsers may not support it at all.
...
Another frequent issue is data conflicts. If a user edits a document offline and someone else edits the same document on the server, the last write wins by default. If you need more sophisticated conflict resolution, use merge logic or transactions for online read-modify-write flows. Transactions fail while the client is offline, so they are not a complete offline conflict-resolution strategy by themselves.
