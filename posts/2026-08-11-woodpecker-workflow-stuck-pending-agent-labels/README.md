...
A Woodpecker workflow becomes `pending` after the server has accepted its event and configuration but before an agent has started it. Scheduling is label-based: an agent may take a workflow only when every workflow label matches an agent label or wildcard. Platform and backend are labels too.

The fastest diagnosis is to copy the workflow's effective labels, copy the labels from each connected agent, and compare them key by key. If at least one agent matches, move on to capacity, concurrency, and approval. If none matches, waiting will never fix the queue.

## Distinguish the Queue from Backend Scheduling

There are two different “pending” states:

1. **Woodpecker workflow pending**: no agent has accepted the workflow.
2. **Kubernetes Pod pending**: a Kubernetes-backend agent accepted it and created a Pod, but the Kubernetes scheduler has not placed that Pod.

If no workflow steps or Kubernetes Pod exist, debug Woodpecker agent selection. If a labeled Pod exists, the Woodpecker label match already succeeded; inspect PVCs, node selectors, resources, taints, service accounts, and image pulls in Kubernetes.
...
Do not remove a security label merely to drain the queue. A label such as `tier=trusted` may be the boundary keeping signing or deployment credentials on isolated workers.
...
Woodpecker's documented platform format is `GOOS/GOARCH`, such as `linux/amd64` or `linux/arm64`. Check the agent's actual detected platform in the admin UI or startup log. Container image multi-architecture support does not change the agent's platform label.
...
For a multi-platform matrix, each permutation should request the agent platform that can execute it. A normal workflow is assigned to one agent, so a step-level platform condition cannot move later steps to a different architecture.
...
Quote numeric or boolean-looking workflow values so YAML does not change their type:
...
Every workflow carries its repository identity. The normal agent label `repo=*` permits any repository accepted by the instance and other labels.
...
`WOODPECKER_MAX_WORKFLOWS` defaults to 1. A matching agent already running one workflow cannot take another until capacity is free. Increasing the value raises concurrency on that agent:
...
- long-running detached services;
- stuck cleanup that has not released a slot.

A label mismatch remains pending indefinitely. A capacity queue drains when running work completes. Observe whether older matching workflows progress.
...
## A Minimal Portable Test

To prove basic agent connectivity, create a temporary workflow with no custom labels:
...
Run it manually on a safe branch. If it starts, at least one default agent match and capacity path works. Add the intended platform, then backend, then custom labels one at a time. The first addition that leaves it pending identifies the mismatch.
...
10. Check workflow concurrency and autoscaler state.
11. Restart/roll out agents after label changes.
12. Preserve security labels instead of weakening them for convenience.
...
A pending Woodpecker workflow needs one connected agent whose labels satisfy the complete required set. Compare platform, backend, repository, custom values, wildcards, and mandatory agent labels directly. If a match exists, check capacity, concurrency, and approval. If a Kubernetes Pod already exists, stop changing Woodpecker labels and debug Kubernetes scheduling instead.
