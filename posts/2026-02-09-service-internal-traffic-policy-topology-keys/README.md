...
- `Local`: Traffic only goes to endpoints on the same node as the client pod, and is dropped if there are no node-local endpoints
...
Now pods calling api-server only hit api-server pods on their same node. If a client pod is on a node with no api-server pod, kube-proxy drops the traffic.
...
You should only see responses from backend pods on the same node as the client pod. If the client lands on a node with no backend pod, the requests will fail instead of falling back to another node.

## Topology-Aware Routing with Topology Keys

Topology keys provided more granular control than just node-local routing. They let you prefer endpoints based on topology labels like zone, region, or custom labels.

Service topology keys were an alpha feature that was deprecated in Kubernetes 1.21 and removed after Kubernetes 1.22. Understanding them is still valuable when reading older manifests, but they should not be used on current clusters.
...
On clusters that supported Service Topology, this configuration created a preference order: same node first, then same zone, then any node.

## Traffic Distribution (Current Approach)

The modern approach uses the Service `trafficDistribution` field to express topology preferences:
...
With this field, kube-proxy and the EndpointSlice controller work together to prefer endpoints in the same zone when possible. On older clusters, the related topology-aware routing feature used the `service.kubernetes.io/topology-mode: Auto` annotation, and the pre-1.27 annotation was `service.kubernetes.io/topology-aware-hints`.
...
4. For internal Service traffic, `internalTrafficPolicy` must not be set to `Local`

If these conditions aren't met, hints won't be allocated and routing falls back to the default cluster-wide behavior.
...
For internal traffic, `internalTrafficPolicy: Local` takes precedence over topology preferences. Choose based on your needs:

- `internalTrafficPolicy: Local`: Strict node-local routing, no cross-node traffic
- `trafficDistribution: PreferSameZone`: Best-effort zone-local routing with cluster-wide fallback
...
`internalTrafficPolicy: Local` takes precedence over topology preferences for internal traffic.

### Traffic Still Goes Cross-Zone

If topology hints are allocated but traffic still crosses zones:

1. Verify kube-proxy is running a Kubernetes version that supports the topology feature you configured:

```bash
kubectl get daemonset -n kube-system kube-proxy -o jsonpath='{.spec.template.spec.containers[*].image}'
```

2. Check kube-proxy logs for hint processing:

```bash
kubectl logs -n kube-system -l k8s-app=kube-proxy --tail=200 | grep -i topology
```
...
```yaml
# High-traffic service - prefer same-zone traffic
apiVersion: v1
kind: Service
metadata:
...
- Use `trafficDistribution: PreferSameZone` for multi-zone clusters with even pod distribution
