...
export class TracingService {
  ...
  private initTracing(): void {
    ...
    // Zone.js context manager preserves context across async operations.
    // Check your build target because it is intended for ES2015-compatible output.
...
export class RouterTracingService {
  ...
  private navigationContexts: Map<number, Context> = new Map();
  private routeConfigLoadSpans: Map<string, Span> = new Map();
  ...
  private initializeRouterTracing(): void {
    ...
    this.router.events.pipe(
      ...
      this.navigationContexts.set(event.id, trace.setSpan(context.active(), span));
    });

    // Track lazy route configuration loading
    this.router.events.pipe(
      filter(event => event instanceof RouteConfigLoadStart)
    ).subscribe((event: RouteConfigLoadStart) => {
      const path = event.route.path ?? '(unnamed route)';
      const navigation = this.router.getCurrentNavigation();
      const parentContext = navigation
        ? this.navigationContexts.get(navigation.id)
        : undefined;
      const span = tracer.startSpan('angular.route_config_load', {
        attributes: {
          'route.path': path,
          'module.name': event.route.data?.['moduleName'] ?? path,
          'module.type': 'lazy',
        }
      }, parentContext);
      this.routeConfigLoadSpans.set(path, span);
    });
    ...
    this.router.events.pipe(
      ...
      const path = event.route.path ?? '(unnamed route)';
      const span = this.routeConfigLoadSpans.get(path);
      if (span) {
        span.setStatus({ code: SpanStatusCode.OK });
        span.end();
        this.routeConfigLoadSpans.delete(path);
      }
    });
    ...
    this.router.events.pipe(
      ...
      if (span) {
        ...
        this.navigationContexts.delete(event.id);
      }
    });
    ...
    // Handle skipped navigation, such as a same-URL navigation
    this.router.events.pipe(
      filter(event => event instanceof NavigationSkipped)
    ).subscribe((event: NavigationSkipped) => {
      const span = this.navigationSpans.get(event.id);
      if (span) {
        span.setAttribute('navigation.skipped', true);
        span.setAttribute('navigation.reason', event.reason);
        span.setStatus({ code: SpanStatusCode.OK });
        span.end();
        this.navigationSpans.delete(event.id);
...
Lazy loading is where things get interesting. Angular loads route configuration and module chunks on demand, and this download time can be significant. Use the router's `RouteConfigLoadStart` and `RouteConfigLoadEnd` events, as shown in the router tracing service above, to measure the lazy route loading window.
...
export const dataResolver: ResolveFn<any> = (
  route: ActivatedRouteSnapshot,
  state: RouterStateSnapshot
) => inject(DataResolver).resolve(route, state);
```

## Visualizing Navigation Flow

Here's how the trace hierarchy can look for a typical lazy-loaded route navigation when child spans are started with the navigation span's context:

```mermaid
graph TD
    A[angular.navigation] --> B[guard.auth]
    A --> C[angular.route_config_load]
...
**Use Context Propagation**: Zone.js wraps Angular asynchronous operations, and the ZoneContextManager can preserve trace context through those zones. Check your JavaScript build target because the OpenTelemetry Web SDK notes that ZoneContextManager is intended for ES2015-compatible output.
...
OpenTelemetry can add low overhead when you keep instrumentation focused, batch exports, and sample appropriately. The exact cost depends on how many spans you create, how much attribute data you attach, browser performance, and exporter behavior. The ZoneContextManager uses Zone.js to preserve context across asynchronous work, but you should still measure overhead in your own application.
