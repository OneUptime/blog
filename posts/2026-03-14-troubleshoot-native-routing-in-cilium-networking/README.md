...
```bash
# Check IP-to-identity cache entries used by the datapath
kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf ipcache list | head -20
...
If Hubble is enabled and the Hubble CLI can reach the Hubble API, use it to trace traffic flows:

```bash
# Observe all flows related to a specific pod
hubble observe --pod default/diag-pod --last 20

# Filter for dropped flows
hubble observe --verdict DROPPED --last 20

# Filter by protocol
hubble observe --protocol tcp --last 20

# Check for policy-related drops
hubble observe --verdict DROPPED --type policy-verdict --last 20
```
...
- **BPF maps are full**: Check connection tracking entries with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf ct list | wc -l`. If approaching limits, increase conntrack table size in Helm values.
- **Performance issues after configuration change**: Check the loaded BPF programs with `bpftool prog show | grep -i cilium` on the affected node, or inspect Cilium datapath metrics with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf metrics list`.
