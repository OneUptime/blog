...
A proper validation process tests connectivity from multiple pods on different nodes, verifies that supporting infrastructure like kube-proxy (if used) and Calico Felix is healthy, and confirms that the fix survives component restarts. Rushed validation is a common cause of incident recurrence.
...
kubectl run validate-same-node --image=nicolaka/netshoot \
  --overrides='{"apiVersion":"v1","spec":{"nodeName":"<endpoint-node>"}}' \
  --restart=Never --attach --rm --command -- curl -s --connect-timeout 5 -o /dev/null -w "%{http_code}" \
...
kubectl run validate-ip --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
kubectl run validate-dns --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
RESOLVED_IP=$(kubectl run dns-check --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
# Optional legacy Endpoints compatibility check
kubectl get endpoints <service-name> -n <namespace> -o yaml
```

## Step 4: Infrastructure Health Validation

Confirm that kube-proxy, if your cluster uses it, and Calico components are healthy after the fix.

```bash
# Verify kube-proxy on all nodes, if your cluster uses kube-proxy
...
# Check Calico pods are running (use kube-system instead for non-operator installs)
...
# Validate iptables rules on a sample node when kube-proxy is in iptables mode
...
kubectl run load-test --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
kubectl run concurrent-test --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
```bash
# Restart kube-proxy and re-validate, if your cluster uses kube-proxy
kubectl rollout restart daemonset/kube-proxy -n kube-system
kubectl rollout status daemonset/kube-proxy -n kube-system

# Re-test connectivity after restart
kubectl run post-restart-test --image=nicolaka/netshoot --restart=Never --attach --rm --command -- \
...
kubectl get svc --all-namespaces -o json | \
  jq -r '.items[] | select(.spec.type == "ClusterIP" and .spec.clusterIP != "None" and .spec.ports != null) | "\(.metadata.namespace) \(.metadata.name) \(.spec.ports[0].port)"' | \
  head -10 | while read ns name port; do
    echo -n "$ns/$name: "
    kubectl run "check-$name" --image=busybox --rm --attach --restart=Never --command -- \
...
- [ ] kube-proxy pods healthy on all nodes, if kube-proxy is used
- [ ] Calico node status shows no issues
- [ ] No deny entries in Felix logs
- [ ] kube-proxy or Calico eBPF service dataplane has programmed the service
- [ ] 100 consecutive requests succeed
- [ ] Fix persists after kube-proxy restart, if kube-proxy is used
