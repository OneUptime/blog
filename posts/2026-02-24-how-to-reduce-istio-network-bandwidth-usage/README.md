...
3. **Outlier detection work**: Envoy periodically evaluates upstream endpoints for ejection
...
```promql
# xDS configuration bytes generated per second
sum(rate(pilot_xds_config_size_bytes_sum[5m]))

# Average xDS response size
sum(rate(pilot_xds_config_size_bytes_sum[5m])) / sum(rate(pilot_xds_config_size_bytes_count[5m]))
```
...
Setting `maxRequestsPerConnection: 0` allows unlimited requests per connection, which means connections are reused extensively. The `h2UpgradePolicy: DEFAULT` setting uses the mesh-wide HTTP/2 upgrade policy; set it to `UPGRADE` when you want this destination to opt in to HTTP/2 upgrades explicitly.
...
## Reducing Outlier Detection Work

Envoy outlier detection periodically analyzes upstream endpoint health and can eject unhealthy endpoints from the load balancing pool. This is passive detection based on observed failures, not active health-check traffic between proxies.

Configure outlier detection intervals to be less aggressive:
...
Increasing the outlier detection interval from the default 10s to 30s reduces how often Envoy runs ejection sweep analysis. The tradeoff is slower detection of failed endpoints, which is acceptable for some services.
...
# xDS configuration bytes generated by the control plane
sum(rate(pilot_xds_config_size_bytes_sum[5m]))
