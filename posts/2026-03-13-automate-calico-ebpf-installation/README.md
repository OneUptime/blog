...
- name: Create eBPF ConfigMap
  kubernetes.core.k8s:
    ...
    definition:
      ...
      data:
        KUBERNETES_SERVICE_HOST: "{{ calico_api_server_host }}"
        KUBERNETES_SERVICE_PORT: "{{ calico_api_server_port | default('6443') }}"

- name: Wait for Tigera Operator
  ansible.builtin.command: kubectl rollout status deploy/tigera-operator -n tigera-operator --timeout=120s
...
- name: Wait for Calico to be ready
  ansible.builtin.command: kubectl wait --for=condition=Available tigerastatus/calico --timeout=300s
  retries: 3
  delay: 30

- name: Validate eBPF is active
  ansible.builtin.shell: >
    kubectl logs -n calico-system ds/calico-node -c calico-node --tail=200
    | grep -q "BPF enabled, starting BPF endpoint manager and map manager"
  changed_when: false
```
...
Automating Calico eBPF installation through Terraform, Ansible, and CI/CD ensures every cluster in your fleet starts with eBPF correctly configured. The automation handles the three critical prerequisites - compatible kernel, no kube-proxy, stable API server endpoint in the ConfigMap - as integrated steps rather than manual checklists. By adding automated validation as the final step, you get immediate confirmation that eBPF is actually active, not just configured.
