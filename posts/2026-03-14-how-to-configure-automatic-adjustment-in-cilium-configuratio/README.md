...
bpf:
  ...
  # Connection tracking map capacity settings
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel version is 4.19 or later. Check init container logs with `kubectl logs -n kube-system <pod> -c <init-container-name>`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl -n kube-system exec ds/cilium -- cilium-dbg policy get`. Verify node routing state with `kubectl -n kube-system exec ds/cilium -- cilium-dbg bpf ipcache list`.
