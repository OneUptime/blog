...
- A running Kubernetes cluster with a Cilium release that supports your Kubernetes version
...
- Prometheus and Grafana for metrics visualization, with Prometheus Operator CRDs if you enable ServiceMonitor resources (recommended)
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the host meets Cilium's current kernel requirements. Check the init container names with `kubectl describe pod -n kube-system <pod>`, then inspect the relevant init container logs with `kubectl logs -n kube-system <pod> -c <init-container-name>`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl exec -n kube-system ds/cilium -- cilium-dbg policy get`. Verify inter-node tunnel connectivity with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf tunnel list`.
