...
You've hit a service quota limit. Maybe Lambda's 1,000 concurrent execution limit is throttling your API, or you need more than 5 VPCs in a region, or your auto-scaling group can't launch new instances because you've hit the EC2 On-Demand vCPU limit. Whatever the case, you need to request an increase.
...
```bash
# Request an increase for EC2 On-Demand Standard instance vCPUs
aws service-quotas request-service-quota-increase \
  --service-code ec2 \
  --quota-code L-1216C47A \
  --desired-value 500
...
- **NOT_APPROVED** - Request couldn't be approved by Service Quotas
- **CASE_CLOSED** - Associated support case was closed
- **INVALID_REQUEST** - Resource-level request couldn't be processed

## Requesting via Support Case

For quotas not yet available in the Service Quotas service, or when you need urgent assistance, use the AWS Support Center to create a service quota increase case. The AWS Support `create-case` API does not support service limit increase requests directly.

```bash
# The AWS Support API cannot submit service limit increase cases.
# Open the Support Center and choose a service quota increase case:
...
quota_requests = [
    {'service': 'ec2', 'quota': 'L-1216C47A', 'value': 500, 'name': 'On-Demand Standard instance vCPUs'},
...
**Use the right support level.** Quota increase requests don't receive priority support, but if you have urgent production impact, the severity levels available in your support plan determine Support response times.
...
2. **Use another instance quota as a workaround** - if you're hitting an EC2 On-Demand vCPU quota for standard instance families, you might have headroom in another quota such as Spot Instances or a specialized instance family
...
```bash
# Emergency: open the Support Center and create an urgent service quota case:
# https://console.aws.amazon.com/support/home#/case/create
```
