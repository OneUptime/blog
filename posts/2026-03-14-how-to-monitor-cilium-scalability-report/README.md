...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- curl -s http://localhost:9962/metrics | head -10
```
...
# Policy metrics
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "policy|policy_implementation_delay"
...
- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get cnp,ccnp --all-namespaces`. Verify inter-node health with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-health status`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl top pods -n kube-system -l k8s-app=cilium --no-headers`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl get ciliumendpoints --all-namespaces`. Confirm the policy is applied with `kubectl get cnp,ccnp --all-namespaces`. Check that the endpoint has the correct identity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint get <id>`.
