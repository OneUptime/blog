...
Istio's sidecars expose workload metrics on port 15020 at the `/stats/prometheus` endpoint. Port 15090 exposes Envoy-only Prometheus telemetry. The key metrics you will care about include:
...
This tells the Datadog Agent to look for the Istio proxy image and scrape its Prometheus endpoint.
...
Once the pipeline is running, head over to the Datadog Metrics Explorer. Search for `istio_requests_total` or `istio.mesh.request.count.total` (Datadog normalizes metric names). You should see metrics flowing in within a few minutes.
...
1. **Request rate by service** - Graph `istio_requests_total` or `istio.mesh.request.count.total` grouped by `destination_service`
2. **Error rate** - Graph `istio_requests_total` or `istio.mesh.request.count.total` filtered by `response_code:5xx` divided by total requests
...
- **Filter metrics early.** Istio generates hundreds of metric series per pod. Use Prometheus `metric_relabel_configs` in OTel or `exclude_metrics` in the Datadog config to only send what you actually use. Otherwise your Datadog bill will spike.
