...
systemctl show httpd.service -p LimitNOFILE -p Restart -p RestartUSec
...
- If the service fails to start, check the logs with `journalctl -u httpd.service -e --no-pager`.
