...
```bash
# Add the Project Contour Helm repository
...
Create a root HTTPProxy resource that defines the external host and includes the HTTPProxy generated by Flagger. Flagger will manage the included HTTPProxy during canary analysis to shift traffic weights.
...
Once the Canary resource is applied, Flagger initializes the canary by creating the primary Deployment, the apex, primary, and canary Services, and a generated HTTPProxy with weighted routing.

```bash
# Check the canary status
kubectl get canary -n test

# Inspect the generated HTTPProxy to see Flagger's routing
