...
- It provides a columnar engine for analytical queries on transactional data
- It supports many PostgreSQL extensions
...
Use a schema conversion tool such as AWS Schema Conversion Tool or pgloader, or do it manually for smaller databases:
...
## Step 5: Migrate Data Using pgloader

Google Cloud Database Migration Service does not currently support Aurora MySQL to AlloyDB for PostgreSQL as a heterogeneous migration path. For this engine-to-engine migration, use a tool such as pgloader, or use a staged export and load process after you convert the schema.
