...
With an IPv4 underlay, VXLAN typically adds 50 bytes of overhead per packet (outer IP, UDP, VXLAN, and inner Ethernet headers) and operates on UDP port 4789. IP-in-IP adds 20 bytes of overhead (an additional IPv4 header) and operates as protocol 4. IP-in-IP in Calico supports IPv4 only, while VXLAN can also support IPv6 underlays on supported kernels. VXLAN is more likely to traverse NAT devices because it uses UDP, while IP-in-IP may be blocked by firewalls unfamiliar with protocol 4.
...
|---------|-------|----------|
| Overhead with IPv4 underlay | 50 bytes | 20 bytes |
