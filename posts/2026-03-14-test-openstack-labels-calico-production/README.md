...
Labels drive network policy in Calico, and in an OpenStack environment, Calico adds labels to workload endpoints for the VM's project, network, namespace, and security groups. Testing labels in a production-like environment catches issues with endpoint label generation, policy selector logic, and the impact of security group changes on active traffic.

This guide provides a structured test plan for validating label-based policies in OpenStack with Calico, from verifying endpoint labels through testing policy enforcement based on those labels. Each test validates a specific aspect of the label-to-policy pipeline.
...
- VMs in projects, networks, and security groups that produce distinct Calico endpoint labels
- `calicoctl` and `openstack` CLI tools configured
- OpenStack credentials scoped to the `label-test` project for VM creation
...
Create VMs with specific security groups to test policy enforcement.

```bash
# Create security groups that Calico exposes as endpoint labels
openstack security group create web-pci
openstack security group create db-pci
openstack security group create web-staging

# Keep OpenStack security groups from blocking the positive Calico policy test
openstack security group rule create --protocol tcp --dst-port 5432 \
  --remote-group web-pci db-pci
openstack security group rule create --protocol tcp --dst-port 5432 \
  --remote-group web-staging db-pci

# Create VMs with different security groups

openstack server create --flavor m1.small --image ubuntu-22.04 \
  --network test-network \
  --security-group web-pci \
  label-test-web-1

openstack server create --flavor m1.small --image ubuntu-22.04 \
  --network test-network \
  --security-group db-pci \
  label-test-db-1

openstack server create --flavor m1.small --image ubuntu-22.04 \
  --network test-network \
  --security-group web-staging \
  label-test-web-staging
```

## Testing Label Propagation

Verify that OpenStack project, network, namespace, and security group labels appear correctly on Calico workload endpoints.
...
check_label "label-test-web-1" "projectcalico.org/openstack-project-name" "label-test"
check_label "label-test-web-1" "projectcalico.org/openstack-network-name" "test-network"
check_label "label-test-web-1" "sg-name.projectcalico.org/openstack-web-pci" "web-pci"
check_label "label-test-db-1" "sg-name.projectcalico.org/openstack-db-pci" "db-pci"
check_label "label-test-web-staging" "sg-name.projectcalico.org/openstack-web-staging" "web-staging"
...
# Policy: Only VMs in the web-pci security group can reach VMs in the db-pci security group
apiVersion: projectcalico.org/v3
kind: GlobalNetworkPolicy
metadata:
  name: pci-db-access
spec:
  selector: "has(sg-name.projectcalico.org/openstack-db-pci)"
  types:
    - Ingress
  ingress:
    ...
    - action: Allow
      source:
        selector: "has(sg-name.projectcalico.org/openstack-web-pci)"
      protocol: TCP
      destination:
        ports:
          - 5432
...
# Ensure there is a TCP listener for the connectivity test
ssh ubuntu@${DB_PCI_IP} "nohup python3 -m http.server 5432 >/tmp/label-test-db.log 2>&1 &"
...
Verify that changing a VM's security group correctly updates policy enforcement.

```bash
#!/bin/bash
# test-label-change.sh
# Test that security group changes update policy enforcement

echo "=== Label Change Impact Test ==="

# Move staging VM into the PCI web security group
openstack server remove security group label-test-web-staging web-staging
openstack server add security group label-test-web-staging web-pci

# Wait for endpoint label updates
sleep 15
...
- **Labels not appearing on Calico endpoints**: Check that the OpenStack-to-Calico integration is configured. Verify the Neutron Calico plugin is creating the documented project, network, namespace, and security group labels.
...
- **Label changes not taking effect**: Felix re-evaluates policies when endpoint labels change, but propagation from OpenStack to Calico may have delay. Check Felix logs for label update events.
