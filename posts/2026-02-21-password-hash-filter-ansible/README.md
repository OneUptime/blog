...
The `$6$` prefix indicates SHA-512 crypt. The salt is generated randomly, and Ansible computes the hash with passlib or the system crypt backend.
...
- name: SHA-512 (portable default for password_hash)
...
For Linux user passwords, **sha512** is a common portable choice and is also the default `password_hash` algorithm in Ansible. Some newer distributions now default to yescrypt for locally created `/etc/shadow` passwords, but `password_hash` does not currently expose yescrypt as one of its built-in hash type choices. Check your distribution's password hashing policy if you need to match hashes generated by local tools such as `passwd`.
...
- name: Create user with idempotent password hash
  ansible.builtin.user:
    name: deploy
    password: "{{ user_password | password_hash('sha512', 'myFixedSalt123', rounds=5001) }}"
    shell: /bin/bash
```

With a fixed salt and explicit rounds, the same password always produces the same hash, so the `user` module only reports a change when the password actually changes.
...
- name: Create users with per-user salts
  ansible.builtin.user:
    name: "{{ item.name }}"
    password: "{{ item.password | password_hash('sha512', 65534 | random(seed=item.name) | string, rounds=5001) }}"
...
The `random(seed=item.name)` generates a deterministic "random" number based on the username, giving each user a stable per-user salt.
...
You can use the filter in Jinja2 templates, for example when generating password files for services that accept crypt-style hashes:

```yaml
# auth_file.yml - Generate password file
...
```jinja2
{# auth_file.j2 - crypt-style password file #}
{# Note: For actual htpasswd files, use the community.general.htpasswd module instead #}
{% for user in web_users %}
{{ user.name }}:{{ user.password | password_hash('sha512', 65534 | random(seed=user.name) | string, rounds=5001) }}
{% endfor %}
```
...
- name: Create database user
  community.postgresql.postgresql_user:
    ...
    encrypted: true
    state: present
  # PostgreSQL handles its own hashing; use password_hash only
...
More rounds means the hash takes longer to compute, which slows down brute-force attempts. Defaults vary depending on whether Ansible uses passlib or the system crypt backend, so specify rounds explicitly when you need stable output across control nodes. For high-security accounts, you might want to increase this to 100000 or more. Be aware that higher round counts also make login slightly slower.
...
- name: Create deploy user
  ansible.builtin.user:
    name: deploy
    password: "{{ vault_deploy_password | password_hash('sha512', 65534 | random(seed='deploy') | string, rounds=5001) }}"
  no_log: true
```
  ...
  tasks:
    - name: Generate password hash
      ansible.builtin.set_fact:
        test_hash: "{{ 'testpassword' | password_hash('sha512', 'testsalt', rounds=5001) }}"
...
**Random salt causing unnecessary changes.** Without a fixed salt, every playbook run generates a different hash, causing the user module to update the password every time. Use a per-user deterministic salt and explicit rounds.
...
The `password_hash` filter is essential for managing Linux user passwords in Ansible. Use SHA-512 as a portable default algorithm, provide per-user deterministic salts and explicit rounds for idempotency, store actual passwords in Ansible Vault, and always use `no_log: true` to keep passwords out of your logs. With these practices in place, you can automate user management securely across your infrastructure.
