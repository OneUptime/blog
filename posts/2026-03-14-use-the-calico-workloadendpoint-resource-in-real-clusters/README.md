...
For clusters with fewer than 50 nodes, the WorkloadEndpoint resources generated by the Calico CNI plugin usually work well without direct changes:

```bash
# Verify current WorkloadEndpoint resources

calicoctl get workloadendpoint -A -o yaml

# Check Calico's view of a specific node
calicoctl get node <node-name> -o yaml
```

Start with the defaults and only customize higher-level Calico configuration resources when you have a measured reason to change them. In Kubernetes clusters, WorkloadEndpoint lifecycle is normally handled by the CNI plugin, so direct edits should be rare.

## Pattern 2: Multi-Environment Configuration

In clusters that run workloads across multiple environments (dev, staging, production), you can use node labels and selector-scoped configuration to apply different settings:
...
Then use these labels in selector-scoped FelixConfiguration resources when you need environment-specific Felix settings. WorkloadEndpoint resources themselves do not have a nodeSelector field; use their labels for policy selection, and use node labels for node-scoped Calico configuration.
...
# Review Calico Kubernetes controller reconciliation settings
calicoctl get kubecontrollersconfiguration default -o yaml
```

Key considerations at scale:
- Tune Kubernetes controller reconciliation intervals only after measuring API server load and convergence time
...
kubectl get workloadendpoints.projectcalico.org -A -w

# Set up alerts on calico-node restarts
kubectl get events -n calico-system --field-selector reason=BackOff --watch
```

Consider checking Felix health endpoints if Felix health checks are enabled and the health port is reachable:
...
After reviewing WorkloadEndpoint resources and configuring related Calico resources for your production use case, run a comprehensive check:
...
- Verify the resource is present: `calicoctl get workloadendpoint -A -o yaml`.
...
- Review whether Kubernetes controller reconciliation intervals are too aggressive.
- Consider enabling Typha if not already in use.

**Inconsistent behavior across nodes:**
- Verify selector-scoped FelixConfiguration resources are matching the intended nodes.
