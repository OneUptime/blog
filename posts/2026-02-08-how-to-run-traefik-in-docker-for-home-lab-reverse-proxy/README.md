...
graph LR
    ...
    B --> C{Host Header}
    C -->|grafana.lab.yourdomain.com| D[Grafana Container]
    C -->|nextcloud.lab.yourdomain.com| E[Nextcloud Container]
    C -->|portainer.lab.yourdomain.com| F[Portainer Container]
```

## Prerequisites

You need Docker and Docker Compose on your server. For automatic SSL certificates from Let's Encrypt, use a publicly registered domain that you control. You can use a DNS challenge for internal-only services, as long as Let's Encrypt can validate the public DNS TXT challenge. If you only need local access, self-signed certificates or a local certificate authority like mkcert work fine.
...
services:
  traefik:
    ...
    labels:
      ...
      - "traefik.enable=true"
      - "traefik.http.routers.dashboard.rule=Host(`traefik.lab.yourdomain.com`)"
...
environment:
  ...
  # Optional if your DNS token does not include zone read permissions
...
```bash
# Generate an Apache MD5 hash for basic auth (install apache2-utils if needed)
...
services:
  grafana:
    ...
    labels:
      ...
      - "traefik.http.routers.grafana.rule=Host(`grafana.lab.yourdomain.com`)"
...
- "traefik.http.routers.dashboard.tls.domains[0].main=lab.yourdomain.com"
- "traefik.http.routers.dashboard.tls.domains[0].sans=*.lab.yourdomain.com"
```
...
192.168.1.100  traefik.lab.yourdomain.com
192.168.1.100  grafana.lab.yourdomain.com
192.168.1.100  nextcloud.lab.yourdomain.com
192.168.1.100  portainer.lab.yourdomain.com
```

A better approach is running a DNS server like Pi-hole or AdGuard Home that returns a wildcard record for *.lab.yourdomain.com pointing to your Traefik host.
