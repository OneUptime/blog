...
Managing user accounts manually does not scale. When someone joins the company, they need accounts in Google Workspace or Cloud Identity, access to specific GCP projects, membership in the right groups, and the correct IAM roles. When they leave, all of that needs to be revoked - quickly. SCIM (System for Cross-domain Identity Management) automates this lifecycle by connecting your identity provider to the systems that manage user accounts and access.

This guide covers implementing SCIM-based provisioning with Google Workspace and Cloud Identity APIs so that user accounts and group memberships stay in sync with your authoritative identity source.

## How SCIM Works with Google Cloud

SCIM is a REST API standard for managing user identities across systems. Your identity provider (like Okta, Microsoft Entra ID, OneLogin, or Ping Identity) usually acts as the SCIM client. For Google Workspace and Cloud Identity directory accounts, many IdPs use a native Google connector backed by Google's Admin SDK Directory API rather than a generic Google Cloud Identity SCIM endpoint. If your IdP only speaks SCIM, you can put a small SCIM bridge in front of the Admin SDK Directory API. When a user is created, updated, or deleted in the identity provider, the connector or bridge sends the corresponding API call to Google, and the change is reflected in the directory.

The main benefits are that account creation is automated when someone joins, account deactivation happens automatically when someone leaves, group memberships stay synchronized with your HR system or identity provider, and there is a single source of truth for identity.
...
# Check Cloud Identity groups are searchable
gcloud identity groups search \
  --organization=ORG_ID \
  --labels="cloudidentity.googleapis.com/groups.discussion_forum" \
  --format="table(displayName,groupKey.id)"
```
...
Every SCIM integration needs a SCIM endpoint URL (provided by your target application or by the bridge you deploy), an authentication token or OAuth credentials, attribute mappings between your IdP schema and Google's schema, and provisioning rules for when to create, update, or deactivate accounts.

### Okta Example

```bash
# In Okta, Google Workspace provisioning is available through a pre-built connector
# You need:
# 1. A Google Workspace or Cloud Identity admin account
# 2. Admin SDK API access enabled for your domain

# Enable the Admin SDK API in the Google Cloud project
gcloud services enable admin.googleapis.com \
  --project=PROJECT_ID

# Create a service account for Directory API automation
gcloud iam service-accounts create scim-provisioning \
  --display-name="SCIM Provisioning Service" \
  --project=PROJECT_ID

# Create a JSON key if your connector or bridge cannot use keyless credentials
gcloud iam service-accounts keys create scim-key.json \
  --iam-account=scim-provisioning@PROJECT_ID.iam.gserviceaccount.com

# In the Google Admin console, authorize domain-wide delegation for the
# service account's client ID and the exact Admin SDK scopes you need.
```

## Step 3: Build a Custom SCIM Bridge (If Needed)

If your identity provider does not have a native Google Workspace or Cloud Identity connector, build a SCIM bridge using Cloud Run.
...
def generate_temp_password(length=32):
    """Generate a temporary password that satisfies Workspace complexity rules."""
...
def format_as_scim_group(google_group):
    """Convert Google Directory group to SCIM format."""
    return {
        "schemas": ["urn:ietf:params:scim:schemas:core:2.0:Group"],
        "id": google_group["id"],
        "displayName": google_group["name"],
        "members": [],
    }
```
...
```python
def transfer_drive_ownership(user_email, new_owner_email):
    """Transfer Drive files using the Google Workspace Data Transfer API."""
    print(f"Start a Data Transfer API job from {user_email} to {new_owner_email}")

def log_deprovisioning_event(user_email, org_id):
    """Write the offboarding event to your audit log."""
    print(f"Logged deprovisioning event for {user_email} in {org_id}")

def schedule_account_deletion(user_email, days):
    """Schedule deletion after your organization's retention period."""
    print(f"Scheduled deletion for {user_email} in {days} days")
...
def deprovision_user(user_email, org_id):
    ...
    # Step 3: Revoke all OAuth tokens issued to third-party applications
    tokens_response = directory_service.tokens().list(userKey=user_email).execute()
    for token in tokens_response.get("items", []):
        directory_service.tokens().delete(
            userKey=user_email,
...
Automated user provisioning with SCIM eliminates one of the biggest security risks in any organization - orphaned accounts. When the identity provider is the single source of truth and your connector or bridge keeps Google Cloud in sync, you can be confident that when someone leaves, their access is revoked consistently without waiting for manual cleanup. The initial setup takes some work, but the ongoing operational savings and security improvements are well worth it.
