...
# Make the route permanent with your host OS or network manager
# (for example: netplan, NetworkManager, or an ifupdown routes file).
```

Option 2: BGP peering with external hosts running Bird:

```bash
# On a machine with Kubernetes API access: configure Calico to peer with the external host
kubectl apply -f - << 'EOF'
...
```bash
# From a machine with Kubernetes API access, get the pod IP
