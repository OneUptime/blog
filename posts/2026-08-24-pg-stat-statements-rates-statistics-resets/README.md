...
Sample the view in each database where the extension is installed. On PostgreSQL 17 and later, include both the module-wide reset timestamp and the entry's `stats_since` timestamp:
...
Suppose `monitoring.pgss_samples` stores the preceding fields. A reset-aware calls-per-second calculation is:
...
WITH ordered AS (
  ...
  WINDOW w AS (
    PARTITION BY instance_id, dbid, userid, toplevel, queryid
    ORDER BY sampled_at
  )
)
SELECT sampled_at,
...
Return `NULL` across a boundary instead of zero. Zero means a valid interval with no increase; null means the rate is unknown. Apply the monotonic check independently to every counter used in a calculation.
...
- absence beyond the allowed scrape gap;
- a PostgreSQL major upgrade.

The `dealloc` count is global and does not identify which entry was evicted. It is evidence of churn, not enough to repair a particular missing series.
...
Prometheus `rate()` handles a decrease in a continuously present counter as a reset. It cannot know that a newly appearing label set is the same logical query after eviction, migration, or a changed query ID. Export only monotonic cumulative fields as counters, and expose reset metadata and deallocation as separate metrics.

Avoid `query` as a label. Even normalized text can be long and high-cardinality, and query text may expose identifiers or literals depending on how it was generated. Use the bounded identity labels for metrics and place sanitized text in a separate lookup keyed by query ID. Remember that two semantically different statements can share a normalized entry, while identical text can have different identities under different roles, databases, search paths, or top-level settings.
