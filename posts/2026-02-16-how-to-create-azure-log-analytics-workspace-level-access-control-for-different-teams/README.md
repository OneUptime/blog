...
- **Log Analytics Data Reader**: Can run queries and read workspace data, and is the best built-in role to combine with granular RBAC conditions
...
Assign broad workspace roles only to teams that should see all tables in the workspace:

```bash
# Give the operations team broad read access to the workspace
az role assignment create \
  --assignee operations-team@company.com \
...
Granular RBAC is the most granular control option and can restrict access at the table or row level. The role actions below show the legacy table-specific method, which still works for standard tables; for new designs, Microsoft recommends granular RBAC with the `Microsoft.OperationalInsights/workspaces/tables/data/read` data action and role assignment conditions.
...
Now the security team can only query the tables listed in the role definition, as long as they do not also have a broader Reader, Contributor, or Log Analytics Reader assignment that grants access to all workspace data. Queries against any other table will return an access denied error.
...
This means a developer with Reader access to `web-app-prod` can query from the resource context and will only see logs generated by `web-app-prod`. They cannot see logs from `database-prod` or `vm-security-01` unless they also have access to those resources.
