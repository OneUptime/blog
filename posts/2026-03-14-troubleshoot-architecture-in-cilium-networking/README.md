...
kubectl exec -n kube-system ds/cilium -- bpftool prog 2>/dev/null | head -20
...
kubectl exec -n kube-system ds/cilium -- hubble observe flows --type policy-verdict --last 20
```
...
- **Performance issues after configuration change**: Check whether the loaded BPF programs have changed. Use `bpftool prog` to see loaded programs and their translated and JIT-compiled sizes.
