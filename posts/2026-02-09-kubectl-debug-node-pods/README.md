...
The kubectl debug node command creates debug pods on specific nodes with host namespace access. This enables deep system-level debugging without SSH access. Use it to inspect node file systems, check system logs, analyze network configuration, and debug kubelet issues. The debug pod runs with host PID, network, and IPC namespaces and mounts the node root filesystem at `/host`, but it is not privileged by default. Use the `--profile=sysadmin` flag or create a privileged pod manually if you need privileged access.
