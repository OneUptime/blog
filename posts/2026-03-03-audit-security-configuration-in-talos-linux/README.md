...
#
# talosctl get machineconfig redacts CA/secret fields by default, so we read
# the Talos API CA from the local talosconfig, the Kubernetes CA from the
# local kubeconfig, and the live API server certificate from the endpoint.
...
# Check Talos API CA from local talosconfig
TALOS_CTX=$(yq '.context' ~/.talos/config)
TALOS_CA_PEM=$(yq ".contexts.\"$TALOS_CTX\".ca" ~/.talos/config | base64 -d)
TALOS_CA_EXPIRY=$(echo "$TALOS_CA_PEM" | \
...
# Check Kubernetes CA from local kubeconfig
K8S_CA_PEM=$(kubectl config view --raw \
  -o jsonpath='{.clusters[0].cluster.certificate-authority-data}' | base64 -d)
K8S_CA_EXPIRY=$(echo "$K8S_CA_PEM" | \
  openssl x509 -noout -enddate 2>/dev/null | cut -d= -f2)
...
# Check the live API server certificate served on :6443
API_CERT_EXPIRY=$(echo | openssl s_client -connect ${CONTROL_PLANE}:6443 \
  -servername kubernetes 2>/dev/null | \
  openssl x509 -noout -enddate 2>/dev/null | cut -d= -f2)
...
echo ""
echo "=== Key Size Audit ==="
TALOS_KEY_SIZE=$(echo "$TALOS_CA_PEM" | \
...
# Get the kube-apiserver static pod resource generated by Talos
API_CONFIG=$(talosctl -n $CONTROL_PLANE get staticpods kube-apiserver -o yaml)
