...
cmk list events type=PROXY.START level=ERROR
cmk list events type=SSVM.START level=ERROR
cmk list asyncjobs
```

CloudMonkey command spelling can vary between profiles and versions; `cmk help listSystemVms` shows the locally supported parameters. Search the exact UUID and job ID in the management log:
...
- No libvirt domain is present: investigate allocation, template readiness, storage, or an agent command failure. Check the logs for a domain that was created and then cleaned up after a failed start.
...
```bash
cmk list hosts zoneid=ZONE_UUID type=Routing
cmk list capacity zoneid=ZONE_UUID
cmk list serviceofferings issystem=true
cmk list storagepools zoneid=ZONE_UUID
```

Look for disabled/disconnected hosts, exhausted memory or CPU, a storage pool in maintenance, mismatched tags, or an architecture mismatch. CloudStack 4.20 and later support both `x86_64` and `aarch64` System VM templates on KVM; other hypervisors support `x86_64` only. For KVM, the preferred architecture and available hosts must align.
...
When SSH diagnostics are possible, run the following command on the KVM/XenServer host running the System VM. Current CloudStack documents port 3922 and the per-host key `/root/.ssh/id_rsa.cloud` for KVM/XenServer System VMs:
...
Use the **Run Diagnostics** UI action for network tests or **Get Diagnostics** to download a bundle when available. The bundle includes network state and System VM logs. Do not expose port 3922 beyond the infrastructure path.
...
Follow recreation in separate terminals: run the management-log command on the management server and the agent-log and libvirt commands on the assigned KVM host. Each `tail -F` runs continuously; rerun `virsh list --all` to refresh the domain list:
...
`Starting` means CloudStack records the VM as being started; it does not identify a particular failed handshake. Check the System VM’s `agentstate` separately from its VM state. Trace the async job to the host, then validate capacity, the exact System VM template, both storage tiers, bridges/VLANs, and the link-local management path. Recreate the VM only after that chain is healthy; otherwise CloudStack will faithfully reproduce the same failure.
