...
After a Patroni failover, the old PostgreSQL primary cannot simply start streaming from the promoted replica. Promotion creates a new timeline. If the old primary generated WAL after the common fork point, its data directory contains changes that do not exist on the new primary's timeline.
...
- The target had either data checksums enabled at `initdb` time **or** `wal_log_hints=on` while it ran.
- `full_page_writes=on` was enabled.
- The target WAL needed to reach the divergence point remains in `pg_wal` or is retrievable from its WAL archive.
- The source is reachable using a normal SQL connection with sufficient rewind permissions (or a superuser).
- The target is shut down cleanly. Current `pg_rewind` can complete crash recovery in single-user mode by default, but the safer Patroni path manages this state for you.
...
After bootstrap, `use_pg_rewind` is dynamic configuration. Verify or change it through `patronictl`:
...
Keeping both removal options `false` prevents an ordinary inability to rewind or a divergence check from becoming blanket permission to delete and rebuild the replica. It does not guarantee preservation after `pg_rewind` has already modified the target: a failed operation can leave `PGDATA` irrecoverable, and Patroni may remove a directory it knows is broken. Preserve any required forensic copy before the attempt. Once backups and the reinitialization runbook are proven, an organization may explicitly choose automatic rebuild behavior; that is a destructive policy decision, not a prerequisite for rewind.
...
- DCS quorum is stable.

Fence `pg1` at every write path until Patroni controls it: revoke the HAProxy backend, block application traffic, and use power/storage fencing if its state is uncertain. Fencing must not block the new primary's ability to obtain archived WAL or backups needed for recovery.
...
- `pg_rewind` start and progress/completion
- Recovery configuration for the new leader
...
If automatic rewind does not begin, stop Patroni on the target and gather evidence. A `pg_rewind` dry run changes nothing:
...
Use a protected password file or secret provider rather than putting a password in the command line. The target must remain fenced and offline. `--restore-target-wal` uses the target configuration's `restore_command`; verify that command and archive credentials before the dry run.
...
Expect `pg_is_in_recovery() = true`, one active WAL receiver, and receive/replay LSNs that advance.
...
| `could not find previous WAL record` or missing segment | Target no longer has WAL back to divergence | Restore target WAL from archive with `restore_command`, or reinitialize |
...
| Replica streams but is far behind | Source WAL replay/copy still catching up | Keep it out of reads/failover until within policy |
