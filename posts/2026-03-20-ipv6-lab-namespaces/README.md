...
Use [OneUptime](https://oneuptime.com) to monitor services running inside network namespaces. If running long-lived services in namespaces, configure monitors only for IPv6 addresses that are reachable from the system performing the checks.

## Conclusion

How to Build IPv6 Lab Environments with Network Namespaces uses standard Linux `ip` commands with the `netns` subcommand. Standard IPv6 configuration commands work the same way inside namespaces. Network namespaces are an excellent, zero-cost way to test IPv6 configurations before deploying to production.
