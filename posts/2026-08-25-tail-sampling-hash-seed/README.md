...
The Collector Contrib `probabilistic_sampler` processor can make a stateless decision from a trace ID. In its legacy hash mode, the underlying hash decision is a function of the trace ID, sampling percentage, and 32-bit `hash_seed`. Replicas with the same configuration therefore make the same hash selection for a trace ID without sharing state.
...
exporters:
  otlp_grpc:
    endpoint: downstream-collector.example.com:4317
...
For trace spans, absent a `sampling.priority` override or sampling-related error, the processor hashes the trace ID, so all spans with that ID receive the same decision even if they happen to pass through different replicas. This provides decision determinism; it does not assemble a trace or make arbitrary content-based tail decisions.

At the 10% setting above, input that already carries OpenTelemetry probability-sampling `rv` or `th` fields is one such error. Current legacy hash mode enters the `fail_closed` error path rather than applying the hash decision normally.
...
Do not extend the older same-seed/different-seed rule to a chain of current `hash_seed` processors. Current trace processing writes OpenTelemetry sampling randomness and threshold information into W3C `tracestate` on spans it keeps. Once configured to perform a hash decision, a later `hash_seed` processor treats an existing `rv` or `th` value as an error because legacy hash mode is not intended to consume probability-sampling state.

With the current default `fail_closed: true`, that second legacy hash processor's error path rejects the affected span. Setting `fail_closed: false` makes the error path pass the span. In either case, the requested second percentage is not applied; a span-level `sampling.priority` attribute is evaluated afterward and can override the error-path result.
...
Current `probabilistic_sampler` also supports `proportional` and `equalizing` modes based on OpenTelemetry probability-sampling information in W3C `tracestate`. Those modes use 56-bit randomness and propagate thresholds for adjusted counts. `hash_seed` selects the legacy FNV-based decision algorithm. It emits compatible `rv` and `th` state on normally kept spans, but it does not consume incoming probability-sampling state or make its decision from the standard 56-bit TraceID/R-value randomness.
...
The tail sampler has a separate alpha, off-by-default `processor.tailsamplingprocessor.usetracestate` gate for probability `tracestate` handling, including its probabilistic policy. Review that feature independently; it does not rename `hash_salt` to `hash_seed`.
