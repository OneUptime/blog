...
Flux requires SSH secrets to contain `identity` and `known_hosts`. Secrets created by `flux create secret git` also include `identity.pub`, which is useful when adding the deploy key to your Git provider.
...
data:
  ...
  # The public key (base64 encoded, generated by the Flux CLI)
...
kubectl patch secret flux-system -n flux-system \
  --type merge \
  -p "{\"data\":{\"known_hosts\":\"$(printf '%s' "$KNOWN_HOSTS" | base64 | tr -d '\n')\"}}"
...
GitHub can rotate its SSH host keys. If your known_hosts is outdated, connections will fail.
...
Verify that the cluster can reach GitHub over SSH and validate the host key.
...
kubectl run -n flux-system ssh-test --rm -it --restart=Never \
  --image=alpine/git -- sh -c "
    mkdir -p ~/.ssh && \
    echo 'Testing SSH reachability to GitHub...' && \
