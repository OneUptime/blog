...
- The inventory warning pitfall said using `localhost,` on the command line may produce a no-inventory warning. Official docs describe comma-separated host lists as valid inventory input, so this was changed to refer to relying on implicit localhost without specifying inventory.
