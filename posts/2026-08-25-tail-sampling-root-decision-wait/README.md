...
In `span-ingest` mode, positive or drop outcomes may already finalize during ingestion. The two waits govern cleanup or earlier cleanup of traces that remain pending.
...
Once the accelerated decision is made, later spans do not reopen it. Configure sampled and non-sampled decision caches so late spans inherit the original outcome instead of forming a second trace evaluation.
...
A cache preserves consistency, not correctness of the earlier policy evaluation. An error arriving after an accelerated drop will follow the cached drop. Increase the root grace if decision-relevant spans fall into that interval.

## Check Root Identification and Trace Shape

The implementation recognizes a root by an empty `ParentSpanID`. A span that merely represents an important service entry is not necessarily the distributed trace root. Broken propagation can create multiple apparent roots, and partial traces can make an internal span look root-like after context is lost.
...
Successful acceleration reduces average residence time, but `num_traces` must still cover traffic whose root is missing or delayed and therefore uses the full `decision_wait`. Size from burst behavior, not only the new average. Keep the fixed wait as a real fallback rather than shrinking capacity until the fallback itself causes early eviction.
