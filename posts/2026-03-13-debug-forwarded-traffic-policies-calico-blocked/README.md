...
```bash
# Apply host protection policy first
...
# View Calico iptables chains (iptables dataplane)
sudo iptables-save | grep -i cali

# Check Felix status on node
kubectl exec -n <calico-namespace> calico-node-xxx -- /bin/calico-node -felix-live
```
