...
| On-Call | Grafana OnCall | PostgreSQL |
| Incidents | Grafana Incident (Cloud only) | - |
...
**Operational overhead.** Each component is a separate deployment with its own scaling characteristics. Prometheus needs persistent storage and careful retention tuning. Loki's ingester and querier need separate scaling. Tempo needs object storage configuration. Grafana OnCall is yet another service. Multiply this by staging and production environments, and you have a lot of infrastructure to maintain.
...
**Status pages and incident management.** Grafana Cloud offers Grafana Incident, but it's cloud-only. Self-hosted Grafana has no built-in incident management or public status pages. You'll need a separate tool - Cachet, Statuspage.io, or something custom.
...
|------|-----------------|
| Metrics (20K active series) | ~$120 |
| Logs (100 GB/month) | ~$50 |
| Traces (50 GB/month) | ~$25 |
| Grafana OnCall (Pro) | ~$19/user × 10 on-call = $190 |
...
| Item | Monthly estimate |
|------|-----------------|
| Infrastructure (Prometheus, Loki, Tempo, Grafana, OnCall) | 3-5 nodes, ~$300-600 |
