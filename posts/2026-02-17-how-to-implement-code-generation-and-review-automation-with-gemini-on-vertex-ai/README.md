...
AI-assisted coding has gone from novelty to practical tool. Gemini on Vertex AI is particularly interesting for teams already on GCP because it integrates naturally with Cloud Build, Secret Manager, Cloud Run, and the rest of the GCP ecosystem. Rather than using it as a chatbot, you can embed it into your development workflow as an automated code reviewer, a boilerplate generator, and a documentation writer.
...
Only output the JSON array, nothing else. If no issues found, output an empty array."""
    ...
    summary = generate_pr_summary(client, model_id, diff, pr_description, all_comments)
...
def generate_pr_summary(client, model_id, diff, description, comments):
