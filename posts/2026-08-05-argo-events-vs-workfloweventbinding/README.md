...
| Durable intermediary | Argo Server has an in-memory asynchronous processing queue | EventBus supplies broker-backed transport |
...
The Argo Workflows events endpoint returns within ten seconds because it queues work asynchronously. A successful HTTP response does not prove that a Workflow was created. The official documentation also warns that events may be processed out of order, that the endpoint returns `503` when its queue is full, and that at least two Argo Server replicas are required if you do not want a single server loss to lose accepted events.
...
For `WorkflowEventBinding`, account for the Argo Server queue, `503` responses, producer retries, asynchronous submission failures, and server high availability. Retain the producer's event ID so a replay can be identified.
...
5. Fill or disconnect the intermediate queue and verify producer-visible status.
