...
The Cilium agent is the per-node daemon responsible for programming datapath state, applying policy, and reporting node-level health. When networking or policy behavior is degraded, a systematic review of agent status, logs, metrics, and BPF maps helps narrow the failure quickly.

In this guide, we cover cilium-agent troubleshooting in a Kubernetes environment. Cilium leverages eBPF technology to provide high-performance networking, security, and observability for cloud-native workloads. The eBPF programs are loaded directly into the Linux kernel, enabling efficient packet processing without the overhead of traditional iptables-based networking stacks.
...
- A running Kubernetes cluster with a Cilium-supported Kubernetes version
...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg identity list | wc -l
...
helm upgrade cilium cilium/cilium \
  ...
  --set 'labels=!pod-template-hash !controller-revision-hash'
```
...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list --match-pattern drop

# Verify inter-node health from a Cilium agent pod
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-health status --verbose

# Check BPF service maps
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf lb list | head -20
```
...
kubectl logs -n kube-system -l k8s-app=cilium --tail=200 -c cilium-agent | grep "regeneration completed" | tail -5

# Check Cilium policy resources
kubectl get ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies -A

# Verify endpoint regeneration rate
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list --match-pattern 'endpoint.*regeneration'
```
...
flowchart TD
    ...
    F -->|Connectivity| H[Check Health and BPF]
...
# Verify endpoint count on the selected Cilium agent
echo "Cilium endpoints on this agent: $(kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list -o json 2>/dev/null | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' 2>/dev/null || echo 'N/A')"
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the node kernel meets the minimum for your Cilium release. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get networkpolicies,ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies -A`. Verify inter-node health with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-health status --verbose`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list --match-pattern process_resident_memory`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list`. Confirm the policy resources are present with `kubectl get networkpolicies,ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies -A`. Check that the endpoint has the correct identity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint get <id>` and inspect the local BPF policy maps with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf policy get --all`.
...
This guide covered cilium-agent troubleshooting with practical steps you can apply to your Kubernetes cluster. Regular monitoring, systematic validation, and proactive management are essential for maintaining a healthy Cilium deployment at any scale.
