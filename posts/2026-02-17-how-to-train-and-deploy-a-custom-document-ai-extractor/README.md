...
- At least 10 labeled documents in the test split
...
def check_training_readiness(project_id, location, processor_id):
    ...
    for doc in page_result:
        if doc.dataset_type == documentai_v1beta3.DatasetSplitType.DATASET_SPLIT_TRAIN:
            train_count += 1
        elif doc.dataset_type == documentai_v1beta3.DatasetSplitType.DATASET_SPLIT_TEST:
            test_count += 1

        if doc.labeling_state == documentai_v1beta3.DocumentLabelingState.DOCUMENT_LABELED:
            labeled_count += 1
    ...
    if train_count >= 10 and test_count >= 10:
...
def check_training_status(location, operation_name):
...
def get_evaluation_results(project_id, location, processor_id, version_id):
    ...
    def best_metrics(multi_confidence_metrics):
        """Return metrics at the confidence threshold with the highest F1 score."""
        if not multi_confidence_metrics.confidence_level_metrics:
            return None

        return max(
            multi_confidence_metrics.confidence_level_metrics,
    ...
    for evaluation in evaluations:
        ...
        # Overall metrics
        all_entities = best_metrics(evaluation.all_entities_metrics)
        if all_entities:
            metrics = all_entities.metrics
            print(f"\nOverall confidence threshold: {all_entities.confidence_level:.3f}")
        ...
        # Per-entity metrics
        print("\nPer-field metrics:")
        for entity_name, multi_confidence_metrics in evaluation.entity_metrics.items():
            entity_metrics = best_metrics(multi_confidence_metrics)
            if not entity_metrics:
                continue

            metrics = entity_metrics.metrics
            print(f"  {entity_name}:")
            print(f"    Confidence threshold: {entity_metrics.confidence_level:.3f}")
