...
Description: Capture SPA route transitions with portable router timings today while progressively adopting Chrome's standardized Soft Navigations performance entries.
...
2. Progressively consume the standardized Soft Navigations API where it is available, without assuming every browser, RUM SDK, or public field dataset supports it yet.
...
| --- | --- |
| Start | User input timestamp for a user-initiated route; immediately before programmatic navigation otherwise |
...
Starting when a router callback finally runs omits any input delay before it. When a click triggers navigation, retain the event's high-resolution timestamp:
...
Event Timing and INP remain the standardized measures of interaction-to-next-paint responsiveness. A route duration can extend beyond the first paint and represent application readiness. Correlate the two rather than renaming route duration as INP.
...
A route transition can be a useful browser span because it connects an interaction, client requests, and server traces. Give it a stable name such as `spa.route /orders/:orderId`, then attach low-cardinality attributes and events.

```js
const span = tracer.startSpan(`spa.route ${toTemplate}`, {
  attributes: {
    "app.route.from": fromTemplate,
    "app.route.to": toTemplate,
...
span.setAttribute("app.route.outcome", "success");
span.end(endTimestamp);
```

The browser OpenTelemetry semantic conventions are still marked Development in the official specification. Avoid claiming custom route attributes are stable OpenTelemetry conventions. Namespace and version your own schema, and review the conventions before migrating attribute names.

Browser trace propagation also needs deliberate CORS and security configuration. Only propagate to trusted origins, configure `traceparent` in allowed request headers, and remember that client-created spans may be sampled or blocked.
...
As of August 2026, Chrome documentation says the feature is launching unflagged from Chrome 151. Older Chrome versions and other browsers may not report it. Feature-detect support:
...
Browser-detected soft navigations and application transitions serve different purposes. The standardized API can make Core Web Vitals slicing comparable, while application hooks capture programmatic transitions, errors, readiness semantics, and unsupported browsers. During rollout, correlate both by time and route, detect duplicates, and do not add both samples to the same denominator.
...
SPA routes are missed because a new URL does not necessarily mean a new document. Instrument the router's intent, data, commit, visible-readiness, and outcome boundaries with a unique transition ID and normalized route. Then feature-detect Chrome's standardized soft-navigation entries and use them as progressive enhancement while coverage matures. This produces portable operational timings now and a clean path toward comparable per-navigation Web Vitals without double-counting either view.
