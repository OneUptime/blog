...
`ibstat` is valuable, but it primarily shows the active width and rate. It cannot, by itself, tell you whether 1X is an intentional breakout, the remote port is restricted, the cable supports a lower generation, or one end failed to train the wider common mode.
...
`ibstat` reports the local LID, SM LID, state, active link width, rate, and physical state. Linux also exposes the current rate through the stable InfiniBand sysfs ABI. `iblinkinfo` gives each discovered link and its remote endpoint, which prevents comparing the HCA with the wrong switch port.
...
Inspect both endpoints. NVIDIA switch commands report supported and active speeds and widths. NVIDIA's `mlxlink` provides a physical-layer view for supported devices and media:
...
Either endpoint may enable only a subset of its supported speeds or widths. On NVIDIA switches, the `speed` and `width` interface settings control those allowed sets; defaults and valid values depend on port/module type. The `ibportstate` diagnostic can query port state and validate link speed/width relative to the peer, and it can also change settings.

Use its query operation first:

~~~console
$ ibportstate -C mlx5_0 -P 1 <switch-lid> <switch-port>
~~~

The LID and port must be discovered from the real fabric. Do not paste example addresses. Configuration operations such as `speed` or `width` can take down or destabilize a link and should be made through the fabric's supported change process.
...
Do not clear counters until you have saved both ends and their collection time. Absolute totals since boot are less useful than synchronized deltas under known traffic.
...
- [rdma-core: `ibstat(8)` active width and port reporting](https://github.com/linux-rdma/rdma-core/blob/master/infiniband-diags/man/ibstat.8.in.rst)
...
- [NVIDIA NVOS: InfiniBand switch speed and width commands](https://docs.nvidia.com/networking/display/nvidianvosusermanualforinfinibandswitchesv25027002/infiniband-interface-commands)
- [NVIDIA MFT: `mlxlink` physical-layer diagnostics](https://networking-docs.nvidia.com/mftswum/426135lts/mlxlink-utility)
- [NVIDIA: `ibdiagnet` fabric diagnostic manual](https://docs.nvidia.com/networking/display/ibdiagnet-infiniband-fabric-diagnostic-tool-user-manual-v2-21.21.pdf)
