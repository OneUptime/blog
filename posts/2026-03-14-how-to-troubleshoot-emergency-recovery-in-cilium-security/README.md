...
kubectl -n kube-system exec ds/cilium -c cilium-agent -- \
  cilium-dbg bpf policy get --all -n
...
# Check health details for any remaining problematic endpoint
kubectl -n kube-system exec ds/cilium -c cilium-agent -- \
...
- **Policy changes not propagating**: Inspect the affected endpoint with `cilium-dbg endpoint get <ENDPOINT_ID>` and `cilium-dbg endpoint log <ENDPOINT_ID>`. If it remains stuck, recreate the affected pod to force a fresh endpoint allocation.
