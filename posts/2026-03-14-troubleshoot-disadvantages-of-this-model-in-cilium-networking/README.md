...
```bash
# Check BPF endpoint map status
kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf endpoint list | head -20
...
# Verify routes on a Cilium node
...
kubectl exec diag-pod -- curl -k -s --max-time 5 https://kubernetes.default.svc:443 2>&1
...
kubectl exec -n kube-system ds/cilium -- hubble observe --pod NAMESPACE/POD_NAME --last 20
...
- **BPF maps are full**: Check connection tracking entries with `cilium-dbg bpf ct list | wc -l`. If approaching limits, increase `bpf-ct-global-any-max` and `bpf-ct-global-tcp-max` in Helm values.
- **Performance issues after configuration change**: Check BPF datapath metrics with `cilium-dbg bpf metrics list` and inspect loaded programs on the node with `bpftool prog show`.
