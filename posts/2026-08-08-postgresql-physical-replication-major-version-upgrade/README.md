...
Before the coordinated stop, confirm every standby is streaming and replaying. On the old primary:
...
After starting the new primary, run the post-upgrade scripts generated by `pg_upgrade`, update extensions as directed, refresh optimizer statistics where required, and rebuild every standby. Verify:
...
All expected tables must reach `r`. While copying, monitor publisher slots and disk because the upgrade slot can retain substantial WAL.
...
PostgreSQL 18 has specific `pg_upgrade` support for preserving logical slots and subscription dependencies when every old topology member is PostgreSQL 17 or later and detailed prerequisites are met. That is different from creating a new cross-version migration subscription. On older source releases, logical slots or subscription dependency state may not migrate through `pg_upgrade`; follow the version-specific upgrade chapter.
...
- [PostgreSQL warm standby planning](https://www.postgresql.org/docs/current/warm-standby.html#WARM-STANDBY-PLANNING)
