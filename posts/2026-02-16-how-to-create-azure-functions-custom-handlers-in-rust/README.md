...
The normal Azure Functions execution model works like this: the Functions host receives a trigger event (HTTP request, queue message, timer tick), processes bindings, and calls your function code through a language-specific worker. With custom handlers, the host instead forwards the event as an HTTP request to a local web server that you control. Non-HTTP triggers use a POST request with the custom handler payload, while HTTP forwarding can preserve the original request method and route.
...
sequenceDiagram
    ...
    FunctionsHost->>RustServer: GET/POST /api/my-function (with trigger data)
...
For local development, also create `local.settings.json` in the project root so the Functions host knows this app uses a custom handler.
...
Now create the function configuration file at `hello/function.json`.
...
async fn hello_get(query: web::Query<HelloQuery>) -> HttpResponse {
...
async fn process_queue_message(
    ...
    // The key must match the trigger binding name in function.json.
    let queue_message = body.data.get("myQueueItem")
        .and_then(|v| v.as_str())
        .unwrap_or("empty");
...
cp host.json deploy/
cp hello/function.json deploy/hello/
...
Rust custom handlers can start quickly because the handler is a compiled binary and does not need to initialize an interpreter. Actual cold start time depends on the hosting plan, package size, dependencies, and how quickly the custom handler process starts listening on `FUNCTIONS_CUSTOMHANDLER_PORT`.

Memory usage can also be lower for small Rust handlers than for equivalent functions in interpreted runtimes. On the Consumption plan, lower memory use can reduce GB-second charges for workloads where memory is a meaningful part of the bill.
