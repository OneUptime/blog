...
`CrashLoopBackOff` is a delay between repeated restart attempts, not the root cause. A sidecar may be exiting because its command fails, an expected mounted file or Secret key is absent, a liveness or startup probe kills it, it exceeds memory, or it cannot reach a dependency.
...
| --- | --- |
| `OOMKilled` (often exit 137) | Memory use, limit, node pressure, and unbounded buffers. Treat exit 137 alone only as a clue; confirm the termination reason and signal. |
| Shell exit 126 or 127 | For 126, executable permissions or format and architecture; for 127, image contents, `PATH`, command, and args. |
...
| Liveness or startup probe failures | Probe path, port, command, startup time, timeout, and dependency assumptions. |
...
NEW_POD=ledger-6c86bbf5f7-r7x9q
