...
- A running Kubernetes cluster supported by your installed Cilium version
...
kubectl -n kube-system exec ds/cilium -- cilium-dbg metrics list | head -10

# Check resource consumption is within expected bounds
kubectl top pods -n kube-system -l k8s-app=cilium

# Verify no packet drops
kubectl -n kube-system exec ds/cilium -- cilium-dbg metrics list | grep -E "drop|error"
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and that the node kernel meets Cilium's system requirements for your Cilium release. For current Cilium releases, this means Linux kernel 5.10 or later, or an equivalent distribution kernel such as 4.18 on RHEL 8.10. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get ciliumnetworkpolicies --all-namespaces`. Verify inter-node tunnel connectivity from a Cilium pod with `kubectl -n kube-system exec ds/cilium -- cilium-dbg bpf tunnel list`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl get ciliumendpoints --all-namespaces`. Confirm the policy is applied with `kubectl get ciliumnetworkpolicy -n validation-test validation-policy -o yaml`. Check that the endpoint has the correct identity with `kubectl get ciliumendpoint -n validation-test <pod-name> -o yaml`.
