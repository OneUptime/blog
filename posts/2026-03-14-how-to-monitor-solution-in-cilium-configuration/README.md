...
- Access to `cilium-dbg` inside Cilium agent pods for local agent diagnostics
...
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "cilium_identity|cilium_identity_gc"

# Endpoint health metrics
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "cilium_endpoint|cilium_endpoint_regeneration"

# Policy metrics
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "cilium_policy|cilium_policy_implementation"

# Datapath performance metrics
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "cilium_datapath|cilium_forward|cilium_drop"

# Agent resource metrics
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "process_cpu|process_resident_memory"
```
...
# Alert rules for Cilium monitoring with Prometheus Operator
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          labels:
            severity: warning
          annotations:
            summary: "Cilium agent has failing controllers"
        - alert: CiliumEndpointRegenerationSlow
          expr: histogram_quantile(0.95, sum by (le) (rate(cilium_endpoint_regeneration_time_stats_seconds_bucket[5m]))) > 5
          for: 10m
          labels:
            severity: warning
          annotations:
        ...
        - alert: CiliumHighDropRate
          expr: sum(rate(cilium_drop_count_total[5m])) by (pod, reason) > 100
          for: 5m
          labels:
            severity: critical
          annotations:
...
echo ""
echo "Agent Status:"
kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg status --brief
echo ""
echo "Identity Count: $(kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg identity list 2>/dev/null | wc -l)"
echo "Endpoint Count: $(kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg endpoint list 2>/dev/null | wc -l)"
echo ""
echo "Resource Usage:"
...
echo "Cilium endpoints: $(kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg endpoint list -o json 2>/dev/null | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' 2>/dev/null || echo 'N/A')"
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and that your nodes meet the kernel requirements for your Cilium version. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg policy get`. In encapsulation mode, verify inter-node tunnel connectivity with `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg bpf tunnel list`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg metrics list | grep process_resident_memory`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg endpoint list`. Confirm the policy is applied with `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg policy get`. Check that the endpoint has the correct identity with `kubectl exec -n kube-system -l k8s-app=cilium -c cilium-agent -- cilium-dbg endpoint get <id>`.
