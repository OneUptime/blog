...
The `serial` keyword controls how many hosts Ansible updates at a time. Without it, Ansible processes the whole host group in one batch, which can cause a full outage during service restarts.
...
- hosts: webservers
  ...
  pre_tasks:
    ...
    # The HAProxy task above drains active connections; this pause gives the load balancer time to settle
    - name: Pause after disabling host
      wait_for:
        timeout: 30
...
```yaml
# Pre-task: Remove the current batch from nginx upstream
- name: Remove current batch from nginx upstream
  template:
  ...
  loop: "{{ groups['loadbalancers'] }}"
  vars:
    excluded_hosts: "{{ ansible_play_batch }}"
  notify: reload nginx on lb

# Post-task: Add the current batch back to nginx upstream
- name: Add current batch back to nginx upstream
  template:
    src: nginx-upstream.conf.j2
    dest: /etc/nginx/conf.d/upstream.conf
  delegate_to: "{{ item }}"
