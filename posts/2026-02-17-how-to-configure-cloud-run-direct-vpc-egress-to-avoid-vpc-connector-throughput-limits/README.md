...
If you have been using Serverless VPC Access connectors to give your Cloud Run services access to VPC resources, you have probably hit the throughput ceiling at some point. Connector throughput depends on the machine type: e2-micro connectors are estimated at 200 Mbps to 1 Gbps, while e2-standard-4 connectors can go higher. Scaling them still requires managing connector instances and dealing with their own set of limitations.

Direct VPC Egress is the newer, better approach. It lets Cloud Run instances send traffic directly into your VPC without going through a connector, removing the connector instance bottleneck for many high-bandwidth workloads. This guide covers how to set it up and when it makes sense over the older connector approach.
...
- Uses dedicated f1-micro, e2-micro, or e2-standard-4 instances as a bridge between Cloud Run and your VPC
- Throughput depends on the connector machine type, from 100 Mbps to 500 Mbps for f1-micro up to 3.2 Gbps to 16 Gbps for e2-standard-4
...
- The subnet must have enough available IP addresses. For services, plan for at least 2 times your instance count plus headroom for revision updates.
- Cloud Run API enabled
- Compute Engine API enabled (for VPC networking)
- Cloud Run's service agent must have permission to use the VPC subnet. The default Cloud Run Service Agent role usually includes the required permissions.
...
Direct VPC Egress assigns ephemeral IP addresses from your VPC subnet to Cloud Run. For faster scale-up, Cloud Run reserves addresses in blocks of 16, and the subnet must be /26 or larger. For services, plan for at least 2 times your expected instance count plus extra headroom for revision updates.
...
A /24 subnet has 252 usable addresses in Google Cloud primary subnet ranges because Google reserves the first two and last two addresses. That can be enough for a service that scales to 100 instances, but revision updates can temporarily require more addresses. For larger services, use a bigger range.

Check the configured IP ranges in an existing subnet:

```bash
# List configured IP ranges in the subnet
...
- `private-ranges-only`: Only traffic destined for internal addresses goes through the VPC. Public internet traffic goes directly from Cloud Run. This is the more common choice.
  ...
  template:
    metadata:
      annotations:
        ...
        run.googleapis.com/network-interfaces: '[{"network":"default","subnetwork":"cloud-run-subnet"}]'
...
With your Cloud Run service configured with `--vpc-egress=all-traffic`, outbound internet traffic from the service will use the static IP address from Cloud NAT.

## Troubleshooting

Common issues and how to fix them:

**"Subnet does not have enough IPs"** - Your subnet CIDR range is too small for Cloud Run's IP address allocation. Either use a larger subnet or reduce your max instances.
...
```bash
# View IP addresses allocated by Cloud Run
