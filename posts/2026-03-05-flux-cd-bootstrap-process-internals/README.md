...
The first thing bootstrap does is connect to the Git provider. For GitHub, it requires a GitHub personal access token, which can be provided with the `GITHUB_TOKEN` environment variable or entered when the CLI prompts for it. It checks whether the specified repository exists. If not, it creates it. If the repository already exists, bootstrap reuses it.
...
The `kustomization.yaml` file generated by bootstrap can be extended with patches. For example, to increase the memory limit on the source controller.
...
The newer CLI generates updated manifests (with newer controller images), commits them to Git, and the self-management loop rolls out the update. You can also update `gotk-components.yaml` in Git with `flux install --export`; avoid running `flux install` directly against the cluster for an upgrade, because that would bypass the bootstrap repository.
