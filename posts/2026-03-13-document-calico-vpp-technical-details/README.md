...
{
  printf '# Auto-exported from cluster worker-vpp-1 on %s\n' "$(date)"
  printf '# Generated by: export-vpp-startup.sh\n\n'
  kubectl get configmap calico-vpp-config -n calico-vpp-dataplane \
    -o jsonpath='{.data.CALICOVPP_CONFIG_TEMPLATE}'
  printf '\n'
} > "docs/vpp/startup-config-$(date +%Y%m%d).conf"
```
