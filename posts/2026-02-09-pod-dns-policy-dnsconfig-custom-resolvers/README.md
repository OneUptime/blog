...
dnsConfig lets you customize DNS settings regardless of dnsPolicy. You can add nameservers, search domains, and options. Kubernetes merges these values with the base DNS configuration generated by the selected dnsPolicy, and dnsConfig can specify at most three nameserver IP addresses.
...
This pod uses Google DNS servers with a custom search domain. It will not resolve Kubernetes Service names unless the configured DNS servers can answer or forward the cluster domain.
...
This pod uses the DNS configuration generated by ClusterFirst, with the custom nameserver appended to the resolver list and the additional search domain merged into the search list.
...
- Includes Kubernetes cluster search domains when corporate DNS is configured to forward those zones to CoreDNS
...
Some applications need cluster search domains plus corporate search domains. Per-pod dnsConfig can adjust the pod resolver, but true split DNS by domain should be configured in CoreDNS forwarding rules rather than by listing several nameservers in a pod.
...
Cluster names still go to CoreDNS through ClusterFirst, while the corporate search domain is added to the pod resolver's search list.
...
Pods with multiple network interfaces might need additional search domains for those networks. dnsConfig applies to the pod resolver as a whole; it does not bind DNS servers to specific pod interfaces.
...
---
# API pods optimize external DNS lookups
apiVersion: apps/v1
kind: Deployment
metadata:
  name: api
...
```yaml
# Temporarily bypass cluster DNS to isolate upstream resolver issues
dnsPolicy: None
dnsConfig:
  nameservers:
  - 8.8.8.8
