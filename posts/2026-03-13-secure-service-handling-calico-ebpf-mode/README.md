...
Calico eBPF mode handles Kubernetes service types that have a data-plane presence - ClusterIP, NodePort, and LoadBalancer - using BPF programs and maps that are loaded directly into the kernel. ExternalName services are pure DNS CNAME records with no cluster IP or endpoints, so they are not programmed into BPF maps. This provides lower latency service routing compared to iptables-based approaches and scales better with the number of services and endpoints.
