...
AppArmor provides mandatory access control (MAC) that restricts program capabilities regardless of user privileges. Kubernetes supports AppArmor through security context fields that can apply different profiles to different containers within the same pod. This granular control lets you tailor security policies to each container's specific needs.
...
Kubernetes applies AppArmor profiles through pod or container security contexts. Each container can have its own profile, enabling fine-grained security policies within multi-container pods.
...
spec:
  ...
  - name: sidecar
    ...
    securityContext:
      appArmorProfile:
        type: Localhost
        localhostProfile: custom-sidecar
```

The per-container format is `containers[*].securityContext.appArmorProfile` with `type` set to `RuntimeDefault`, `Localhost`, or `Unconfined`.
...
Use RuntimeDefault for most containers, avoid Unconfined except for debugging, and use Localhost profiles for specialized requirements.
...
profile custom-web-profile flags=(attach_disconnected) {
  ...
  # Explicitly deny high-risk access
  deny /etc/shadow r,
  deny /etc/passwd w,
...
kubectl debug node/node-name -it --image=ubuntu -- chroot /host sh -c "cat /sys/module/apparmor/parameters/enabled"
...
spec:
  ...
  - name: app
    ...
    securityContext:
      appArmorProfile:
        type: Localhost
        localhostProfile: strict-profile
...
spec:
  ...
  - name: app
    ...
    securityContext:
      appArmorProfile:
        type: Localhost
        # Note: complain mode is set when the profile is loaded on the node
        localhostProfile: dev-profile-complain

# Step 2: Analyze denials and update profile
# Step 3: Switch to enforce mode
# localhostProfile: dev-profile-enforce
```
...
Init containers can use different restrictions for setup while main containers remain restricted.
...
AppArmor profiles enable per-container mandatory access control policies in Kubernetes. Apply different profiles to different containers based on their specific requirements, using RuntimeDefault for general workloads and Localhost profiles for specialized needs. Create custom profiles that permit necessary operations while blocking dangerous ones, test them in complain mode, then enforce them in production. Distribute custom profiles to nodes using DaemonSets or configuration management. Combine AppArmor with other security context settings like running as non-root, dropping capabilities, and read-only filesystems for defense in depth. Monitor for AppArmor denials that indicate either profile misconfiguration or security incidents. Per-container AppArmor profiles provide kernel-level mandatory access control that significantly strengthens container security boundaries.
