...
- History that helps identify flaky tests

Report Groups support two types of reports:
1. **Test reports** - JUnit XML, NUnit XML, NUnit3 XML, Cucumber JSON, TestNG XML, Visual Studio TRX, Visual Studio TRX XML
2. **Code coverage reports** - JaCoCo XML, SimpleCov JSON, Clover XML, Cobertura XML, LCOV INFO
...
```bash
# Install the coverage plugin; pytest has built-in JUnit XML output
  ...
  build:
    commands:
      ...
      - |
        ...
        npx jest --testPathPatterns='tests/unit' --ci \
          --reporters=default \
      ...
      - |
        ...
        JEST_JUNIT_OUTPUT_NAME=junit.xml \
        npx jest --testPathPatterns='tests/integration' --ci \
          --reporters=default \
          --reporters=jest-junit
...
- Trend data for unexpired reports
...
## Step 6: Set Up Coverage Thresholds

CodeBuild report groups collect and display results, but threshold enforcement should happen in your test command or buildspec. For example, fail the build if coverage drops below a threshold:
...
For Jest, configure `coverageThreshold` so the test command exits with a non-zero status when coverage is too low:
