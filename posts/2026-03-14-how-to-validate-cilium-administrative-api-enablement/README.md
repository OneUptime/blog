...
- A running Kubernetes cluster on a Kubernetes version supported by your Cilium release, with Cilium installed (v1.14+)
- `kubectl` configured for cluster access
- `cilium` CLI installed (matching your Cilium version), and access to `cilium-dbg` inside Cilium agent pods
...
# Step 2: Validate administrative API access settings on a Cilium agent
CILIUM_POD=$(kubectl -n kube-system get pods -l k8s-app=cilium -o jsonpath='{.items[0].metadata.name}')
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg config get enable-cilium-api-server-access
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg config get enable-cilium-health-api-server-access
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg status --verbose

# Step 3: Validate all agents are running and healthy
...
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg identity list | wc -l

# Verify metrics are being collected
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg metrics list | head -10

# Check resource consumption is within expected bounds
kubectl top pods -n kube-system -l k8s-app=cilium

# Verify no packet drops
kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg metrics list | grep -E "drop|error"
```
...
```bash
# Select a Cilium agent pod for node-local API checks
CILIUM_POD=$(kubectl -n kube-system get pods -l k8s-app=cilium -o jsonpath='{.items[0].metadata.name}')
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the node meets the Cilium system requirements for your Cilium release. For current stable Cilium releases, that means Linux kernel 5.10 or later, or an equivalent vendor kernel such as 4.18 on RHEL 8.10. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-dbg policy get`. Verify inter-node connectivity with `kubectl exec -n kube-system "$CILIUM_POD" -c cilium-agent -- cilium-health status`.
