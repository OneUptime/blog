...
```bash
# Check Istio's self-signed CA secret, if it is in use

kubectl get secret istio-ca-secret -n istio-system -o jsonpath='{.data.ca-cert\.pem}' | base64 -d | openssl x509 -text -noout
```

If this secret does not exist, check whether Istio is using the plug-in CA secret instead:

```bash
kubectl get secret cacerts -n istio-system -o jsonpath='{.data.ca-cert\.pem}' | base64 -d | openssl x509 -text -noout
```

Look at the Issuer field. If the signing CA is generated by Istio, you are using the default self-signed CA.
...
kubectl get secret cacerts -n istio-system -o jsonpath='{.data.cert-chain\.pem}' | base64 -d | openssl crl2pkcs7 -nocrl -certfile /dev/stdin | openssl pkcs7 -print_certs -noout
```
    ...
    - name: istio-certs
      rules:
        - alert: IstioCACertExpiringSoon
          expr: |
            istio_citadel_server_root_cert_expiry_timestamp - time() < 86400 * 30
          for: 1h
          labels:
            severity: warning
          annotations:
...
```bash
kubectl exec deploy/sleep -c sleep -- openssl s_client -showcerts -connect my-service:8080 2>/dev/null | openssl x509 -noout -text | grep -A2 "Validity"
```
...
If you are using cert-manager as a custom CA through the Kubernetes CSR API, verify the integration:
...
A cert-manager ClusterIssuer for Istio:
...
Verify cert-manager is ready to sign certificate requests:

```bash
kubectl get clusterissuer istio-ca -o wide
```
...
```bash
# List all Istio Gateway TLS secrets. Use the namespace where the gateway workload runs.
gateway_workload_namespace=istio-system
kubectl get gateway.networking.istio.io -A -o jsonpath='{range .items[*]}{.metadata.namespace}{" "}{.metadata.name}{" "}{range .spec.servers[*]}{.tls.credentialName}{"\n"}{end}{end}' | sort -u |
while read -r ns gateway secret; do
  [ -z "$secret" ] && continue
  echo "Gateway: $ns/$gateway, secret: $secret"
  kubectl get secret "$secret" -n "$gateway_workload_namespace" -o jsonpath='{.data.tls\.crt}' | base64 -d | openssl x509 -noout -subject -dates
  echo "---"
done
```
...
```bash
# Check workload certificate expiration
kubectl exec deploy/my-service -c istio-proxy -- curl -s localhost:15000/certs | python3 -c "
import sys, json
data = json.load(sys.stdin)
for cert in data.get('certificates', []):
    for c in cert.get('cert_chain', []):
        print('Serial:', c.get('serial_number', ''))
        print('Expires:', c.get('expiration_time', ''))
"
```

Key metrics to monitor:

- `istio_citadel_server_root_cert_expiry_timestamp` - CA cert expiration in Prometheus
- `istio_agent_cert_expiry_seconds` - workload cert expiration in Prometheus (exported by the proxy from the agent metric `cert_expiry_seconds`)
