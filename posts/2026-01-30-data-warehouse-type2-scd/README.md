...
**Valid From/To Dates**: Define the time period when each version was current. In these examples, "valid_to" is exclusive, and the date 9999-12-31 indicates the current active record.
...
flowchart TD
    ...
    E -->|Yes| G[Expire Current Record<br/>valid_to = today<br/>is_current = FALSE]
...
The staging table approach separates change detection, expiration, insertion, and Type 1 updates into clear steps.

```sql
-- Type 2 SCD procedure using a PostgreSQL-style staging table approach
-- This procedure handles inserts, updates, and expiration in separate statements
...
WHERE
    ...
    (
        s.customer_name IS DISTINCT FROM d.customer_name OR
        s.email IS DISTINCT FROM d.email OR
        s.phone IS DISTINCT FROM d.phone OR
        s.address_line1 IS DISTINCT FROM d.address_line1 OR
        s.city IS DISTINCT FROM d.city OR
        s.state IS DISTINCT FROM d.state OR
        s.postal_code IS DISTINCT FROM d.postal_code OR
        s.country IS DISTINCT FROM d.country OR
        s.customer_segment IS DISTINCT FROM d.customer_segment
    );

-- Step 3: Expire old records that have changes
-- Set valid_to to the new version's start date and is_current to FALSE
...
SELECT
    ...
    NULL,                         -- replace with your ETL batch identifier
...
AND d.data_quality_score IS DISTINCT FROM s.data_quality_score;
...
### Using MERGE Statement (Snowflake)

```sql
-- Type 2 SCD using MERGE statement
-- This approach uses MERGE for expiration and a separate insert for new versions

CREATE OR REPLACE TEMPORARY TABLE changed_customer_records AS
SELECT
    s.*,
    CASE WHEN d.customer_sk IS NULL THEN 'INSERT' ELSE 'UPDATE' END AS action_type
FROM stg_customer s
...
WHERE d.customer_sk IS NULL
   OR s.customer_name IS DISTINCT FROM d.customer_name
...
MERGE INTO dim_customer AS target
USING changed_customer_records AS source
...
-- abc123     | CUST-1001   | John Smith    | Boston   | 2020-01-15     | 2022-06-15   | 2020-01-15 08:00
-- def456     | CUST-1001   | John Smith    | Chicago  | 2022-06-15     | 2024-03-20   | 2022-06-15 08:00
...
# Compile snapshot SQL without materializing it
...
WHERE
    ...
    valid_from <= DATE '2023-06-15'
    AND valid_to > DATE '2023-06-15';
...
FROM dim_customer c
WHERE
    c.valid_from < DATE '2024-01-01'
    AND c.valid_to >= DATE '2023-01-01';
```
...
SELECT
    ...
    (CASE WHEN valid_to = DATE '9999-12-31'
          THEN CURRENT_DATE
          ELSE valid_to END - valid_from) as days_active
FROM dim_customer
WHERE customer_id = 'CUST-1001'
...
UPDATE dim_customer
SET
    valid_to = DATE '2023-06-01',
    is_current = FALSE
WHERE customer_id = 'CUST-1001'
  AND DATE '2023-06-01' > valid_from
  AND DATE '2023-06-01' < valid_to;

-- Step 2: Insert the missing historical version
INSERT INTO dim_customer (
...
SELECT
    ...
    'Chicago',
    'IL',
    DATE '2023-06-01',  -- When change actually occurred
    COALESCE(MIN(valid_from), DATE '9999-12-31'),
    MIN(valid_from) IS NULL
FROM dim_customer
WHERE customer_id = 'CUST-1001'
  AND valid_from > DATE '2023-06-01';
```
...
-- Catch-all partition for rows effective in 2025 and later
CREATE TABLE dim_customer_2025_plus
    PARTITION OF dim_customer_partitioned
...
SELECT
    ...
    (next_valid_from - valid_to) as gap_days
