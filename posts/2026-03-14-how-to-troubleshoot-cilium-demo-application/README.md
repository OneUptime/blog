...
- Access to `cilium-dbg` through the Cilium agent pods
...
kubectl -n kube-system exec ds/cilium -c cilium-agent -- \
  ...
  jq '.[] | select(.status.labels.id | any(contains("app="))) | {
    ...
    ingress_l4: .status.policy.realized.l4.ingress,
    egress_l4: .status.policy.realized.l4.egress
  }'
```
...
- **Policy changes not propagating**: Check the endpoint state and agent logs. If an endpoint remains stale after policy changes, recreate the affected pod so Cilium allocates and programs a fresh endpoint.
