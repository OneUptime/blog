...
Adding an unrelated response field is usually compatible when the consumer's decoder tolerates it. Adding an optional request field is normally compatible. A broad schema-diff policy can still flag those changes for review, but a consumer contract should not fail unless the consumer actually relies on an exact closed object.
...
Then assert consumer behavior rather than merely asserting that the mock returned its own fixture:
...
Provider verification should call the real routing, deserialization, validation, authorization policy appropriate to the test identity, and response serialization. Pact's provider guidance warns against stubbing above the layer where request bodies are extracted and validated; doing so can allow invalid payloads to pass verification. Stub only unavoidable downstream boundaries, and contract-test those separately when they are services you control.
...
consumer commit
  ...
  -> query compatibility before deployment
```
...
Pact Broker's recommended deployment check uses version and target environment information, for example:
...
Run the equivalent check for a provider candidate. Use the CLI version pinned by your build and the current Pact Broker documentation; older tag-based workflows and newer environment-based workflows have different flags.
...
2. provider verifies all known consumer contracts;
3. consumers migrate and publish new contracts that no longer require `amountMinor`;
4. new consumer versions deploy and their deployments are recorded; and
5. provider removes `amountMinor` only when the compatibility matrix shows no relevant deployed consumer requires it.
...
- **Exact matching of irrelevant fields:** makes harmless additions or dynamic values look breaking.
- **Broad matching of required fields:** allows a type or structure the consumer cannot parse.
- **No provider states:** makes tests depend on shared data and fail intermittently.
