...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf ct list | tail -20
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf lb list | head -20

# Check for policy-related issues
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg policy get | head -30
...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "errors|drops|failures"
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel version meets Cilium's current system requirements. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg policy get`. Verify node-to-node connectivity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-health status --verbose`.
