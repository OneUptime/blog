...
The official `par2cmdline` project provides `create`, `verify`, and `repair`. Check the installed build:
...
```bash
cd ..
cp -a archive-2026q3 restore-test-2026q3
cd restore-test-2026q3
```

Choose an actual protected file larger than 5 MiB. Set its path explicitly and make the drill stop if the path or size is wrong. Damage a known range in the disposable copy and remove another file only if the chosen recovery budget can cover both:
...
dd if=/dev/zero \
  of="$test_file" \
  bs=1048576 count=2 seek=3 conv=notrunc status=none
...
The first verification should identify damaged slices and say whether enough recovery blocks exist. `repair` verifies its reconstructed files, and the final SHA-256 check supplies an independent byte-for-byte test.
