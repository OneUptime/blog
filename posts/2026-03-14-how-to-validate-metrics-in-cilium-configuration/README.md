...
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg endpoint list | wc -l
...
# Inspect drop and error metrics
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg metrics list | grep -E "drop|error"
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel meets Cilium's documented minimum for your release. Check init container names with `kubectl get pod -n kube-system <pod> -o jsonpath='{.spec.initContainers[*].name}'`, then inspect the relevant logs with `kubectl logs -n kube-system <pod> -c <init-container-name>`.
