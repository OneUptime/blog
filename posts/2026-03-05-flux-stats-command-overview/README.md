...
This immediately tells you how many resources of each kind are running, how many are failing, and how many are suspended.
...
- **RUNNING** -- Count of resources that are not suspended.
- **FAILING** -- Count of resources whose `Ready` condition is `False`.
...
1. **FAILING greater than 0** -- Any non-zero failing count requires investigation.
...
```bash
#!/bin/bash
FAILING_COUNT=$(flux stats --all-namespaces 2>/dev/null | awk 'NR>1 {sum += $3} END {print sum}')

if [ "$FAILING_COUNT" -gt 0 ]; then
    echo "UNHEALTHY: $FAILING_COUNT Flux resources are failing"
    flux stats --all-namespaces
    exit 1
else
    echo "HEALTHY: No Flux resources are currently failing"
    exit 0
fi
```
...
For production use, it is better to rely on Prometheus metrics such as `gotk_resource_info`, with labels like `ready` and `suspended`, which can be continuously scraped and visualized in Grafana. The `flux stats` command is best suited for interactive use and quick health checks.
    ...
    steps:
      ...
      - name: Check Flux stats
        run: |
          ...
          FAILING=$(flux stats --all-namespaces 2>/dev/null | awk 'NR>1 {sum += $3} END {print sum}')
          if [ "$FAILING" -gt 0 ]; then
            echo "::error::$FAILING Flux resources are failing"
...
The `flux stats` command is the quickest way to assess Flux CD health across your cluster. It provides aggregate counts of running, failing, and suspended resources by kind, plus storage usage for source artifacts. Use it as a daily health check, integrate it into CI/CD verification steps, and combine it with `flux get`, `flux events`, and `flux logs` for a complete debugging workflow. For continuous monitoring, combine this CLI-based approach with Prometheus metrics and Grafana dashboards for persistent tracking and alerting.
