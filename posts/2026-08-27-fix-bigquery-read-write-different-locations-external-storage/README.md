...
In a standard regional query, the source and destination datasets must match the query job's location. The Cloud Storage bucket must also be colocated with the dataset that contains the external table. A mismatch leads to errors such as `Cannot read and write in different locations`.
...
For Cloud Storage external tables, compatible locations are defined by BigQuery's external-table rules. They are more specific than simply being on the same continent.
...
Suppose the bucket is in `us-central1`. Create both BigQuery datasets in `us-central1`:
...
CREATE OR REPLACE EXTERNAL TABLE
  `example-source-project.external_data.events`
OPTIONS (
  format = 'PARQUET',
...
bq --location="${DATA_LOCATION}" query \
  ...
  --replace=true \
  --destination_table="${DESTINATION_PROJECT_ID}:${DESTINATION_DATASET}.daily_events" \
  'SELECT * FROM `example-source-project.external_data.events`'
```

Explicit `--location` prevents an empty or dynamically generated query from falling back to an unintended default. The SQL must still reference only resources permitted by that execution mode and location.

## Move the right side of the boundary

If the existing dataset location is mandatory, use a Cloud Storage location allowed for an external table in that dataset. If the bucket location is mandatory, create the external-table and destination datasets in a compatible BigQuery location.
...
Global queries do not remove the requirement that a Cloud Storage bucket be colocated with the dataset containing its external table. They address the separate boundary between BigQuery dataset locations.
...
- [Location considerations for external tables](https://cloud.google.com/bigquery/docs/external-tables#location_considerations)
