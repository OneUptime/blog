...
# Collect node diagnostics. calicoctl node commands must run on the host
# running the Calico node instance, not inside the calico-node container.
echo "Collecting diagnostics from $NODE..."
if [ "$NODE" = "$(hostname)" ]; then
  DIAG_FILE=$(sudo calicoctl node diags | awk '/Diags saved to/ {print $4}')
  if [ -n "$DIAG_FILE" ]; then
    sudo cp "$DIAG_FILE" "$DIAG_PATH/"
    sudo chown "$USER":"$(id -gn)" "$DIAG_PATH/$(basename "$DIAG_FILE")" 2>/dev/null || true
  fi
else
  DIAG_FILE=$(ssh "$NODE" "sudo calicoctl node diags" | awk '/Diags saved to/ {print $4}')
  if [ -n "$DIAG_FILE" ]; then
    ssh "$NODE" "sudo cat '$DIAG_FILE'" > "$DIAG_PATH/$(basename "$DIAG_FILE")"
  fi
fi
...
BUNDLE=$(find "$DIAG_PATH" -maxdepth 1 -name "*.tar.gz" -print -quit)
if [ -z "$BUNDLE" ]; then
...
echo "--- System Info ---"
find "$WORK_DIR" -name hostname -exec cat {} \; 2>/dev/null
find "$WORK_DIR" -name date -exec cat {} \; 2>/dev/null
echo ""

echo "--- Network Summary ---"
ROUTES=$(find "$WORK_DIR" -name ip-route -print -quit)
if [ -n "$ROUTES" ]; then
...
find "$STORAGE" -maxdepth 1 -type d -name "*incident*" -mtime +90 -exec rm -rf {} +

# Keep pre/post change diagnostics for 30 days
find "$STORAGE" -maxdepth 1 -type d \( -name "*pre-*" -o -name "*post-*" \) | while read -r d; do
...
find "$STORAGE" -maxdepth 1 -type d -name "*weekly*" -mtime +14 -exec rm -rf {} +
