...
This guide deploys Dex using the official Helm chart with GitHub and a static password database configured via Flux CD.
...
kubectl create secret generic dex-client-secrets \
  --namespace dex \
  --from-literal=GRAFANA_CLIENT_SECRET=$(openssl rand -hex 16)
```
    ...
    config:
      ...
      staticPasswords:
        - email: admin@example.com
          # bcrypt hash of "AdminPassword123!" - generate with: htpasswd -bnBC 10 "" 'AdminPassword123!' | tr -d ':\n'
          hash: "$2a$10$chKU4XZeE0ZIPRqXvcP77u3LxB4Czq2jl6JD8xvCkPS2ChqEN.JR6"
          username: admin
          userID: "1234567890"
...
- Rotate connector client secrets regularly; after updating the Kubernetes Secret, restart the Dex pods so environment-based secret values are reloaded.
- Request the `groups` scope from downstream clients and configure connector group support to propagate group membership into OIDC tokens for role-based access control in downstream apps.
