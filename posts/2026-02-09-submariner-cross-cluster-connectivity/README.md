...
The command writes broker information to `broker-info.subm` in the current directory. Use that file when joining each cluster:
...
```bash
# Use the broker-info.subm file generated by deploy-broker
...
Create a ServiceExport for the Service:

```bash
subctl export service webapp --namespace production --context cluster-1
```

Verify the ServiceExport:
...
```bash
kubectl get clusters.submariner.io -n submariner-k8s-broker --context cluster-1 -o yaml
```
...
```bash
subctl export service cassandra --namespace database --context cluster-1
```
...
kubectl exec client --context cluster-2 -- \
  nc -zv cassandra-0.cluster-1.cassandra.database.svc.clusterset.local 9042
```
...
Export the Service in each cluster:

```bash
subctl export service api-service --namespace production --context cluster-1
subctl export service api-service --namespace production --context cluster-2
```
...
## Configuring Service Exports

Create ServiceExport resources only for the services that should be visible across the cluster set:
...
Remove the ServiceExport when the service should no longer be discoverable from other clusters:

```bash
subctl unexport service webapp --namespace production --context cluster-1
```
...
subctl diagnose firewall inter-cluster --context cluster-1 --remotecontext cluster-2
...
**Services not imported**: Verify the Service exists and a ServiceExport with the same name exists in the same namespace. Check submariner-lighthouse logs:

```bash
kubectl logs -n submariner-operator deployment/submariner-lighthouse-agent --context cluster-1
```

**DNS not resolving clusterset.local**: Ensure CoreDNS forwards the `clusterset.local` domain to the Lighthouse DNS service:
...
```yaml
clusterset.local:53 {
    forward . <submariner-lighthouse-coredns-cluster-ip>
}
```
...
```bash
# Check broker endpoint resources
