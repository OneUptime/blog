...
**Protect your session cookies.** The session ID should be generated by the framework with enough entropy, and the cookie should be protected with the framework's signing or session-cookie controls.
...
def monitor_session_store():
    ...
    # Count active sessions. Use "sess:*" for the Express example, or the
    # appropriate prefix for your framework.
    session_key_pattern = os.environ.get("SESSION_KEY_PATTERN", "session:*")
    session_count = 0
    for key in r.scan_iter(match=session_key_pattern, count=1000):
        session_count += 1
