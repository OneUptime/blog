...
# Enable only the Cilium agent APIs your automation needs
extraConfig:
  enable-cilium-api-server-access: "Get*,PutEndpointID,DeleteEndpointID,PostIPAM,DeleteIPAMIP"
  enable-cilium-health-api-server-access: "GetHealthz,GetStatus,PutStatusProbe"

# Enable only the operator APIs your automation needs
operator:
  extraArgs:
    - --enable-cilium-operator-server-access=GetHealthz,GetMetrics
...
# Optimize identity management by ignoring high-cardinality labels
...
echo "Cilium endpoints: $(kubectl get ciliumendpoints -A --no-headers 2>/dev/null | wc -l | tr -d ' ')"
```
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl get ciliumendpoints -A -o wide`. Confirm the policy is applied with `kubectl get ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies -A`. Check that the endpoint has the correct identity with `kubectl get ciliumendpoint <name> -n <namespace> -o jsonpath='{.status.identity.id}'`.
