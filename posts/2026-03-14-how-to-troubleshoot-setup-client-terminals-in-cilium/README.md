...
- Access to the in-agent `cilium-dbg` CLI through the Cilium agent pod
...
- **Policy changes not propagating**: Confirm that the policy was accepted with `kubectl get cnp -A` and recreate the affected pod if its endpoint remains stale.
