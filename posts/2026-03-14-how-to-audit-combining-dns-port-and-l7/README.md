...
kubectl get ciliumendpoints --all-namespaces -o json | \
  jq '[.items[].status.policy.realized."policy-enabled"] |
      group_by(.) | map({policy_enabled: .[0], count: length})'
...
kubectl -n kube-system get pods -l k8s-app=cilium -o name | while read pod; do
  ...
  for key in enable-policy enable-l7-proxy enable-hubble; do
    kubectl -n kube-system exec "$pod" -c cilium-agent -- \
      cilium-dbg config get "$key" 2>/dev/null || true
  done
done
```
...
# Gather audit data
ENDPOINTS_JSON=$(kubectl get ciliumendpoints --all-namespaces -o json)
TOTAL_ENDPOINTS=$(echo "$ENDPOINTS_JSON" | jq '.items | length')
...
kubectl get ccnp -o json 2>/dev/null | jq -r '.items[].metadata.name' 2>/dev/null || true
```
...
- **Policy count mismatch**: Some policies may be in a failed state. Check Cilium policy status conditions with `kubectl get cnp --all-namespaces -o json | jq '.items[] | {namespace: .metadata.namespace, name: .metadata.name, status: .status}'`.
