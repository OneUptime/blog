...
Data residency regulations require that certain data stays within specific geographic boundaries. The European Union's GDPR restricts transfers of personal data outside the EEA unless an adequacy decision, appropriate safeguards, or another valid transfer mechanism applies. Similar regulations exist in Brazil (LGPD), Canada (PIPEDA), Australia, India, and many other jurisdictions. Telemetry data often contains personal identifiers like user IDs, IP addresses, and session tokens, which means it can fall under these regulations.
...
- **GDPR (EU):** Transfers of personal data of EU data subjects outside the EEA require an adequacy decision, appropriate safeguards such as standard contractual clauses, or another valid transfer mechanism.
...
For telemetry data, this translates to: spans, metrics, and logs containing user identifiers or generated by EU-serving infrastructure may need to be routed to EU-based storage or another approved destination.
...
def residency_middleware(request, handler):
    ...
    # Set the residency region as attributes on the current span
...
connectors:
  routing:
    ...
    table:
      ...
      - context: span
        condition: attributes["data.residency.region"] == "eu"
        pipelines: [traces/eu]
      # Route AP data to AP backend
      - context: span
        condition: attributes["data.residency.region"] == "ap"
        pipelines: [traces/ap]
      # Route US data (and default) to US backend
      - context: span
        condition: attributes["data.residency.region"] == "us"
        pipelines: [traces/us]
...
processors:
  ...
  transform/strip-for-global:
    error_mode: ignore
    trace_statements:
      - keep_keys(resource.attributes, ["service.name"])
      - keep_keys(span.attributes, ["http.request.method", "http.response.status_code", "data.residency.region"])
...
service:
  pipelines:
    ...
    traces/global-index:
      receivers: [otlp]
      processors: [transform/strip-for-global, batch/global]
...
processors:
  transform/strip-pii:
    ...
    trace_statements:
      ...
      - delete_key(span.attributes, "session.id")
      - delete_key(span.attributes, "client.address")

      # Redact any remaining potentially identifying attributes
      - set(span.attributes["http.request.header.x-forwarded-for"], "REDACTED")
        where span.attributes["http.request.header.x-forwarded-for"] != nil
...
processors:
  filter/require-residency:
    ...
    trace_conditions:
      ...
      - 'span.attributes["data.residency.region"] == nil'
```
