...
Nebula is an open-source overlay networking tool originally developed by Slack. It creates a mesh network where every node can communicate directly with every other node using UDP hole-punching, using lighthouse nodes for discovery and relays when direct paths fail if you've configured them. Unlike traditional hub-and-spoke VPNs, there's no single bottleneck - nodes communicate peer-to-peer whenever possible.
...
curl -LO https://github.com/slackhq/nebula/releases/download/v1.10.3/nebula-linux-amd64.tar.gz
...
# -networks: the nebula IP this node will have (pick your own /24 or larger range)
nebula-cert sign -name "lighthouse1" -networks "10.20.0.1/24"

# Generate certificates for regular nodes
nebula-cert sign -name "webserver1" -networks "10.20.0.10/24"
nebula-cert sign -name "dbserver1" -networks "10.20.0.20/24"
nebula-cert sign -name "devlaptop" -networks "10.20.0.100/24"
...
```bash
# Create the config directory on the lighthouse server
ssh user@lighthouse-ip 'sudo mkdir -p /etc/nebula'

# Copy files to the lighthouse (from your CA machine)
scp ca.crt lighthouse1.crt lighthouse1.key user@lighthouse-ip:~/

# Move them into place on the lighthouse server
ssh user@lighthouse-ip 'sudo mv ~/ca.crt ~/lighthouse1.crt ~/lighthouse1.key /etc/nebula/'
```
...
```bash
# Create the config directory on the regular node
ssh user@webserver1 'sudo mkdir -p /etc/nebula'

# Copy files (from CA machine)
scp ca.crt webserver1.crt webserver1.key user@webserver1:~/

# Move them into place on the regular node
ssh user@webserver1 'sudo mv ~/ca.crt ~/webserver1.crt ~/webserver1.key /etc/nebula/'
```
...
# Check routes installed for the Nebula interface
...
nebula-cert sign -name "dbserver1" -networks "10.20.0.20/24" -groups "servers,database"
nebula-cert sign -name "webserver1" -networks "10.20.0.10/24" -groups "servers,web"
nebula-cert sign -name "devlaptop" -networks "10.20.0.100/24" -groups "developers"
```
