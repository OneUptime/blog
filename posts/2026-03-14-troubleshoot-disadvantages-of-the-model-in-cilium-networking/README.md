...
```bash
# Check tunnel map entries used in encapsulation mode
...
- **BPF maps are full**: Check map sizes with `cilium-dbg bpf ct list | wc -l`. If approaching limits, increase `bpf-ct-global-any-max` and `bpf-ct-global-tcp-max` in Cilium configuration.
- **Performance issues after configuration change**: Check whether loaded BPF programs or maps changed unexpectedly. Use `bpftool prog show` to inspect loaded programs.
