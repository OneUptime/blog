...
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p "identity|identity_gc"

# Endpoint health metrics
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p "endpoint|endpoint_regeneration"

# Policy metrics
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p "policy|policy_implementation"

# Datapath performance metrics
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p "datapath|forward|drop"

# Agent resource metrics
kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p "process_cpu|process_resident_memory"
```
...
cilium status --brief
echo ""
echo "Identity Count: $(kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg identity list 2>/dev/null | wc -l)"
echo "Endpoint Count: $(kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list 2>/dev/null | wc -l)"
echo ""
echo "Resource Usage:"
...
echo "Cilium endpoints on one agent: $(kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list -o json 2>/dev/null | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' 2>/dev/null || echo 'N/A')"
```
...
- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get cnp,ccnp,networkpolicy -A`. Verify inter-node tunnel state with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf tunnel list` when running in tunnel mode.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list -p process_resident_memory`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list`. Confirm the policy is applied with `kubectl get cnp,ccnp,networkpolicy -A`. Check that the endpoint has the correct identity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint get <id>`.
