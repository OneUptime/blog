...
`hostvars` is a dictionary keyed by hostname. Each entry contains the host-level variables associated with that host, including gathered facts, inventory variables, and variables created with `set_fact` or `register` during the playbook run. You access it like this:

```yaml
# Access the IP address of a specific host

{{ hostvars['db-server-01']['ansible_facts']['default_ipv4']['address'] }}
```

The key thing to understand is that `hostvars` only contains facts for a host if facts have already been gathered or loaded from a fact cache for that host. If you skip fact gathering for a host and do not use cached facts, its facts will not be available through `hostvars`.
...
database:
  host: {{ hostvars['db-01']['ansible_facts']['default_ipv4']['address'] }}
  port: {{ hostvars['db-01']['db_port'] }}
  name: {{ hostvars['db-01']['db_name'] }}

app:
  listen_address: {{ ansible_facts.default_ipv4.address }}
  listen_port: 8080
```
...
# Works with AWS EC2 dynamic inventory when keyed_groups creates tag_Role_app
---
  ...
  tasks:
    - name: Build list of all monitoring targets
      ansible.builtin.set_fact:
        monitoring_targets: >-
          {{
            groups['tag_Role_app'] | map('extract', hostvars, ['ansible_facts', 'default_ipv4', 'address']) | list
          }}
...
- name: Configure webservers
  ...
  tasks:
    - debug:
        msg: "{{ hostvars['db-01']['ansible_facts']['os_family'] }}"
