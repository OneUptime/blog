...
Checking that a GET happens to include an `ETag` header is not enough. The suite must prove that comparison is strong, evaluation happens atomically with the update, a stale request has no side effect, and the response follows the API's documented conditional-update policy.
...
The final read is essential. A server can return `412` after accidentally applying the mutation. Also inspect audit events, version rows, or outbox records so a rejected update cannot leave hidden side effects.
...
The sequential stale test checks semantics but may not expose a check-then-update race. Add a barrier immediately before precondition evaluation or issue many synchronized writes using the same original tag. Release them together and assert exactly one incompatible write wins:
...
| stale strong tag | `412`, no visible or hidden side effect |
...
| `If-Match: *` and absent resource | precondition fails |
