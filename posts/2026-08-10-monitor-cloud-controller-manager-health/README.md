...
A `Running` cloud-controller-manager (CCM) Pod proves only that a process exists. In a highly available deployment, most replicas are expected standbys. The real service-level question is whether one healthy leader is reconciling Nodes, routes, and Services fast enough without being denied or throttled by Kubernetes or the provider API.
...
kubectl get service,servicemonitor,podmonitor -A | grep -i cloud-controller
...
Do not define readiness as “this replica is leader.” Standbys must stay ready enough to acquire leadership. Use the provider's built-in probes unchanged unless its documentation describes safe customization.
...
These have `name`, `type`, and status-related labels documented in the Kubernetes metrics reference. Confirm the endpoint and authorization on the deployed provider before creating a scrape job.
...
```bash
kubectl get lease -n kube-system CCM_LEASE -o json | jq '.spec | {
  holderIdentity,
  acquireTime,
  renewTime,
...
- `renewTime` becomes older than the lease duration plus a small observation allowance;
- transitions rise repeatedly without intentional rollout; or
- all replicas log inability to get or update the Lease.

The common alpha metric `leader_election_slowpath_total{name=...}` counts slow-path lease renewal behavior. Alert on an increase correlated with API latency, rather than on its lifetime value:

```promql
increase(leader_election_slowpath_total[10m]) > 0
```

Tune the window to the environment and exclude planned control-plane rollouts. Lease object monitoring is still valuable because metric scraping can fail at the same time as the component.
...
# Pending load-balancer status and recent Events
kubectl get service -A -o json | jq -r '.items[] |
  select(.spec.type=="LoadBalancer") |
  [.metadata.namespace, .metadata.name, (.status.loadBalancer // {})] | @json'
kubectl get events -A --sort-by=.lastTimestamp | grep -iE 'loadbalancer|cloud|route|initialize'
```
...
- new `LoadBalancer` Service lacks `.status.loadBalancer` beyond its provisioning objective;
...
CCM talks to the Kubernetes API through client-go. Monitor request errors, latency, and throttling by status code and verb while avoiding unbounded URL labels. Common client metrics can include `rest_client_requests_total`, request duration, and client-side rate-limiter metrics depending on the linked libraries and release.
...
| Pending LB Service age high | Service controller or provider mutation failed | Service Events, class, annotations, IAM/quota |
