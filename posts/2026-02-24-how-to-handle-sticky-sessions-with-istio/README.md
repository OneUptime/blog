...
Kubernetes Services support `sessionAffinity: ClientIP`, but it's crude and can be bypassed for in-mesh traffic because the sidecar proxy makes the upstream routing decision. Istio has its own mechanism: consistent hash load balancing, which is more flexible and gives you multiple options for identifying the "same user."

## How Consistent Hashing Works

Consistent hashing maps a key (like a user ID or cookie value) to a specific backend endpoint. As long as the key is the same and the backend set does not change, the request goes to the same pod. If a pod goes away, only a fraction of traffic gets redistributed instead of reshuffling all sessions.
...
Every request with the same `x-user-id` value goes to the same pod while the endpoint set is stable. Different user IDs may or may not go to the same pod depending on the hash distribution.
...
When a request arrives without the `istio-session` cookie, Istio generates the cookie value in the response. Subsequent requests include the cookie, and Istio uses that value as the hash key to route them to the same backend while the endpoint set is stable.
...
Be careful with source IP affinity when traffic comes through a load balancer or proxy, because all requests might appear to come from the same IP. For HTTP traffic, use `x-forwarded-for` or another header instead.
...
Requests to `http://api-service/data?session_id=abc123` will go to the same backend while the endpoint set is stable.
...
Each version subset has its own session cookie when a VirtualService routes traffic to that subset. This is useful during canary deployments where you want session affinity within each version.
...
If you configure HTTP route timeouts, set a WebSocket route timeout to `0s` so long-lived connections are not closed by that route timeout:
...
Setting `timeout: 0s` disables the route timeout for WebSocket connections, which are long-lived by nature. If you are not setting HTTP route timeouts, Istio's default HTTP route timeout is already disabled.

## What Happens When a Pod Scales Down

When a pod that has session affinity gets removed (scaling down, rolling update, pod eviction), the sessions that were pinned to it need to go somewhere. Consistent hashing handles this more gracefully than a simple modulo hash - only a fraction of sessions get redistributed instead of reshuffling all sessions.
...
istioctl experimental envoy-stats deploy/webapp -n default --type clusters | grep upstream_rq_total
```

You can also check the Envoy consistent hash stats:

```bash
istioctl experimental envoy-stats deploy/frontend -n default --type clusters | grep lb_healthy_panic
```

In Prometheus, if your scrape configuration preserves the pod label:

```bash
# Requests per pod (should show uneven distribution with session affinity)
sum(rate(istio_requests_total{destination_workload="webapp", reporter="destination"}[5m])) by (pod)
```
