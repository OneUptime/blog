...
String sort gives: `["medium", "large", "small"]` because string values are sorted lexicographically, so `"10"` and `"100"` come before `"9"`.

To fix string number sorting, convert to int first:

```yaml
    - name: Convert priority values to integers
      ansible.builtin.set_fact:
        normalized_numbers: >-
          {{ normalized_numbers | default([]) + [item | combine({'priority': item.priority | int})] }}
      loop: "{{ string_numbers }}"

    - name: Sort normalized numbers
      ansible.builtin.debug:
        msg: "{{ normalized_numbers | sort(attribute='priority') | map(attribute='name') | list }}"
```

## Case-Insensitive Sorting

For string attributes, `sort` is case-insensitive by default. You can use `case_sensitive=true` when uppercase and lowercase values should be sorted separately:
...
- name: Case-insensitive sort
  ...
  tasks:
    - name: Case-insensitive sort (default)
      ansible.builtin.debug:
        msg: "{{ items | sort(attribute='name') | map(attribute='name') | list }}"

    - name: Case-sensitive sort
      ansible.builtin.debug:
        msg: "{{ items | sort(attribute='name', case_sensitive=true) | map(attribute='name') | list }}"
```
...
# Implements custom sort order using a priority list
- name: Custom sort logic
  ...
  tasks:
    - name: Sort tasks by custom priority order
      ansible.builtin.set_fact:
        sorted_tasks: >-
          {{ sorted_tasks | default([]) + (tasks_list | selectattr('priority', 'equalto', item) | list) }}
      loop: "{{ priority_order }}"
...
# Demonstrates stable sorting: first by name, then by department
...
graph TD
    ...
    B -->|Case-sensitive| E["sort(attribute='field', case_sensitive=true)"]
    ...
    - name: Generate zone file
      ansible.builtin.copy:
        content: |
          ...
          {{ record.name }}\tIN\t{{ record.type }}{% if record.type == 'MX' %}\t{{ record.priority }}{% endif %}\t{{ record.value }}
          {% endfor %}
        dest: /tmp/zone.db
```
...
The `sort` filter with `attribute` is the primary tool for ordering lists of dictionaries. Use `reverse=true` for descending order, `case_sensitive=true` when uppercase and lowercase values should be sorted separately, and dot notation for nested attributes. For multi-key sorting, take advantage of stable sort by applying sorts in reverse order of priority (sort by the least important key first, then by the most important key last). When you need custom ordering logic that does not fit the built-in comparisons, fall back to Jinja2 expressions.
