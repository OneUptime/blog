...
The Istio DNS proxy is useful because it knows about ServiceEntries that CoreDNS has no idea about. If you define a ServiceEntry for a custom hostname that is not present in DNS, CoreDNS won't resolve it (unless you also add a Kubernetes DNS record such as an ExternalName service). But Istio's DNS proxy can resolve it because it has access to the full Istio service registry.
...
- `PILOT_ENABLE_IP_AUTOALLOCATE` enables the controller that automatically assigns virtual IPs to ServiceEntries that don't have addresses. This lets Envoy route TCP traffic to these services without needing NONE resolution.
...
With `resolution: DNS`, the proxy asynchronously performs DNS lookups for the host and load balances across the results. Istio currently refreshes these DNS results on a fixed 30-second interval rather than synchronously resolving on each request.
...
`DNS_ROUND_ROBIN` is similar to `DNS`, but when a new connection is initiated it uses the first IP address returned by DNS instead of relying on the complete DNS result set. Existing connections are retained even if DNS records change frequently, which reduces connection pool draining for large external services.
...
When using `REGISTRY_ONLY`, you need ServiceEntries for every external service. This is where the DNS proxy becomes valuable. Without it, a ServiceEntry for a custom hostname that only Istio knows about would not make that hostname resolvable by CoreDNS. For TCP ServiceEntries without explicit addresses, DNS proxying plus IP auto-allocation lets Istio return a virtual IP and route the connection to the right service.
...
For wildcard TLS or HTTP entries on current Istio versions, `resolution: DYNAMIC_DNS` lets the proxy dynamically resolve the concrete hostname from SNI or the HTTP Host header. For raw TCP wildcard entries, `resolution: NONE` is still the usual passthrough choice because the proxy cannot recover the original hostname.
...
The DNS proxy can answer known ServiceEntry and Kubernetes service names locally and forward unknown names upstream. This can improve DNS performance compared to going to CoreDNS every time, especially in large clusters where CoreDNS might be under load. Remember that Envoy's own `resolution: DNS` lookups are separate from application DNS proxying and are refreshed asynchronously.
