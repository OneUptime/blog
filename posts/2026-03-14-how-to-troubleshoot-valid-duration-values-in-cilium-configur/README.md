...
- A running Kubernetes cluster on a Kubernetes version supported by your Cilium release (for Cilium 1.19, Kubernetes 1.31-1.34)
...
# Resolution: Increase memory limits and add label exclusion patterns for labels that are not policy-relevant
helm upgrade cilium cilium/cilium \
  --namespace kube-system \
  --reuse-values \
  --set resources.limits.memory=2Gi \
  --set 'labels=!job-name !rollouts-pod-template-hash'
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel version meets Cilium's system requirements, such as Linux 5.10 or later for current releases (or an equivalent distribution kernel). Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get cnp,ccnp -A`. Verify inter-node tunnel connectivity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg bpf tunnel list`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning identity-relevant label patterns to reduce identity count. Increase agent memory limits if needed. Check `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg metrics list | grep process_resident_memory`.
...
- **Policy not being enforced**: Verify the policy selectors match the intended pods using `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint list`. Confirm the policy resources are applied with `kubectl get cnp,ccnp -A`. Check that the endpoint has the correct identity with `kubectl exec -n kube-system ds/cilium -c cilium-agent -- cilium-dbg endpoint get <id>`.
