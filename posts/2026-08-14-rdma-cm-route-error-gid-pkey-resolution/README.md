...
`RDMA_CM_EVENT_ROUTE_ERROR` has a precise position in the RDMA CM state machine. Address resolution completed far enough for the application to call `rdma_resolve_route()`, but route resolution did not produce the information needed to establish a connection.
...
rdma_resolve_route()
  -> RDMA_CM_EVENT_ROUTE_RESOLVED or RDMA_CM_EVENT_ROUTE_ERROR
rdma_connect()
~~~

Calling `rdma_resolve_route()` before address resolution has completed is an application error. The official rdma-core `rping` example calls it only from the `ADDR_RESOLVED` event handler.

## Confirm Which Device and Port Address Resolution Selected

When no source address is supplied, RDMA CM uses local routing information to select one and binds the CM ID to a local RDMA device. A route to the destination through the management network can therefore select a different interface from the intended RDMA fabric.
...
$ ls -l /sys/class/infiniband/mlx5_0/ports/1/gid_attrs/ndevs 2>/dev/null
$ ls -l /sys/class/infiniband/mlx5_0/ports/1/gid_attrs/types 2>/dev/null
~~~
...
Both endpoints and the intervening fabric must have compatible partition membership. A nonzero entry is not enough; verify the exact P_Key and full or limited membership policy intended by the fabric administrator. The default partition can make a basic test pass while the application's chosen partition fails.
...
On native InfiniBand, logical port state should be `Active`, and Subnet Administrator queries must succeed. A physical `LinkUp` state alone is insufficient. Confirm:
...
- the requested P_Key, service level, MTU, and rate can produce a path record;
- SA permissions and partitions allow the query.
...
For RoCE, there is no InfiniBand Subnet Administrator path-record lookup. Focus on IP routing, neighbor resolution, GID selection, VLAN, and the Ethernet path. A title or log that mentions GIDs does not imply native InfiniBand.
...
| `rdma link show` | RDMA device, port, and netdev association |
