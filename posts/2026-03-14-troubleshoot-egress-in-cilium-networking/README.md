...
```bash
# Check egress gateway BPF policy entries
...
kubectl exec diag-pod -- curl -sk --max-time 5 https://kubernetes.default.svc/version 2>&1

# Test external connectivity
kubectl exec diag-pod -- curl -s --max-time 5 https://example.com 2>&1

# Check DNS resolution
kubectl exec diag-pod -- nslookup kubernetes.default

# Clean up after completing any Hubble checks below
...
- **Cilium monitor shows no output**: The monitor may not be capturing traffic on the correct endpoint. Use `cilium-dbg monitor --related-to ENDPOINT_ID` to filter for a specific endpoint from a Cilium agent pod.
...
- **BPF maps are full**: Check map sizes with `cilium-dbg bpf ct list | wc -l` from a Cilium agent pod. If approaching limits, increase conntrack table size in Helm values.
- **Performance issues after configuration change**: Check that BPF programs are loaded with `bpftool prog show`, and check Cilium agent logs for verifier or program load errors.
