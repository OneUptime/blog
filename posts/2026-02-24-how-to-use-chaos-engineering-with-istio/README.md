...
Chaos engineering is about proactively breaking things in controlled ways to find weaknesses before they cause real outages. Istio is a great tool for this because it can inject faults in the HTTP traffic path without modifying your application code. You can add latency, return error codes, and emulate slow or failed upstream calls, all through configuration.
...
Delay and abort faults are evaluated independently when both are configured. In this example, 20% of requests are aborted with a 500 response and 40% are delayed by 3 seconds, so some requests can be both delayed and aborted. This simulates a degraded service that is both slow and partially failing.
...
Now make one or more ratings endpoints return real 5xx responses, then send enough traffic to observe outlier detection. Client-side abort faults from a VirtualService are generated by the caller's proxy and are not a reliable way to prove that an upstream endpoint was ejected:
...
```bash
# Quick rollback - delete fault injection VirtualServices labeled as chaos experiments
