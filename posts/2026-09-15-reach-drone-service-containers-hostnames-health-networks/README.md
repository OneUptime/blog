# How to Reach Drone Services: Hostnames, Ports, Health Checks, and Networks
...
Drone converts `$$` to `$` before the commands run, so the doubled dollars shown here are valid but optional for unbraced shell expressions. Escape braced runtime expressions as `$${NAME}` in inline YAML. Checked-in shell scripts need no Drone escaping. See [Drone environment substitution](https://docs.drone.io/pipeline/environment/substitution/).
