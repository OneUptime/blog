...
kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg endpoint list --no-headers | wc -l
...
- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get ciliumnetworkpolicies.cilium.io -A` and `kubectl get networkpolicies -A`. Verify inter-node tunnel connectivity with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg bpf tunnel list`.
