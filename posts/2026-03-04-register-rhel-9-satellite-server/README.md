...
- Network access from the RHEL system to the Satellite Server (ports 80 and 443 for registration, with ports such as 5647, 8000, 8140, 8443, and 9090 required only for features like Katello agent, provisioning, Puppet, telemetry, discovery, or Capsule communication)
...
The RHEL system needs to trust the Satellite Server's SSL certificate. On older Satellite workflows, you can download and install the CA certificate RPM:
...
For Satellite 6.9 and later, Red Hat recommends using the global registration command generated by Satellite instead of installing the `katello-ca-consumer` package manually.
...
The preferred method for Satellite registration is using an activation key. Your Satellite admin should provide the key name and organization label:
...
- Auto-attach subscriptions when Simple Content Access is disabled
...
sudo subscription-manager register --username=admin --password=your_password \
    --org=MyOrganization --environment=Production/MyContentView
```

The `--environment` flag specifies the lifecycle environment and content view in the `LifecycleEnvironment/ContentView` format. Common lifecycle environment names are `Library`, `Development`, `Staging`, and `Production`, depending on how your Satellite is configured.

## Step 5 - Install Host Tools (Optional)

Satellite host tools enable additional host integration features such as Tracer, which reports services and applications that need restarting after updates:

```bash
# Enable the Satellite tools repository
sudo subscription-manager repos --enable=satellite-client-6-for-rhel-9-x86_64-rpms

# Install Tracer host tools
sudo dnf install -y katello-host-tools-tracer
```

After installation, upload trace data with `sudo katello-tracer-upload`.
...
Satellite 6.9 and later support a simplified global registration method, and this is the recommended method for current Satellite versions. In the Satellite web UI, go to Hosts, then Register Host. This generates a curl command you can run on the RHEL system:
...
```bash
# Older workflow: install the Capsule CA certificate
...
For Satellite 6.9 and later, generate the registration command in Satellite with the Capsule selected as the registration Capsule or content source. The Capsule handles content delivery locally while the Satellite maintains centralized management.
...
**Unable to reach Satellite**: Check firewall rules. The client needs access to the Satellite on port 443 for registration and HTTPS content access. Port 80 can also be needed for global registration completion callbacks, provisioning templates, and older CA RPM download workflows:
