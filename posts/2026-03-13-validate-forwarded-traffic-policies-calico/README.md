...
# View host-level policy rules on nodes using the iptables dataplane
sudo iptables -L -n | grep CALICO

# Check Felix status on node
kubectl exec -n calico-system ds/calico-node -c calico-node -- /bin/calico-node -felix-live
```
