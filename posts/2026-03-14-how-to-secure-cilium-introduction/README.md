...
kubectl get cnp -n default intro-sample-policy
```
...
hubble observe --verdict DROPPED --namespace production --last 20
```

## Troubleshooting

- **Policy not taking effect**: Verify endpoint labels match policy selectors with `kubectl get ciliumendpoints -A -o json | jq '.items[] | {namespace: .metadata.namespace, name: .metadata.name, labels: .status.identity.labels}'`.
