...
The IPReservation resource in Calico allows you to reserve specific IP addresses or CIDR ranges so they are not automatically assigned to pods by the Calico IPAM. This is essential when certain IPs within your pool CIDR are already in use by external systems, load balancers, or legacy infrastructure.

Without IP reservations, Calico may automatically assign an IP that conflicts with an existing device on your network. This leads to connectivity failures that are difficult to diagnose because the conflict may not be immediately apparent.
...
- **spec.reservedCIDRs**: A list of CIDRs or individual IPs to reserve. Individual IPs can be written as plain addresses or as /32 or /128 CIDRs
...
This prevents Calico from automatically assigning any of the 384 IPs in these two ranges to pods.
...
## Grouping Reservations for Different Purposes

Organize reservations by purpose inside one or two resources. Calico checks IPReservations during each automatic IPAM allocation request, so avoid creating many small IPReservation resources:

```yaml
apiVersion: projectcalico.org/v3
kind: IPReservation
metadata:
  name: static-infrastructure-reservations
spec:
  reservedCIDRs:
    - 10.244.100.0/28
    - 10.244.50.10/32
...
```bash
calicoctl apply -f reservations.yaml
```
...
# Test that a reserved IP is not automatically assigned
...
- If a pod receives a reserved IP, the reservation may have been created after the IP was already allocated. Restart the pod to trigger reallocation, unless the pod is explicitly requesting that IP
...
Calico IPReservation resources provide a clean way to prevent IP conflicts between automatic pod allocations and existing network infrastructure. By organizing reservations into logical groups and applying them before deploying workloads, you ensure that Calico IPAM does not automatically assign an IP that belongs to another system. Review your network inventory and create reservations for all known static IPs within your pool CIDRs.
