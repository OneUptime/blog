...
Discover all production applications:
  ...
  generators:
    - git:
        ...
        directories:
          ...
          - path: "apps/deprecated-*/overlays/production"
            exclude: true
  template:
    metadata:
...
Use different sync policies based on the environment with Go templating and `templatePatch`:
...
For production environments where you want manual sync, leave `autoSync` false or use ArgoCD Projects with sync windows to restrict when syncs are allowed.
...
kubectl get applications -n argocd -l argocd.argoproj.io/application-set-name=my-api-environments
```
