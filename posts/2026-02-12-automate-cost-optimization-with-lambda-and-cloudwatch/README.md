        ...
        for iid in instance_ids:
            ec2.create_tags(
                ...
                Tags=[
                    ...
                    {'Key': 'StoppedDate', 'Value': datetime.now(timezone.utc).strftime('%Y-%m-%d')}
                ]
            )
    ...
    for page in paginator.paginate(Filters=[{'Name': 'status', 'Values': ['available']}]):
        for volume in page['Volumes']:
            ...
            if 'OrphanedDate' not in tags:
                ...
                orphaned_date = datetime.strptime(tags['OrphanedDate'], '%Y-%m-%d').date()
                days_orphaned = (today - orphaned_date).days

                if days_orphaned >= GRACE_PERIOD_DAYS:
                    # Snapshot before deleting (safety net)
                    ec2.create_snapshot(
                        VolumeId=vol_id,
                        Description=f'Pre-deletion backup by cost-automation on {today.strftime("%Y-%m-%d")}',
                        TagSpecifications=[{
...
def lambda_handler(event, context):
    ...
    cost_automation_cutoff = datetime.now(timezone.utc) - timedelta(days=30)
    ...
    for page in paginator.paginate(OwnerIds=['self']):
        for snap in page['Snapshots']:
            ...
            # Delete pre-deletion backups after a shorter retention
            snapshot_cutoff = cost_automation_cutoff if tags.get('Source') == 'cost-automation' else cutoff

            if snap['StartTime'] < snapshot_cutoff:
...
def lambda_handler(event, context):
    ...
    service_costs = sorted(service_totals.items(), key=lambda x: x[1], reverse=True)
...
aws lambda add-permission \
  --statement-id "daily-cost-check-eventbridge" \
  --action "lambda:InvokeFunction" \
  --principal "events.amazonaws.com" \
  --function-name "arn:aws:lambda:us-east-1:123456789012:function:cost-alert" \
  --source-arn "arn:aws:events:us-east-1:123456789012:rule/daily-cost-check"
...
aws lambda add-permission \
  --statement-id "weekly-idle-check-eventbridge" \
  --action "lambda:InvokeFunction" \
  --principal "events.amazonaws.com" \
  --function-name "arn:aws:lambda:us-east-1:123456789012:function:idle-instance-checker" \
  --source-arn "arn:aws:events:us-east-1:123456789012:rule/weekly-idle-check"
...
aws lambda add-permission \
  --statement-id "monthly-snapshot-cleanup-eventbridge" \
  --action "lambda:InvokeFunction" \
  --principal "events.amazonaws.com" \
  --function-name "arn:aws:lambda:us-east-1:123456789012:function:snapshot-cleanup" \
  --source-arn "arn:aws:events:us-east-1:123456789012:rule/monthly-snapshot-cleanup"
```

## IAM Permissions

Each Lambda function needs appropriate execution role permissions. Here's a sample policy:
