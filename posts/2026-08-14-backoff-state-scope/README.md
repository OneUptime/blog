...
Putting all of these in one shared counter creates coupling. Keeping all of them per request provides no protection against a coordinated fleet. Separate them, then scope each to the failure domain it represents.

## Keep Attempt Progression Per Logical Request

The attempt number, previous decorrelated-jitter delay, elapsed deadline, and last failure belong to one operation. Request A should not enter its fourth-delay backoff because unrelated request B failed three times.
...
server delay for the next attempt
idempotency and conditional-write data
~~~

Resetting this state means the logical operation completed or a genuinely new operation began. A later retry of the same durable job after process restart may need to restore the attempt count and first-seen time rather than pretend it is new.

Always add independent jitter to per-request scheduling. A shared host failure timestamp plus a deterministic delay can make every request wake together.
...
The correct key comes from the dependency contract. AWS adaptive retry documentation warns that its rate limiter operates per SDK client instance and assumes a single resource. Using one adaptive client across multiple resources lets throttling on one resource slow all of them. That is evidence to partition client or limiter state by the service's throttling dimension. As of August 2026, AWS marks the documented 2026 cross-SDK retry behavior as opt-in through <code>AWS_NEW_RETRIES_2026=true</code>, so verify which behavior the deployed SDK is using.
...
gRPC retry throttling is maintained per server name under its retry design, not per individual RPC. That lets success and failure across calls protect a service from retry overload. It does not replace each RPC's own attempt counter or deadline.
...
They add failure modes and latency. A central limiter that is unavailable needs an explicit fail-open or fail-closed policy. Its clock and lease semantics must tolerate partitions. Do not write every request's next backoff timestamp to a central database; coordinate admission or token allocation at a coarser level and keep final jitter local.

A hybrid design can lease a block of retry tokens to each process. The central allocator limits fleet capacity while local acquisition stays fast. Leases need expiry, bounded size, and conservative behavior after a client loses contact.

## Isolate Tenants Within a Shared Destination

A destination-wide safety budget prevents total overload, but one tenant can spend all of it. Use hierarchical admission:

1. wait through cancellation-aware, independently jittered backoff without holding scarce capacity;
2. recheck the deadline, cancellation, and retry eligibility;
3. acquire from the global destination and tenant budgets atomically, or refund the first if the second rejects;
4. acquire an attempt-scoped concurrency permit;
5. recheck cancellation, send, and account for capacity according to the defined outcome.
...
- a shared dependency outage must drain its aggregate retry budget;
...
Keep attempt progression and jitter per logical request. Share retry tokens, rate observations, circuit state, and concurrency only across calls that truly share a failure or throttling domain. Add tenant and fleet coordination as separate admission layers, not as one global backoff counter.
