...
Ansible's `kubernetes.core.k8s` module accepts manifest files through the `src` parameter. For directories of manifests, you can collect the files and loop over them. This guide covers the different approaches and when each makes sense.
...
pip install kubernetes jsonpatch kubernetes-validate
```
...
The `src` parameter takes a local path on the machine running Ansible. Use an absolute path or build a path from `{{ playbook_dir }}` when you want it anchored to the playbook. The file can contain any valid Kubernetes resource definition.
...
Sometimes you want to use existing manifest files but override certain values. Use Ansible's lookup and template capabilities to load the file into `definition`.
...
- name: Override the container image
  ansible.builtin.set_fact:
    deployment: "{{ deployment | combine({'spec': {'template': {'spec': {'containers': [deployment.spec.template.spec.containers[0] | combine({'image': 'myapp/web:' + image_tag})]}}}}, recursive=True) }}"
...
- name: Check Kubernetes manifest
  ...
  tasks:
    - name: Check the deployment manifest
      kubernetes.core.k8s:
        state: present
        src: manifests/deployment.yml
        validate:
    ...
    - name: Show what would change
      ansible.builtin.debug:
        msg: "Would change: {{ check_result.changed }}"
```

You can also run a whole playbook in check mode:

```bash
ansible-playbook apply-manifest.yml --check
```

The `validate` option checks the resource definition against the Kubernetes schema and requires the `kubernetes-validate` Python package.

```yaml
# task: validate manifest before applying
- name: Validate and apply the deployment manifest
  kubernetes.core.k8s:
    state: present
    src: manifests/deployment.yml
    validate:
