...
Use either `source` or `sources` for an Application. When `sources` is set, ArgoCD ignores the singular `source` field. When you switch from `source` to `sources`, ArgoCD treats it as an update to the Application resource.
...
For a lower-risk migration, create a new multi-source application alongside the existing one, verify it, then switch. This can still affect ArgoCD's resource tracking when both Applications point at the same live resources, so keep the new Application on manual sync until the rendered manifests have been compared:
...
```bash
# Save manifests before updating, then compare after the Application uses the external values file
argocd app manifests my-app > my-app-manifests-after-values.yaml
diff my-app-manifests-before.yaml my-app-manifests-after-values.yaml
```
...
All applications generated by the ApplicationSet will be updated with multi-source configuration.
