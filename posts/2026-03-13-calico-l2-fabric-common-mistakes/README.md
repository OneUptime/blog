...
Overlay networking mistakes in Calico tend to cause two categories of failures: complete connectivity loss (when the encapsulation protocol is blocked) and intermittent/partial failures (when MTU is misconfigured). The intermittent failures are the most dangerous because they are hard to reproduce and easy to misdiagnose as application bugs.

This post covers the most common overlay mistakes with diagnostic commands and fixes.
...
The most common overlay mistake is forgetting to add security group or firewall rules for the encapsulation protocol. Without these rules, encapsulated cross-node pod traffic is silently dropped.
...
# For VXLAN, capture on the overlay interface to see if traffic is being sent
...
# Check if VXLAN traffic is being sent but not received
...
# For IP-in-IP, check the tunnel interface and protocol 4 on the node interface
sudo tcpdump -i tunl0 -n -c 5
sudo tcpdump -i eth0 -n 'ip proto 4' -c 10
```
...
**Symptom**: Small HTTP requests work; large file transfers fail or stall. `ping` with small packets works; `ping -s` near the expected pod MTU fails.
...
# If this fails, the usable path MTU is lower than expected
```
...
```
The updated MTU applies to new workloads, so recreate existing pods after changing it.

## Mistake 3: VXLAN and IP-in-IP Both Configured

Configuring both `vxlanMode` and `ipipMode` on the same IPPool is invalid in current Calico releases; the IPPool schema says these fields cannot be set at the same time.
...
- Verify security group rules for the encapsulation protocol as part of your cluster provisioning checklist
...
The most common overlay mistakes are missing firewall rules (encapsulation protocol blocked), incorrect MTU (large packets silently dropped), dual-mode configuration, missing FDB entries for new nodes, and CrossSubnet misconfigured for the actual topology. Building these checks into your provisioning and ongoing monitoring processes prevents the majority of overlay incidents.
