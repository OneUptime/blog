...
Monitoring iptables-based masquerading in cilium provides early warning when configuration drift, resource exhaustion, or component failures affect networking. Cilium can use iptables for masquerading instead of eBPF. The iptables mode is the legacy approach that works on all kernel versions, with lower performance compared to eBPF masquerading. By default, it masquerades traffic leaving on a non-Cilium network device.
...
# cilium_datapath_conntrack_gc_entries - conntrack garbage-collection metric
kubectl exec -n kube-system ds/cilium -- cilium-dbg metrics list | grep "datapath_conntrack_gc_entries"

# PromQL queries for Grafana panels:

# Panel 1: Conntrack entries after garbage collection
...
- **Hubble shows no flows**: Ensure Hubble is enabled with `cilium config view | grep hubble` and `cilium-dbg status` from a Cilium pod. Restart Hubble relay if needed.
