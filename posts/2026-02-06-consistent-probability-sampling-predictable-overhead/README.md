...
Consistent probability sampling offers a solution by reducing data volume while maintaining statistical validity. Unlike independent random sampling in each service, consistent probability sampling uses shared trace context so spans in a distributed trace follow a consistent sampling decision, reducing incomplete traces and sampling bias.

## Understanding Consistent Probability Sampling

Consistent probability sampling works by deriving randomness from the trace ID or from sampling information propagated in trace context. With parent-based SDK sampling, downstream services follow the upstream sampled flag; with Collector-side probabilistic sampling, collectors use the trace ID and the same sampler configuration to make consistent decisions.
...
graph TD
    ...
    H --> I[Service C]
    I --> J[Complete Trace]
    F --> K[Reduced Export and Storage Overhead]
```

## The Math Behind Probability Sampling

Conceptually, the sampling decision is based on comparing deterministic trace randomness against a threshold value. For a sampling rate of P (where 0 < P <= 1):
...
exporters:
  ...
  # Collector internal metrics are configured under service.telemetry
        ...
        and:
          and_sub_policy:
            - name: slow-latency
              ...
              latency:
                threshold_ms: 1000
            - name: slow-probabilistic
              type: probabilistic
              probabilistic:
                sampling_percentage: 50

      # Sample a 5% baseline across traces
      - name: normal-traces
        type: probabilistic
        probabilistic:
...
- `otelcol_processor_probabilistic_sampler_count_traces_sampled{sampled="true"}`: Number of sampled traces
- `otelcol_processor_probabilistic_sampler_count_traces_sampled{sampled="false"}`: Number of dropped traces
...
**Tail-based + Probability**: Use probability sampling as a baseline alongside tail sampling rules

**Priority sampling**: Override probability sampling for high-priority traces

```yaml
processors:
  # Use tail sampling to keep priority traces, then sample a baseline
  tail_sampling:
    policies:
      ...
      # Sample a baseline across traces
      - name: sampled-traces
        type: probabilistic
        probabilistic:
...
- **Solution**: Ensure Collector instances in the same tier use the same hash seed, and verify SDKs use a ParentBased sampler so downstream spans follow the propagated sampling decision
