...
```bash
# Check tunnel map status when Cilium runs in encapsulation mode
...
# Verify current Cilium NAT rules on a node
kubectl exec -n kube-system ds/cilium -- iptables -t nat -L CILIUM_POST_nat -n 2>/dev/null | head -10
...
- **BPF maps are full**: Check connection tracking entries with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf ct list | wc -l`. If approaching limits, review garbage collection settings and consider increasing `bpf.ctTcpMax` or `bpf.ctAnyMax` in Helm values.
- **Performance issues after configuration change**: Check BPF datapath map pressure and traffic counters with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf metrics list`.
