...
The `integrity` field is a Subresource Integrity string, usually using SHA-512, ensuring the downloaded package matches what was expected.
...
- Installs based on package.json and the existing lock file when present
...
Ensure team uses a compatible npm major version:
...
# Use a pinned npm version
npx npm@9.9.4 install
```
...
# View readable diff
git diff --word-diff package-lock.json
```
...
| Format | JSON | Custom |
| Generator | npm | Yarn |
| Lockfile version | 1, 2, 3 | Yarn-specific |
| Compatibility | Used by npm | Used by Yarn; npm can read it only when npm lock files are absent |
