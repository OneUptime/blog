...
A snapshot child records the identity and path of its parent. If a parent was renamed, deleted, replaced, or written after the child was created, the chain can no longer represent one coherent sequence of disk changes. The correct first response is to stop writes, identify the active leaf through vSphere, preserve metadata, and run a read-only consistency check from the correct host.
...
3. Record its datastore, controller slot, capacity, and complete disk-file path.
...
Each descriptor has a content ID, or `CID`. A snapshot child records the expected content ID of its immediate parent as `parentCID`, along with a `parentFileNameHint`. In a healthy link:
...
A mismatch has two materially different causes:

1. Metadata alone was accidentally changed while the parent data remained the same.
2. The parent disk was actually opened and written after the child was created.
...
Determine whether anyone edited descriptors, attached an older base, reverted storage, expanded a disk, or powered a VM from a parent. Broadcom's current repair guidance limits automatic repair to controlled cases where metadata was manually modified and data was not written to the wrong disk. When uncertain, stop.
...
Broadcom documents this clone syntax for a verified, powered-off chain:
...
The source must be the active leaf, not the base. The destination must have enough capacity, the filename must be unique, and the clone must be allowed to finish. This creates a new consolidated disk while leaving the original chain available for recovery. Prefer a supported vSphere clone when the UI workflow is available.

Afterward, attach the clone through **Edit Settings > Add New Device > Existing Hard Disk** to a test or recovery VM while the original VM remains powered off. Validate partition tables, filesystems, applications, and transaction consistency before declaring success.

## Treat Repair Commands as Last Resort

Current Broadcom guidance documents `vmkfstools -x repairChain` for a narrow, controlled metadata-repair scenario. It is not a universal fix for any CID mismatch:

```bash
vmkfstools -x repairChain '/vmfs/volumes/Datastore1/AppVM/AppVM-000005.vmdk'
```

Do not run it unless the supported article applies to the exact ESXi or VCF version, all related descriptor metadata has been backed up, and evidence shows data was not written to the wrong parent. Use the documented dry-run or scripted detection path first when available.
