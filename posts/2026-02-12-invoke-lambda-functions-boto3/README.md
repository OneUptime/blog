...
The `DryRun` invocation type validates parameter values and verifies that you have permission to invoke the function, without actually running it.
...
For synchronous invocations, when a Lambda function raises an exception, the invocation still returns a 200 status code. The error shows up in the `FunctionError` field of the response.
