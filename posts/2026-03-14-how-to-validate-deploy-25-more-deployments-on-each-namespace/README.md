...
- A running Kubernetes cluster on a version supported by your Cilium release with Cilium installed
- `kubectl` configured for cluster access
- `cilium` CLI installed (matching your Cilium version)
- Helm 3.x for configuration management
- Basic familiarity with Kubernetes networking concepts
- Access to cluster nodes or a Cilium agent pod for troubleshooting with `cilium-dbg` and `cilium-health` (recommended)
...
kubectl -n kube-system exec "$CILIUM_POD" -- cilium-dbg endpoint list --no-headers | wc -l
...
echo "Cilium endpoints: $(kubectl -n kube-system exec "$CILIUM_POD" -- cilium-dbg endpoint list -o json 2>/dev/null | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' 2>/dev/null || echo 'N/A')"
```
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and that the node kernel meets the system requirements for your Cilium version. Check failing init container logs with `kubectl logs -n kube-system <pod> -c <init-container>`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl -n kube-system exec <cilium-pod> -- cilium-dbg policy get`. Verify inter-node routing state with `kubectl -n kube-system exec <cilium-pod> -- cilium-dbg bpf ipcache list`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl -n kube-system exec <cilium-pod> -- cilium-dbg metrics list | grep process_resident_memory`.
