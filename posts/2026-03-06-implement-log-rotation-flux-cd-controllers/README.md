...
logging:
  # This controls kubelet's own logs, not application container logs.
  format: text
...
# For clusters using containerd, configure CRI log line handling.
# Container log rotation is controlled by kubelet, not by this setting.
...
## Deploying Fluent Bit for Log Collection and Forwarding

Use Fluent Bit to collect Flux CD controller logs, apply filtering, and forward to a centralized backend.
    ...
    config:
      ...
      inputs: |
        [INPUT]
            Name              tail
            Tag               flux.*
            Path              /var/log/containers/source-controller*.log,/var/log/containers/kustomize-controller*.log,/var/log/containers/helm-controller*.log,/var/log/containers/notification-controller*.log,/var/log/containers/image-reflector-controller*.log,/var/log/containers/image-automation-controller*.log
            Parser            cri
...
Flux CD emits Kubernetes events. Kubernetes events expire automatically according to the API server's `--event-ttl` setting, which defaults to one hour. If your cluster uses a longer event TTL, you can set up cleanup for old Flux CD events to prevent etcd bloat.
...
Prevent Flux CD controllers from consuming excessive CPU and memory while processing reconciliations.
      ...
      rules:
        ...
        - alert: FluxControllerHighLogVolume
          ...
          labels:
            severity: warning
          annotations:
            summary: "Flux controllers are generating excessive logs"
            description: >-
              The Fluent Bit input dedicated to Flux controller logs is ingesting
              more than 1MB/s. Consider adjusting the controller log level.
...
- Setting resource limits on controllers to prevent excessive CPU and memory use
