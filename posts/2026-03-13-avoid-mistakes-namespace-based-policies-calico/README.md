...
The most common mistake: you set up namespace isolation perfectly, then a new namespace is created without the required labels, and it can bypass the isolation policies that select namespaces by those labels.
...
## Mistake 2: Using kubernetes.io/metadata.name When You Mean Semantic Groups

The `kubernetes.io/metadata.name` label is automatically set by Kubernetes and equals the namespace name. It is useful when you intentionally want to target one exact namespace, but using it for broader policy groups creates brittle rules tied to specific namespace names rather than semantic attributes.

```yaml
# Brittle - tied to exact namespace name
namespaceSelector: kubernetes.io/metadata.name == 'monitoring'

# Better - semantic label not tied to one namespace name
namespaceSelector: team == 'observability' && purpose == 'monitoring'
```

## Mistake 3: Missing Egress Rules for Cross-Namespace Calls

When a pod in namespace A calls a service in namespace B, you need an egress rule in namespace A if namespace A is isolated for egress, and an ingress rule in namespace B if namespace B is isolated for ingress. Missing either required rule blocks the traffic.
...
An allow rule for new connections from A->B does not automatically allow new connections from B->A. Always test both directions explicitly.
...
If you have both a namespace-scoped NetworkPolicy and a GlobalNetworkPolicy with conflicting rules in the same tier, policies are evaluated from lower `order` to higher `order`, and the first matching Allow or Deny action is final.
...
flowchart TD
    ...
    B -->|No| C[Can miss label-based isolation]
...
Namespace-based Calico policy mistakes typically fall into three categories: missing labels on namespaces, incomplete directional rules, and policy ordering conflicts. Build namespace labeling into your namespace creation process (use Helm hooks or admission webhooks), configure ingress and egress rules where the source or destination namespace is isolated for that direction, and regularly audit your policy ordering to prevent conflicts. A namespace creation checklist that includes labeling will prevent the majority of these issues.
