...
When Calico is installed via binary management on bare metal, configuration lives in two places: the systemd service unit files on each node and the Calico resources in the Kubernetes API. A good binary management setup treats both as code - templating the service unit files through your configuration management tool and managing the Calico resources through version-controlled YAML files applied with `calicoctl`.

This dual-source configuration model is the key distinction of binary management Calico deployments. Changes to process-level settings require re-running the configuration management playbook, while changes to Calico datastore-backed networking settings are applied through `calicoctl` without touching the nodes. Understanding this separation helps you design a maintainable configuration strategy.
...
## Step 4: Configure Felix via Resource
...
Configuring binary-managed Calico on bare metal uses Ansible templates for process-level service configuration and version-controlled Calico resource YAML for cluster-wide settings. This dual-source model makes all configuration auditable and repeatable, which is essential for maintaining a large fleet of bare metal Kubernetes nodes over time.
