...
- A Kubernetes cluster with Flux CD installed with the `helm.toolkit.fluxcd.io/v2` HelmRelease API, such as Flux v2.3 or later
...
Strategic merge patches are configured under `spec.postRenderers` in a HelmRelease. Use the `kustomize.patches` field with an inline strategic merge patch:
...
The `kustomize.patches` field also supports target selectors:
...
Strategic merge patches merge arrays for fields that define a merge strategy. For containers and environment variables, the merge key is `name`. This lets you add environment variables to an existing container without removing existing ones:
