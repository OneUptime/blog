...
graph TD
    ...
    C --> E[Pay per request<br/>No capacity planning<br/>Automatic scaling]
...
- Instantly accommodates up to double the previous peak traffic
...
```python
# Connect to Keyspaces and query with on-demand
...
# Write data - on-demand scales automatically within its burst limits
insert_stmt = session.prepare("""
    INSERT INTO user_events (user_id, event_time, event_type, payload)
    VALUES (?, ?, ?, ?)
""")

session.execute(insert_stmt, ['user-123', datetime.now(timezone.utc), 'page_view', '{"url": "/home"}'])
```
...
On-demand:
  ...
  Cost:   2.592B / 1M x $0.125 = $324/month for reads
  Writes: 500 writes/sec x 86,400 x 30 = 1.296B writes
  Cost:   1.296B / 1M x $0.625 = $810/month for writes
  Total:  ~$1,134/month
...
For a steady workload, provisioned is more than 3 times cheaper.
...
On-demand:
  ...
  Cost: 1.318B / 1M x $0.125 = $165/month (reads only)
...
Note: You can switch from on-demand to provisioned at any time, but you can switch from provisioned to on-demand only once in a 24-hour period.
...
aws cloudwatch put-metric-alarm \
  ...
  --dimensions Name=Keyspace,Value=my_application Name=TableName,Value=orders Name=Operation,Value=SELECT \
  --statistic Sum \
  --period 60 \
  --threshold 10 \
...
3. **Are there idle periods?** On-demand read/write throughput costs $0 when there is no traffic. Provisioned charges for the minimum capacity 24/7.

4. **Can you tolerate brief throttling during scale-up?** Auto-scaling modifies provisioned throughput only after workload changes are sustained for several minutes. On-demand handles bursts up to double the previous peak instantly.

5. **Is cost the top priority?** Provisioned capacity with Database Savings Plans gives the lowest possible price for steady workloads.

## Summary

On-demand and provisioned capacity modes serve different workload patterns. Start with on-demand for new projects and unpredictable workloads, then switch to provisioned with auto-scaling once your traffic patterns stabilize. The cost difference can be significant - provisioned mode with auto-scaling can be several times cheaper for steady workloads. But on-demand's zero-management overhead and automatic scaling make it the right choice when simplicity and flexibility matter more than cost optimization.
