...
For Cassandra, the documented SSTable compression ratio is stored size relative to uncompressed size. Measure it per table because data and algorithms differ. Its denominator is uncompressed SSTable data, not accepted raw payload bytes, so it is not interchangeable with the encoded ratio above.
...
For tiered or changing retention, sum only the time buckets and classes still retained at the forecast date, using the stored size for their tier at that date:
...
Add measured steady components only to the extent they are not already included in the primary stored bytes. Sum their physical footprint across all copies in the storage pool:
...
Prometheus documents that source and output blocks coexist during compaction and recommends setting size retention to at most 80 to 85 percent of its allocated disk to leave space for compaction. WAL and memory-mapped head chunks count toward the size limit, but only persistent blocks are deleted to enforce it, so their peak footprint must also fit. Apache Cassandra documents that immutable SSTables retain old versions and tombstones until compaction writes merged SSTables and releases the old files.
...
```text
allocated disk capacity >= required physical bytes / safe fill ratio
```

Use the capacity visible to the engine for this ratio; convert to raw device capacity separately if lower storage layers consume space. Avoid counting the same reserve in both forms. If 55 TB includes all steady and temporary requirements and the platform's tested maximum safe fill is 80 percent:

```text
allocated disk capacity >= 55 / 0.80 = 68.75 TB
```
