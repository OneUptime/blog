...
Troubleshooting masquerade traffic to remote nodes in cilium requires understanding how Cilium implements this feature and where failures can occur in the data path. When endpoints communicate with remote node addresses, Cilium can be configured to masquerade or not masquerade this traffic in BPF masquerading mode. This behavior is controlled separately from normal endpoint-to-endpoint traffic between pods on different nodes, and requires BPF masquerading to be enabled. For regular pod-to-pod traffic, the behavior depends on whether the pod CIDR is natively routable between nodes. In overlay (VXLAN/Geneve) mode, masquerading for inter-node pod traffic is typically not needed because the overlay handles encapsulation.
...
```bash
# Check tunnel endpoint map status when running in encapsulation mode
...
kubectl exec diag-pod -- curl -sk --max-time 5 https://kubernetes.default.svc:443 2>&1
...
- **Cilium monitor shows no output**: The monitor may not be capturing traffic on the correct endpoint. Use `kubectl exec -n kube-system ds/cilium -- cilium-dbg monitor --related-to ENDPOINT_ID` to filter for a specific endpoint.
...
- **BPF maps are full**: Check map sizes with `kubectl exec -n kube-system ds/cilium -- cilium-dbg bpf ct list global | wc -l`. If approaching limits, increase conntrack table size in Helm values.
- **Performance issues after configuration change**: Check whether BPF programs are being loaded successfully. Use `bpftool prog show` on the node and inspect Cilium agent logs for verifier or attachment errors.
