...
When kube-apiserver reports that it cannot create or initialize its storage backend, it has failed before it can reliably serve Kubernetes objects. The supported storage backend is etcd3. For the mutual-TLS deployment described here, kube-apiserver must resolve and reach at least one configured etcd client endpoint, verify the etcd serving certificate, present its own valid client certificate and key, and communicate with a healthy quorum.
...
The exact paths and endpoints are deployment-specific. Confirm the paths exist **inside** the container through its configured mounts, the key is readable only by the intended account, and every API-server replica has the endpoint set intended for its topology and valid trust material for the same etcd cluster. Local stacked-etcd endpoints and individual client certificates can differ between replicas.
...
Run resolver checks on the control-plane host as an initial check. To reproduce kube-apiserver resolution, use its network context and its actual `/etc/resolv.conf` and `/etc/hosts`; entering only the network namespace does not reproduce those files:
...
Compare answers, address families, `/etc/hosts`, `/etc/resolv.conf`, and routing on every replica. `getent ahosts` does not report DNS TTLs. A Pod DNS lookup does not prove a host-networked static Pod uses the same resolver.

Avoid round-robin DNS that can silently include nonmembers or servers whose certificates do not cover the configured endpoint hostname. A comma-separated set of named endpoints makes failures observable and avoids one DNS record hiding individual member health.
...
- etcd `--listen-client-urls` binds the destination interface (or a wildcard address) with the HTTPS scheme; account for any load-balancer or NAT forwarding;
- control-plane security groups and host firewalls allow kube-apiserver to 2379;
- return routing works through any stateful firewalls and MTU works for TLS records;
...
Inspect the served leaf certificate (the pipeline below prints only the leaf; add `-showcerts` to the standalone `s_client` command above to display all certificates sent by the server):
...
Verify the endpoint's DNS name or IP is in the SAN extension, the certificate is within its validity period, the chain terminates at `--etcd-cafile`, and the certificate is valid for server authentication. Kubernetes uses Go TLS verification, which does not treat a legacy Common Name as a replacement for SANs. OpenSSL hostname verification can fall back to Common Name, so explicitly check the SAN extension even if this command succeeds.

If connecting by IP, replace `-verify_hostname` with `-verify_ip` and the endpoint IP; SNI does not make an IP match a DNS-only SAN. Prefer correcting endpoint identity or reissuing a properly scoped certificate over skipping verification.
...
The certificate must be valid for client authentication (if Extended Key Usage is present, it must permit client authentication), be signed by an authority etcd trusts through `--trusted-ca-file`, be time-valid, and match the configured private key. Compare derived public-key digests through an approved local procedure; never paste a private key or its raw output into tickets.
...
Run this only on an authorized host and avoid shell tracing, which can expose paths or credentials. In etcd 3.6, `endpoint health` performs a linearizable read and checks active alarms, so it validates more than a TCP or TLS handshake; it does not write a test key. A permission-denied read can still pass its consensus check, so it does not prove Kubernetes object access is authorized. Add `--cluster` only for a separate check of client URLs discovered from the member list; that option replaces the explicit endpoint list being tested. Compare member IDs, leader, Raft term/index, database size, errors, and version.

If one client endpoint fails but quorum is healthy, distinguish a temporary kube-apiserver client configuration change from an etcd membership change. Any approved change to `--etcd-servers` must respect each replica's topology and keep it connected to the same etcd cluster, preserving redundant healthy endpoints where configured. Remove or replace an etcd member only through a quorum-safe repair procedure; membership changes can destroy quorum if the cluster is already degraded.

## Check etcd Itself

Inspect member logs and metrics for leader elections, lost peers, slow `wal_fsync`, slow backend commits, quota alarms, corruption alarms, clock problems, and listener errors. The lower bound on etcd consensus write latency is determined by peer network round-trip and durable disk latency. A reachable but overloaded member can make storage initialization exceed its deadline.
