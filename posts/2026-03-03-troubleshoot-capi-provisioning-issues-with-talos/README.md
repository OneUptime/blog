...
# Retrieve the Talos client configuration generated by CABPT
kubectl get secret my-cluster-talosconfig \
  -o jsonpath='{.data.talosconfig}' | base64 -d > cluster-talosconfig
...
talosctl service --nodes <cp-ip> | grep etcd

# Check etcd logs for cluster formation issues
talosctl logs etcd --nodes <cp-ip> --tail 50
...
talosctl logs kubelet --nodes <worker-ip> --tail 50

# Verify the worker can reach the API server
talosctl get machineconfig --nodes <worker-ip> \
