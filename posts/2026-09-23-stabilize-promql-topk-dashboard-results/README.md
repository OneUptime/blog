...
For membership independent of dashboard width, use a fixed ranking window, such as one day, at the endpoint:
...
Use this expression inside the same `topk` filter. For reproducible incident reports, freeze the dashboard's absolute start and end timestamps. If identical membership must survive endpoint changes, save a reviewed service selection as a dashboard variable or configuration rather than recomputing the ranking every refresh.
...
The graph step determines how many displayed values are evaluated. With the ranking fixed at `end()`, changing the graph resolution does not independently rerank each step. Use a fixed display window such as `[5m]` when comparing values across panel sizes.
...
Run the ranking as an instant query at the chart endpoint and save its labels. Run the complete graph at two different steps and compare the distinct returned identities with that set. The graph may contain fewer selected services at individual times if a service did not yet exist or lacks samples.
