...
Google Cloud's Application Load Balancers support path-based routing out of the box, but most guides only cover simple prefix matching. For load balancer types that support route-rule regular expressions, regex matching gives you the flexibility to route based on URL patterns like API version numbers, file extensions, or dynamic path segments.
...
Google Cloud URL map route rules support several types of path matching:
...
- Path template match: Routes paths matching a path template with named variables
...
Regex route matching is not supported on every Google Cloud load balancer. Use it with supported Application Load Balancer products, such as regional external Application Load Balancers and internal Application Load Balancers. Global external Application Load Balancers and classic Application Load Balancers do not support regular-expression route matching.
  ...
  - name: api-matcher
    ...
    routeRules:
      ...
      - priority: 1
        ...
        routeAction:
          weightedBackendServices:
            - backendService: projects/my-project/regions/us-central1/backendServices/api-v1-backend
              weight: 100
      ...
      - priority: 2
        ...
        routeAction:
          weightedBackendServices:
            - backendService: projects/my-project/regions/us-central1/backendServices/api-v2-backend
              weight: 100
      ...
      - priority: 3
        ...
        routeAction:
          weightedBackendServices:
            - backendService: projects/my-project/regions/us-central1/backendServices/static-backend
              weight: 100
...
Import the URL map. Use `--region` for a regional external or regional internal Application Load Balancer.
...
pathMatchers:
  - name: api-matcher-v2
    ...
    routeRules:
      ...
      # /v1/users -> /api/v2/ with a 301 redirect
      - priority: 1
        matchRules:
          - regexMatch: "/v1/(.*)"
        urlRedirect:
...
```hcl
resource "google_compute_region_url_map" "advanced_routing" {
  name            = "my-advanced-url-map"
  ...
  path_matcher {
    ...
    route_rules {
      ...
      route_action {
        weighted_backend_services {
          backend_service = google_compute_region_backend_service.api_v1.id
          weight          = 100
        }
      }
    }
    ...
    route_rules {
      ...
      route_action {
        weighted_backend_services {
          backend_service = google_compute_region_backend_service.api_v2.id
          weight          = 100
        }
      }
    }
    ...
    route_rules {
      ...
      route_action {
        weighted_backend_services {
          backend_service = google_compute_region_backend_service.static.id
          weight          = 100
        }
      }
    }
...
RE2 does not support backreferences, lookahead, or lookbehind assertions. The `.*` pattern is greedy by default; use `.*?` if you need the non-greedy form. Perl character classes like `\d` are supported in RE2, but `[0-9]` is often clearer in URL map examples. Named capture groups use `(?P<name>...)` or `(?<name>...)` syntax.
...
Regex path matching on supported Google Cloud Application Load Balancers gives you the flexibility to handle complex URL patterns without restructuring your application. Combined with route actions like rewrites, redirects, and header modifications, it covers most real-world routing scenarios you will encounter.
