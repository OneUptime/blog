...
- A running Kubernetes cluster supported by your Cilium version (for Cilium 1.19, Kubernetes v1.31-v1.34)
...
echo "Identity count: $(kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg identity list -o json 2>/dev/null | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' 2>/dev/null || echo 'check manually')"
```
...
# Inspect BPF datapath state on a Cilium agent
CILIUM_POD=$(kubectl -n kube-system get pods -l k8s-app=cilium -o jsonpath='{.items[0].metadata.name}')
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg bpf ct list | tail -20
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg bpf lb list | head -20

# Check for policy-related issues
kubectl get ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies,networkpolicies --all-namespaces

# Verify node-to-node connectivity
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-health status --verbose
```
...
CILIUM_POD=$(kubectl -n kube-system get pods -l k8s-app=cilium -o jsonpath='{.items[0].metadata.name}')
kubectl -n kube-system exec "$CILIUM_POD" -c cilium-agent -- cilium-dbg metrics list | grep -E "errors|drops|failures"
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel version is 5.10 or later, or a distribution-supported equivalent such as 4.18 on RHEL 8.10. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.

- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl get ciliumnetworkpolicies,ciliumclusterwidenetworkpolicies,networkpolicies --all-namespaces`. Verify inter-node tunnel connectivity with `kubectl -n kube-system exec <cilium-pod> -c cilium-agent -- cilium-dbg bpf tunnel list`.
...
- **High resource usage**: Review resource consumption with `kubectl top pods -n kube-system -l k8s-app=cilium`. Consider tuning label exclusion to reduce identity count. Increase agent memory limits if needed. Check `kubectl -n kube-system exec <cilium-pod> -c cilium-agent -- cilium-dbg metrics list | grep process_resident_memory`.
