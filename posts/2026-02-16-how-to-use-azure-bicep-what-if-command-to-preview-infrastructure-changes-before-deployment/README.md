...
- **No change**: The resource exists and will be redeployed, but its properties will not change
- **No effect**: The property is read-only and will be ignored by the service
...
This pipeline runs what-if on every pull request that changes infrastructure files and publishes the results as a build artifact.
...
**Incremental mode** (default): Only resources in the template are deployed. Resources that exist in Azure but are not in the template are left alone, so removing a resource block from the template does not delete that resource.
...
**Some expressions cannot be fully evaluated.** For example, what-if cannot resolve the `reference()` function outside the deployment context, and functions like `utcNow()` can appear as unevaluated expressions. These can show as changes even when the final deployed value would not change.
...
if [ "$DELETE_COUNT" -gt 0 ]; then
  ...
  echo ""
  echo "If these deletions are intentional, update the pipeline policy before rerunning."
  exit 1
fi
