...
- `cilium` CLI and Hubble CLI available, with access to `cilium-dbg` in Cilium agent pods
...
CILIUM_POD=<CILIUM_POD_ON_ENDPOINT_NODE>
...
kubectl -n kube-system exec "$CILIUM_POD" -- \
  ...
  jq '.[] | select(.status.labels.id | any(contains("app="))) | {
    ...
    realized_l4_policy: .status.policy.realized.l4
  }'
```
...
- **Policy changes not propagating**: Inspect the rendered endpoint policy with `cilium-dbg endpoint get`; if it remains stale, review agent logs and recreate the affected workload pod.
