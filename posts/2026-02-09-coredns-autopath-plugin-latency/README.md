...
But for a service in a different namespace, a query like "api-service.production" may try multiple paths:

1. Query: api-service.production.default.svc.cluster.local (NXDOMAIN)
2. Query: api-service.production.svc.cluster.local (success)

This wastes bandwidth and adds latency. The autopath plugin solves this.

## How Autopath Works

The autopath plugin performs server-side search path completion based on where the querying pod is located. If it finds an answer at a later search path entry, CoreDNS returns that answer with a CNAME from the original searched name.
...
The `@kubernetes` syntax tells autopath to get pod information from the kubernetes plugin. The kubernetes plugin must use `pods verified` for this to work correctly.
...
cat /etc/resolv.conf
# Should show the normal Kubernetes search path and ndots option
...
curl http://localhost:9153/metrics | grep coredns_autopath_success_total
  ...
  dns-alerts.yaml: |
    ...
    - name: dns
      ...
      - alert: LowCacheHitRate
        expr: sum(rate(coredns_cache_hits_total[5m])) / sum(rate(coredns_cache_requests_total[5m])) < 0.8
        for: 10m
        labels:
          severity: warning
        annotations:
...
Be careful when combining autopath with NodeLocal DNSCache:
...
Queries hit the node-local cache first, and only miss queries reach CoreDNS. However, Kubernetes-backed autopath requires the DNS packet received by CoreDNS to have the querying pod's IP address as the remote address. If NodeLocal DNSCache forwards the query from the node-local agent address, CoreDNS cannot identify the original pod, so autopath may not apply. Test this interaction in your cluster before relying on both optimizations together.
...
The autopath plugin is one of the most effective optimizations for DNS performance in Kubernetes. By reducing unnecessary query attempts, you reduce latency, lower CoreDNS load, and improve overall application responsiveness. When combined with proper caching strategies and tested carefully alongside NodeLocal DNSCache, autopath can help DNS infrastructure scale efficiently even in large clusters.
