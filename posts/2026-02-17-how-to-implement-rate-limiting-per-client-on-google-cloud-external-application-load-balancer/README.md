...
You can also rate limit by HTTP cookie, XFF IP (the client IP from X-Forwarded-For header), or a combination of up to three keys using `--enforce-on-key-configs`:
...
Rules are evaluated in priority order (lower number = higher priority), so the payment endpoint rule at priority 800 evaluates before the general API rule at priority 901.
...
You can configure a custom error response body so clients get a meaningful message when rate limited. Custom error responses are configured on the URL map, and the error response policy must match the status code generated by your Cloud Armor rule:

```yaml
# In the exported URL map YAML
defaultCustomErrorResponsePolicy:
  errorResponseRules:
    - matchResponseCodes:
      - 429
      path: '/rate-limited.json'
  errorService: https://www.googleapis.com/compute/v1/projects/PROJECT_ID/global/backendBuckets/CUSTOM_ERRORS_BUCKET
```

```bash
# Export, edit, and re-import the URL map after adding the custom error response policy
...
# Add a Retry-After response header to proxied backend responses if your application needs it
gcloud compute backend-services update my-api-backend \
    --custom-response-header="Retry-After: 60" \
    --global
```

## Monitoring Rate Limiting in Action

Cloud Armor logs are part of Cloud Load Balancing request logs. Enable logging on the backend service and keep the sampling rate in mind, then query these logs to understand your traffic patterns:
