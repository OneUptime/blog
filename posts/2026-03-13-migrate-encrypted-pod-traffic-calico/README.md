...
Encrypted Pod Traffic in Calico helps protect inter-node pod communication from interception or tampering while traffic is in transit between nodes. Using WireGuard, Calico encrypts supported data-plane traffic transparently, without requiring application changes.
...
kubectl exec -n kube-system <calico-node-pod-on-node1> -- wg show all

# Check encryption statistics
kubectl exec -n kube-system <calico-node-pod-on-node1> -- wg show all | grep transfer

# Verify no unencrypted traffic (packet capture should show WireGuard frames)
kubectl debug node/node1 -it --image=nicolaka/netshoot --profile=sysadmin -- tcpdump -i any -n udp port 51820
```
...
flowchart LR
    ...
    E[Attacker] -.->|Cannot read traffic| B
```
