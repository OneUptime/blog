...
Calico's `projectcalico.org/v3` `NetworkPolicy` and `GlobalNetworkPolicy` resources (with application layer policy enabled for Istio integration) allow you to write rules that are evaluated by Istio's Envoy sidecar proxies rather than only at the network layer. This enables fine-grained control like "allow GET requests to /api/health while other methods and paths remain denied by default."
...
echo "GET /api/v1/data should return 200"

# Test method/path denied by default
kubectl exec -n production frontend-pod -- curl -s -o /dev/null -w "%{http_code}\n" -X DELETE http://backend-api:8080/api/v1/admin
echo "DELETE /api/v1/admin should return 403"
```
