...
**Memory encryption**: Each virtual machine gets its own encryption key, managed by the hardware. Memory pages belonging to the VM are encrypted by the processor's memory encryption engine before they leave the CPU. Even if someone reads the physical RAM, they get encrypted garbage.
...
az vm list-sizes --location eastus -o table | grep -i "Standard_DC.*_cc_v5"

# The DCas_cc_v5 series supports confidential child VMs for AKS confidential containers
# Example sizes:
# Standard_DC4as_cc_v5 (4 vCPU, 16 GB RAM)
# Standard_DC8as_cc_v5 (8 vCPU, 32 GB RAM)
# Standard_DC16as_cc_v5 (16 vCPU, 64 GB RAM)

# Install the required preview and policy-generation extensions
...
# You should see kata-cc-isolation listed
```
...
In a traditional container, the host kernel and hypervisor can read container memory. In a confidential container, the application and its guest kernel are inside the trust boundary. The hypervisor and host OS are outside - they can see encrypted data but cannot decrypt it.

## Configuring Security Policies

Confidential containers on AKS use a security policy to define what operations are allowed inside the encrypted VM. Generate this policy from the workload manifest with the `confcom` Azure CLI extension; the policy is enforced by the Kata agent in the guest and measured for attestation.

```bash
# Generate and inject the base64-encoded security policy into the manifest
az confcom katapolicygen --yaml confidential-deployment.yaml
```
  ...
  template:
    metadata:
      labels:
        app: secure-processor
      annotations:
        # Base64-encoded policy generated by az confcom katapolicygen
        io.katacontainers.config.agent.policy: "<generated-policy-base64>"
    spec:
      runtimeClassName: kata-cc-isolation
      nodeSelector:
        workload: confidential
...
# Deploy with an attestation sidecar that can verify the TEE and perform secure key release
apiVersion: v1
kind: Pod
metadata:
  name: attested-app
spec:
  ...
  containers:
    - name: app
      ...
      ports:
        - containerPort: 8443
    # SKR sidecar performs attestation-backed secure key release
    - name: attestation
...
The attestation sidecar uses Microsoft Azure Attestation claims to verify that the pod is running in a genuine SEV-SNP confidential environment. Your application can call the sidecar before processing sensitive data or releasing keys.
