...
Cloud Audit Logs contain a goldmine of security and operational intelligence. API calls, resource changes, and authentication events are recorded across Google Cloud services. But raw audit logs in Cloud Logging are hard to query at scale, expire after a retention period, and are not easy to visualize. By routing them to BigQuery, you unlock SQL-based analysis, long-term retention, and the ability to build dashboards in Looker Studio that give your team visibility into what is happening across your GCP organization.

This guide covers setting up the full pipeline from log export through analysis and visualization.

## Architecture

The pipeline is straightforward. Cloud Audit Logs are generated by GCP services. Organization-level log sinks route them to BigQuery. BigQuery stores and enables SQL analysis. Looker Studio connects to BigQuery for visualization. BigQuery scheduled queries run periodic analysis queries.
...
# Grant query access to the security team
bq query --use_legacy_sql=false \
  'GRANT `roles/bigquery.dataViewer`
   ON SCHEMA `audit-project`.org_audit_logs
   TO "group:security-team@example.com"'
```
...
Raw audit log tables are complex. Create views that simplify common queries.

```sql
-- View: User activity summary
-- Tracks what each user has been doing across the organization
CREATE VIEW `audit-project.org_audit_logs.user_activity_summary` AS
SELECT
    timestamp,
...
CREATE VIEW `audit-project.org_audit_logs.iam_changes` AS
SELECT
    timestamp,
    ...
    protopayload_auditlog.requestMetadata.callerIp AS source_ip,
    JSON_QUERY(protopayload_auditlog.requestJson, '$.policy') AS new_policy,
    severity
FROM `audit-project.org_audit_logs.cloudaudit_googleapis_com_activity`
...
FROM `audit-project.org_audit_logs.cloudaudit_googleapis_com_data_access`
WHERE protopayload_auditlog.serviceName = 'login.googleapis.com'
...
bq query --use_legacy_sql=false --schedule="every 1 hours" \
  ...
  WHERE r.ip NOT IN ("private", "gce-internal-ip")
...
Audit logs can generate significant BigQuery storage costs at scale. A few tips for managing this. Use partitioned tables (the log sinks create them automatically with the `--use-partitioned-tables` flag). Set up table expiration for older detailed logs while keeping scheduled query output tables for summarized data. Use BigQuery's long-term storage pricing by keeping tables untouched for over 90 days - the price drops automatically.

Building an audit log analysis pipeline with BigQuery gives your security team superpowers. Instead of manually searching through Cloud Logging, they get SQL-powered analysis across your entire organization with dashboards that update as Looker Studio refreshes the BigQuery data. The upfront investment in setting up the sinks and views pays off every time someone asks "what happened?" and you have the answer in seconds instead of hours.
