...
talosctl patch mc --nodes 10.0.1.10 --patch @my-patch.yaml

# Apply a patch without rebooting (if the change supports it)
talosctl patch mc --nodes 10.0.1.10 --patch @my-patch.yaml --mode no-reboot

# Apply patches from multiple files
talosctl patch mc --nodes 10.0.1.10 \
...
# Inline JSON patch
talosctl patch mc --nodes 10.0.1.10 \
    --patch '[{"op": "add", "path": "/machine/nodeLabels", "value": {"topology.kubernetes.io/zone": "us-east-1a"}}]'

# Inline strategic merge patch (using YAML)
talosctl gen config my-cluster https://10.0.1.100:6443 \
    --config-patch '{"cluster": {"proxy": {"disabled": true}}}'
```
...
talosctl patch mc --nodes 10.0.1.10 --patch @patches/worker-1.yaml
```
...
cluster:
  proxy:
    disabled: true
# Result: disabled is set to true
```
...
**Lists** behave differently depending on the field. In general, list values are appended, while some fields are replaced or merged using a key. For example, multi-document configuration patches are matched by `kind`, `apiVersion`, and `name` for named documents:

```yaml
# Existing config has a LinkConfig document named eth0
# Patch adds a LinkConfig document named eth1
apiVersion: v1alpha1
kind: LinkConfig
name: eth1
up: true
...
# Result: both eth0 and eth1 LinkConfig documents are present
```
