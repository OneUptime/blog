...
When you deploy a pod with Istio sidecar injection enabled, two things typically happen before your application container starts receiving traffic:
...
If Istio CNI is enabled, the CNI node agent sets up the same traffic redirection instead of injecting the privileged `istio-init` container. Either way, the redirection rules send the configured inbound and outbound TCP traffic through the Envoy proxy. Your application thinks it's talking directly to the network, but captured TCP connections go through Envoy first.

## How the Init Container Sets Up Rules

The `istio-init` container runs a small program called `istio-iptables` that configures the pod's network namespace. In clusters that use Istio CNI, the CNI plugin applies equivalent rules during pod network setup. You can see the resulting rules by exec-ing into a pod:
...
**ISTIO_INBOUND**: This chain catches configured incoming TCP traffic to the pod. It skips certain ports that Envoy uses internally (like 15008 for HBONE tunneling, 15021 for health checks, and 15090 for Prometheus metrics) and redirects the rest to the `ISTIO_IN_REDIRECT` chain.
...
**ISTIO_OUTPUT**: This chain handles outbound traffic originating from within the pod. The rules here are a bit more nuanced. Traffic from UID 1337 (the Envoy proxy user) is handled specially to avoid infinite redirect loops. Envoy's upstream traffic is allowed to leave without being captured again, while Envoy-to-application traffic on the pod loopback path is handled as inbound traffic.
...
With the default `REDIRECT` interception mode, your application usually sees the connection as coming from the sidecar over the pod-local path rather than directly from the original client. This is worth noting because if your app relies on the source IP of the client, you'll need to take extra steps, such as using TPROXY mode where it is supported.
...
Second, if you use `hostNetwork: true` pods, automatic Istio sidecar injection is ignored. The sidecar model assumes the iptables changes are isolated inside the pod network namespace; host-network pods violate that assumption and can cause host-level routing failures.

Third, health check probes from the kubelet need special handling. Istio rewrites HTTP, TCP, and gRPC probe definitions by default so the kubelet checks the sidecar agent's probe endpoint instead of being broken by mTLS or TCP redirection behavior.
...
If your mesh uses the `istio-init` init container, you can also check the init container logs:
