...
The `v1beta1` API documents region reachability as `<cluster-name>.<namespace>.svc.<domain>`. The chart also creates a public client Service and the operator creates the internal service paths it needs. With chart-managed self-signer or cert-manager TLS and `fullnameOverride: orders-db`, generated node certificates include names such as `orders-db-public.crdb-prod.svc.cluster.local`, wildcard pod-service names, and the join Service. If your Kubernetes DNS suffix is not `cluster.local`, change `cockroachdb.clusterDomain`, set each region's `domain` to the DNS domain through which that region is reachable, and update any external certificate SANs to match.

For a direct `CrdbCluster` rather than Helm, the namespace-related fields have this shape; this is not a complete deployable manifest:
...
```bash
kubectl -n database-operators get services,endpointslices,pods
...
List the actual Services and EndpointSlices in the database namespace:

```bash
kubectl -n crdb-prod get service,endpointslices
...
A Helm release is namespaced; changing `--namespace` is not an in-place upgrade. For the namespace-scoped setup above, install a new operator release in the destination namespace, use the same operator version during the handoff, verify its certificate and webhook routing, and remove the old release promptly. A global operator (`watchNamespaces: ""`) cannot be installed side by side in another namespace with the same chart defaults because its cluster-scoped RBAC and PriorityClass names are shared; use the documented global-to-scoped handoff or plan a remove-and-reinstall window. Run the same operator version across all simultaneously deployed scoped operators because CRDs and admission webhooks are cluster-scoped, and keep any overlapping reconciliation window as short as possible.
