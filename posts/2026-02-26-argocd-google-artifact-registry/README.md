...
Replace `123456789012-compute@developer.gserviceaccount.com` with the IAM service account used by your node pool. For GKE Standard clusters, the default compute service account can pull from Artifact Registry in the same project when it has the Artifact Registry Reader role.
...
# Also grant access to Docker repos if the repo server needs it
gcloud artifacts repositories add-iam-policy-binding my-docker-repo \
  --location=us-central1 \
  --member="serviceAccount:argocd-repo-server@my-project.iam.gserviceaccount.com" \
  --role="roles/artifactregistry.reader"
...
For ArgoCD Image Updater, use a separate service account and bind it to the Image Updater Kubernetes service account:
...
For authentication, Workload Identity gives the repo server a Google identity, but Helm still needs registry credentials or a credential helper configured in the repo-server environment. If you need explicit credentials, use a repository credential template:

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: artifact-registry-helm-creds
  namespace: argocd
  labels:
...
```bash
# Grant the GKE node service account access to another project's Artifact Registry for image pulls
...
# Grant ArgoCD repo server or Image Updater service accounts access if they read the same repository
gcloud artifacts repositories add-iam-policy-binding shared-images \
  --project=shared-project \
  --location=us-central1 \
  --member="serviceAccount:argocd-image-updater@my-project.iam.gserviceaccount.com" \
...
Artifact Registry provides automatic vulnerability scanning when the Container Scanning API is enabled. Block deployments with critical vulnerabilities using a PreSync hook:
...
Google Artifact Registry integrates smoothly with ArgoCD when you use Workload Identity-backed credentials for authentication. The key components are: Workload Identity on the repo server for OCI Helm chart access, node-level authentication for container image pulls, and the Image Updater with a GAR login script for automated deployments. With multi-project access configured, you can centralize your artifact storage while deploying across multiple GKE clusters and projects.
