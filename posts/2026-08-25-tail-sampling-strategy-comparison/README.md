...
| --- | --- | --- |
| Policy input | Accumulated received batches for the trace | Only the current incoming batch; previous batches are not re-evaluated |
...
| Pending pressure | Higher and longer | Can be lower when policies reach terminal outcomes early |
| Completeness model | Best for facts distributed across the trace | Best for monotonic facts visible in one incoming batch |
...
- count the completed trace's spans; or
...
The setting still uses `decision_wait`; in this mode it is a pending cleanup deadline, not the ordinary positive-decision time.
...
Lower-bound-only latency and minimum-only span count are monotonic in principle: once the lower bound is crossed, later data cannot undo the match. Simple status, attribute, OTTL, trace-flag, and probabilistic evaluators report stateless behavior.
...
Hard drop policies deserve special testing. The current ingest path evaluates drop policies first and defers a positive decision while a drop remains unresolved, because a future batch might match it. Pending cleanup still does not re-evaluate policies. Confirm that your intended non-dropped traces are retained; otherwise use `trace-complete` for exclusions that require a whole-trace absence proof.

## Compare Resource Use Correctly

`span-ingest` can reduce the residence time of traces that quickly match a terminal policy. It does not guarantee low memory by itself:
...
Choose `trace-complete` when the decision depends on facts spread across a trace or on stateful allocation. Choose `span-ingest` only when each desired terminal match can be proven from one incoming batch and its cleanup semantics are acceptable. Replay reordered, split batches before treating the two strategies as operational substitutes.
