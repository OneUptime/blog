...
Kubernetes ConfigMaps provide a natural home for non-sensitive shared Terraform variables. The Tofu Controller can read variables from ConfigMaps using the `varsFrom` field, allowing you to centralize shared configuration and inject it into multiple Terraform resources. When the ConfigMap changes, dependent Terraform resources will use the updated values on their next reconciliation.
...
data:
  # AWS configuration
  region: "us-east-1"
  availability_zones_json: '["us-east-1a", "us-east-1b", "us-east-1c"]'

  # Network configuration (populated after VPC is provisioned)
  vpc_cidr: "10.0.0.0/16"
  private_subnet_cidrs_json: '["10.0.11.0/24", "10.0.12.0/24", "10.0.13.0/24"]'
  public_subnet_cidrs_json: '["10.0.1.0/24", "10.0.2.0/24", "10.0.3.0/24"]'
...
data:
  region: "us-east-1"
  availability_zones_json: '["us-east-1a", "us-east-1b"]'
...
spec:
  ...
  # Omit approvePlan or set it to an empty string for manual approval mode
  approvePlan: ""
  ...
  # Avoid duplicate names here; varsFrom values override same-named inline vars
  vars:
    - name: vpc_name
...
spec:
  ...
  # Later varsFrom entries override earlier entries and same-named inline vars
  vars:
...
data:
  ...
  # ConfigMap values are strings; decode this in Terraform with jsondecode(var.addon_versions_json)
  addon_versions_json: |
    {
...
# Check the Terraform resource status and pending plan
kubectl get terraform production-networking \
  -n flux-system \
  -o jsonpath='{.status.conditions[?(@.type=="Ready")].message}{"\n"}{.status.plan.pending}{"\n"}'

# View the stored JSON plan to confirm variables are applied correctly
kubectl get secret \
  -n flux-system \
  -l infra.contrib.fluxcd.io/plan-name=production-networking.json,infra.contrib.fluxcd.io/plan-workspace=production-networking \
  -o jsonpath='{.items[0].data.tfplan}' | base64 -d | gunzip | jq '.variables'
```
...
- Use consistent naming conventions for ConfigMap keys that match your Terraform variable names exactly. The Tofu Controller passes ConfigMap keys directly as Terraform variable names. For structured values stored in ConfigMaps, pass JSON strings and decode them in Terraform with `jsondecode`.
