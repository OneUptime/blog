...
Calico on GCE has unique operational documentation requirements compared to other cloud providers. When using GCE cloud routes for native routing, the VPC static routes must be maintained in sync with each node's Pod CIDR assignment - a relationship that isn't obvious to operators unfamiliar with how Kubernetes and the GCE cloud provider allocate Pod ranges. Clear documentation of this relationship, combined with runbooks for adding nodes and responding to connectivity incidents, prevents common operational mistakes.
...
- `gcloud`, `kubectl`, and, where Calico IPAM is used, `calicoctl` access
...
graph TD
    subgraph GCP Project
        ...
        subgraph Kubernetes Cluster on GCE
...
|-----------|--------------|--------|--------|---------|
| allow-kubelet | TCP/10250 | control-plane tag or API server CIDR | kubernetes-node tag | API server to kubelet |
...
### VPC Static Routes
One route per node Pod CIDR when using GCE cloud routes:
- Format: `<node-name>-pod-cidr`
- Destination: Pod CIDR assigned to the node (/24 in GKE routes-based clusters; size may vary in self-managed clusters)
...
2. Verify Kubernetes assigns a Pod CIDR to the new node:
   kubectl get node worker-new -o jsonpath='{.spec.podCIDR}'

3. Add VPC route for the node's pod CIDR:
   POD_CIDR=$(kubectl get node worker-new -o jsonpath='{.spec.podCIDR}')
...
| Felix not starting | GCE kernel compatibility | kubectl -n kube-system logs ds/calico-node |
```

## Conclusion

GCE Calico documentation must clearly explain the VPC static route lifecycle - especially that routes must exist when nodes join and be removed when they leave. In clusters where the Kubernetes GCE cloud provider is not managing those routes automatically, operators can add nodes without the corresponding VPC route, resulting in one-way connectivity failures that are confusing to diagnose. A clear provisioning checklist and quick troubleshooting reference table help operators get to the right fix quickly.
