...
- **Policy not taking effect**: Verify endpoint labels match policy selectors with `kubectl get ciliumendpoints --all-namespaces -o json | jq '.items[] | .status.identity.labels'`.
