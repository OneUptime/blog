...
- A running Kubernetes cluster with a Cilium version supported for your Kubernetes version
...
kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg bpf metrics list | grep DROP
...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` or can be mounted by Cilium, and confirm the host kernel meets the minimum required for your Cilium version. Check init container logs with `kubectl logs -n kube-system <pod> -c config`.
