...
ml-platform-gitops/
  kubeflow-pipelines/
    base/
      kustomization.yaml
      namespace.yaml
      cluster-scoped-resources/ # KFP CRDs and cluster-scoped resources
        kustomization.yaml
...
Kubeflow Pipelines uses Argo Workflows as its default execution engine. If you manage Argo Workflows separately from the upstream KFP manifests, install it before the KFP API server:
...
Store pipeline definitions alongside your infrastructure. In Kubernetes Native API mode (KFP 2.14.0+), compiled pipelines can be managed as Kubernetes custom resources. When a data scientist updates a pipeline definition, ArgoCD deploys it:
...
metadata:
  ...
  labels:
    ...
    version: "3"
spec:
  displayName: Training Pipeline v3
  pipelineName: training-pipeline
  pipelineSpec:
    # Compiled KFP PipelineSpec IR generated by the KFP SDK
    pipelineInfo:
      name: training-pipeline
    root:
      dag:
