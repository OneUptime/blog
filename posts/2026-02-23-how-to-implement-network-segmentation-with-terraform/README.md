...
resource "aws_eip" "nat" {
  ...
  tags = { Name = "${var.project}-nat-eip-${var.availability_zones[count.index]}" }
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = { Name = "${var.project}-nat-${var.availability_zones[count.index]}" }
...
resource "aws_network_acl" "isolated" {
  ...
  # Allow isolated workloads to initiate HTTPS connections to VPC endpoints
  egress {
    protocol   = "tcp"
    rule_no    = 90
    action     = "allow"
