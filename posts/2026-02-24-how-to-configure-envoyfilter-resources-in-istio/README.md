...
- Adding custom HTTP response headers with Envoy-specific logic that Istio APIs do not support
- Configuring Envoy features that Istio does not expose (like compression, low-level CORS fine-tuning, or custom load balancing)
...
- **BOOTSTRAP** - Deprecated bootstrap configuration patching
...
Bootstrap patches are deprecated in EnvoyFilter. When used, they apply when the proxy starts and cannot be changed at runtime:
...
istioctl proxy-config listener deployment/my-service -o json | python3 -m json.tool | grep "my-filter-name"

# Dump the full Envoy config
istioctl proxy-config all deployment/my-service -o json > envoy-config.json

# Check for configuration errors
istioctl analyze -n production

# Look at proxy logs for config rejection
kubectl logs deployment/my-service -c istio-proxy | grep -i "error\|reject\|invalid"

# Compare expected vs actual config
istioctl proxy-config listener deployment/my-service -o json | jq '.[] | select(.name=="0.0.0.0_8080")'
```
...
3. **Use `proxyVersion` matches for version-sensitive patches** - When Istio upgrades, the underlying Envoy config structure can change
4. **Document your EnvoyFilter resources** - Future you will not remember why a specific patch exists
5. **Monitor proxy config rejection** - Some patches may have no effect, and invalid generated config can be rejected by Envoy
