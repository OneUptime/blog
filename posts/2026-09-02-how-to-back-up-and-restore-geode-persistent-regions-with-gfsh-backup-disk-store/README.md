...
`backup disk-store` is an online command and requires `gfsh` to be connected to a JMX manager. The directory passed to `--dir` must already exist and be writable from every server process that will back up a disk store.
...
no INCOMPLETE_BACKUP file exists
...
If a member was absent from the baseline or did not exist then, that member writes a full backup into the incremental set. If a member's incremental backup is incomplete, its next backup is full rather than trusting the broken chain; unaffected members can still use their valid baselines. Do not assume that missing or corrupted files inside an otherwise selected baseline chain will be repaired automatically; validate the retained chain and periodically start a new full baseline.
...
Validation is an offline disk-store operation. Run it against the backup copies, not a disk store owned by a running member. For each backed-up store, resolve the directory or directories containing that store's backup files and run:
...
The exact member directory includes host, member, process, and disk-store identity information generated by Geode; do not assume the simplified example path. If a disk store uses multiple directories, provide all corresponding backup directories as a comma-separated list.
