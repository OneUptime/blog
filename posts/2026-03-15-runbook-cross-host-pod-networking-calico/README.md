...
Cross-host pod networking failures are high-severity incidents that affect inter-node pod communication in a Kubernetes cluster. When these failures occur, on-call engineers need a clear, step-by-step runbook that guides them from alert to resolution without requiring deep Calico networking expertise.

A well-built runbook for cross-host failures must cover BGP peering verification where BGP is enabled, encapsulation protocol checks, route table inspection, security group validation, and tunnel interface diagnostics. The runbook should be structured as a decision tree so engineers can quickly narrow down the root cause.
...
- BGP topology: [Full mesh/Route reflectors/N/A for VXLAN-only]
- Pod CIDR: [e.g., 10.244.0.0/16]
- Calico version: [e.g., v3.27]
- Calico namespace: [calico-system/kube-system]
```
...
kubectl exec -n <namespace> <pod-on-node-A> -- ping -c 2 -W 3 <pod-ip-on-node-B>
# FAIL -> Continue to Step 2
# PASS -> Not a cross-host issue, check application layer

# Step 2: Check calico-node status
kubectl get pods -n <calico-namespace> -l k8s-app=calico-node -o wide
# Any pods not Running/Ready -> Go to "calico-node Recovery"
# All pods Running -> Continue to Step 3

# Step 3: Check BGP peering if the cluster uses BGP
sudo calicoctl node status
# Peers not Established -> Go to "BGP Recovery"
# VXLAN-only cluster or peers Established -> Go to "Route/Tunnel Diagnosis"
```

## Phase 2: BGP Recovery Branch

```bash
# Applies to IP-in-IP, unencapsulated, and other BGP-enabled Calico designs.
# VXLAN-only pod networking does not use BGP.

# 2a: Check if BGP port is reachable between nodes
nc -zv -w 3 <peer-node-ip> 179
# FAIL -> Firewall/security group issue

# 2b: Restart calico-node on the node with broken peering
kubectl delete pod <calico-node-pod> -n <calico-namespace>
...
kubectl exec -n <namespace> <pod-on-node-A> -- ping -c 3 <pod-ip-on-node-B>
...
3. Add inbound rule: TCP 179 (BGP) from node CIDR if BGP is enabled
4. Verify: `nc -zv -w 3 <peer-node-ip> 179`

### Action: Restart calico-node
1. `kubectl delete pod <calico-node-pod> -n <calico-namespace>`
2. Wait: `kubectl wait --for=condition=Ready pod -l k8s-app=calico-node -n <calico-namespace> --timeout=120s`
...
3. Restart: `kubectl rollout restart ds/calico-node -n <calico-namespace>`
...
4. Restart: `kubectl rollout restart ds/calico-node -n <calico-namespace>`
```
...
kubectl exec -n <namespace> <pod-on-node-A> -- ping -c 5 <pod-ip-on-node-B>
...
for node_pod in $(kubectl get pods -o wide -A --no-headers | awk '$4=="Running" {print $1"/"$2":"$8}' | sort -t: -k2 -u | head -5); do
  ns_pod=$(echo "$node_pod" | cut -d: -f1)
  ns=$(echo "$ns_pod" | cut -d/ -f1)
  pod=$(echo "$ns_pod" | cut -d/ -f2)
  echo -n "$ns/$pod -> <test-ip>: "
  kubectl exec -n "$ns" "$pod" -- ping -c 1 -W 2 <test-ip> 2>&1 | grep -o "1 received" || echo "FAILED"
done
...
- All BGP peering is down across the entire cluster where BGP is enabled
