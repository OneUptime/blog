...
Calico provides multiple observability mechanisms: Felix Prometheus metrics (port 9091), flow logs for connection-level visibility through Goldmane and Whisker in Calico Open Source, and integration with Grafana for dashboards. This guide covers how to configure and use these capabilities effectively.
...
# Enable Open Source flow logs API and Whisker
kubectl apply -f - <<EOF
...
flowchart LR
    ...
    C[Flow logs API / file flow logs] --> D[Whisker / Fluent Bit]
    D --> E[Whisker / Loki / Elasticsearch]
    B --> F[Grafana]
    E --> F
    ...
    - name: calico.network
      rules:
        - alert: CalicoDataplaneFailures
          ...
          for: 5m
          annotations:
            summary: "Calico dataplane failures on {{ $labels.instance }}"
...
Calico observability requires enabling Felix Prometheus metrics, configuring flow logs for connection-level data, and building dashboards that surface actionable signals. The three most important operational signals are Felix dataplane failures (indicates dataplane programming errors), policy deny activity from flow logs or Calico Enterprise policy metrics (indicates policy misconfiguration or security events), and IPAM utilization from kube-controllers metrics (indicates capacity issues). Configure alerts for all three from day one in production clusters.
