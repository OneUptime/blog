...
- Access to run `cilium-dbg` inside Cilium agent pods
...
kubectl -n production get ciliumendpoint <POD_NAME> -o json | \
...
- **Policy changes not propagating**: Check the current policy revision with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg policy get` and wait for propagation with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg policy wait <REVISION>`.
