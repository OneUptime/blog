...
kubectl -n kube-system get configmap cilium-config -o json | \
  jq '.data | with_entries(select(.key | test("policy|audit|monitor|hubble")))'
...
TOTAL_ENDPOINTS=$(jq '.items | length' <<< "$ENDPOINTS_JSON")
...
COVERED=$(jq '[.items[] | select(.status.policy.realized."policy-enabled" != "none")] | length' <<< "$ENDPOINTS_JSON")
...
kubectl get ciliumendpoints --all-namespaces -o json | \
  jq '.items[] | {namespace: .metadata.namespace, name: .metadata.name, identity: .status.identity.id}' | head -30
```

## Troubleshooting

- **Audit script times out on large clusters**: Process namespaces in batches and increase kubectl request timeout.
- **Inconsistent data across nodes**: Ensure all Cilium agents are running the same version with `cilium status` or `cilium-dbg version` inside the Cilium agent pods.
