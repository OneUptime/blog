...
`network.enable` starts network collection, while the exporter's `network` feature makes that metric family available. The Prometheus endpoint exposes the counter as `beyla_network_flow_bytes_total`; OTLP export names it `beyla.network.flow.bytes`.
...
Metadata decoration needs a ServiceAccount with `list` and `watch` for ReplicaSets, Pods, Services, and Nodes. Network collection should run as a DaemonSet with `hostNetwork: true` so each instance can see its node's traffic.
...
```promql
sum by (k8s_dst_namespace, k8s_dst_owner_name) (
...
Use the resulting table as edges: source namespace/owner, destination namespace/owner, and byte rate as weight. A Grafana Node graph or another graph visualization can render those edges, while a regular table is often easier to audit.

## Keep cardinality bounded

Beyla intentionally defaults to owner and namespace attributes for Kubernetes flow metrics. Adding `src.address`, `dst.address`, Pod UID, or ephemeral ports can create a series for every connection and rollout.
...
If an address matches multiple definitions, Beyla uses the narrowest CIDR. A catch-all groups external traffic without exposing every public IP as a label.
...
- UDP and TCP flows have different semantics; filter or split by transport when needed.
...
- Packet sampling reduces collection volume but changes the observed data. Keep `network.sampling: 0` while establishing a baseline; test any sampling ratio before treating the metric as exact billing data.
