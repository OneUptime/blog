...
- **Cilium agent not starting**: Check resource limits and node capacity with `kubectl describe pod -n kube-system -l k8s-app=cilium`. Verify the BPF filesystem is mounted at `/sys/fs/bpf` and the kernel version meets the minimum for your Cilium release. Check init container logs with `kubectl logs -n kube-system <pod> -c cilium-init`.
