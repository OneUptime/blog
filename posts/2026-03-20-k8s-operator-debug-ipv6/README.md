...
# Verify dual-stack is enabled by checking node Pod CIDRs
kubectl get nodes <node-name> -o go-template --template='{{range .spec.podCIDRs}}{{printf "%s\n" .}}{{end}}'

# Verify a Pod has both IPv4 and IPv6 addresses
kubectl get pods -n kube-system <pod-name> -o go-template --template='{{range .status.podIPs}}{{printf "%s\n" .ip}}{{end}}'
```

## Monitoring with OneUptime

Use [OneUptime](https://oneuptime.com) to monitor your operator's health endpoint over IPv6. Configure IP or website monitors to check the operator's metrics and health endpoints over IPv6.

## Conclusion

How to Debug Operator IPv6 Connectivity Issues involves using Go's `net` package for IPv6 validation, checking node Pod CIDRs and Pod IP assignments, and testing against IPv6-enabled Kubernetes clusters. Always validate IPv6 addresses in CRD webhook validators to catch issues before reconciliation.
