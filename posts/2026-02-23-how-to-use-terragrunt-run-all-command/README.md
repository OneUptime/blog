...
terragrunt run --all --queue-ignore-errors -- apply
```
...
terragrunt run --all --filter './**/vpc' --filter './**/rds' -- plan

# Exclude specific modules
terragrunt run --all --filter '!./**/monitoring' -- plan

# Combine both
terragrunt run --all --filter './**/compute/** | !./**/compute/batch' -- apply
```
...
# run --all automatically passes -auto-approve to terraform apply for each module
```
...
cd live
terragrunt run --all --filter './**/vpc' --non-interactive -- apply
```
...
With multiple modules running, output can get noisy. Terragrunt includes the unit name or path in its log output:

```text
14:19:25.081 INFO   [vpc] Running command: tofu apply -auto-approve
14:19:25.174 STDOUT [vpc] tofu: Apply complete! Resources: 5 added, 0 changed, 0 destroyed.
14:19:25.181 INFO   [rds] Running command: tofu apply -auto-approve
14:19:25.188 INFO   [ecs] Running command: tofu apply -auto-approve
```
...
terragrunt run --all --log-level error --non-interactive -- apply
```
  ...
  deploy-dev:
    ...
    steps:
      ...
      - name: Plan dev
        run: |
          cd live/dev
          terragrunt run --all --non-interactive -- plan 2>&1 | tee plan-output.txt
...
For specific details on the plan, apply, and destroy workflows, see:
- [How to Use Terragrunt for Multi-Module Plans](https://oneuptime.com/blog/post/2026-02-23-how-to-use-terragrunt-plan-all-for-multi-module-plans/view)
- [How to Use Terragrunt for Multi-Module Apply](https://oneuptime.com/blog/post/2026-02-23-how-to-use-terragrunt-apply-all-for-multi-module-apply/view)
- [How to Use Terragrunt for Multi-Module Destroy](https://oneuptime.com/blog/post/2026-02-23-how-to-use-terragrunt-destroy-all-for-multi-module-destroy/view)
