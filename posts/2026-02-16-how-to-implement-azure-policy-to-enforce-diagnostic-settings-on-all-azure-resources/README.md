...
Many Azure resources can generate diagnostic data - resource logs, metrics, audit trails - but resource logs are not collected by default. Platform metrics and the Azure activity log are collected automatically, but you still need diagnostic settings to route resource logs, metrics, and activity log entries to a Log Analytics workspace, storage account, or Event Hub. In a growing environment, this creates an obvious problem: resources get deployed without diagnostic settings, and you end up with blind spots in your monitoring and security posture.

Azure Policy with the DeployIfNotExists effect solves this by automatically deploying diagnostic settings on resources that do not have them. When a new or updated resource is evaluated without the correct diagnostic configuration, the policy can deploy the missing setting after the resource provider returns success. In this post, I will show you how to set this up at scale.
...
The policy needs to be specific to each resource type because the log categories differ. This means you will need multiple policy definitions - one per resource type - or a built-in initiative that already bundles supported resource types.
...
az policy definition list \
  --query "[?policyType=='BuiltIn' && (contains(displayName,'Diagnostic') || contains(displayName,'diagnostic'))].{name:name, displayName:displayName, category:metadata.category}" \
  -o table | head -30
```
...
SQL_POLICY=$(az policy definition list \
  --query "[?contains(displayName,'Configure diagnostic settings for SQL Databases to Log Analytics')].name" -o tsv)
...
az policy set-definition create \
  ...
  --definitions "[
    {\"policyDefinitionReferenceId\":\"keyVaultDiagnostics\",\"policyDefinitionId\":\"/providers/Microsoft.Authorization/policyDefinitions/$KV_POLICY\",\"parameters\":{\"logAnalytics\":{\"value\":\"[parameters('logAnalyticsWorkspace')]\"}}},
    {\"policyDefinitionReferenceId\":\"sqlDatabaseDiagnostics\",\"policyDefinitionId\":\"/providers/Microsoft.Authorization/policyDefinitions/$SQL_POLICY\",\"parameters\":{\"logAnalytics\":{\"value\":\"[parameters('logAnalyticsWorkspace')]\"}}}
  ]" \
  --params '{
...
Save the `policyRule` object as `azurepolicy.rules.json`, save the `parameters` object as `azurepolicy.parameters.json`, and create the policy definition:
...
az policy definition create \
  ...
  --rules /path/to/azurepolicy.rules.json \
  --params /path/to/azurepolicy.parameters.json \
  --mode All
```
...
# For subscription-level remediation, create one task for each policy in the initiative
az policy remediation create \
  --name "remediate-kv-diagnostics-subscription" \
  --policy-assignment "enforce-diagnostic-settings" \
  --definition-reference-id "keyVaultDiagnostics" \
...
This Azure Resource Graph query shows the compliance state of diagnostic settings across your subscription:
...
| where tolower(PolicyDefinitionName) contains "diagnostic"
| summarize
