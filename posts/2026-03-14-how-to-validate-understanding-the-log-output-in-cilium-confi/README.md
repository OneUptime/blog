...
- A running Kubernetes cluster with Cilium installed, using a Kubernetes version supported by your Cilium release
...
```bash
# Check endpoint count and identity management from a Cilium agent
...
- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl -n kube-system exec ds/cilium -- cilium-dbg policy get`. When running in encapsulation mode, verify inter-node tunnel connectivity with `kubectl -n kube-system exec ds/cilium -- cilium-dbg bpf tunnel list`.
