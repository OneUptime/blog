...
kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg metrics list | grep -E "cilium_endpoint|cilium_identity|cilium_endpoint_regeneration" > /tmp/cilium-baseline-$(date +%Y%m%d).txt
...
# Optimize identity management to prevent excessive identity creation
...
- **Connectivity failures**: Run `cilium connectivity test` and inspect the specific failing test case. Check for conflicting network policies with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg policy get`. Verify node reachability with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-health status` and inspect remote-node entries with `kubectl -n kube-system exec ds/cilium -c cilium-agent -- cilium-dbg bpf ipcache list`.
