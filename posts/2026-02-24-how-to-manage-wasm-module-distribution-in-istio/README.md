...
4. The Istio agent in the proxy pod fetches the Wasm binary from the specified URL and stores it locally
5. The binary is compiled and loaded into the proxy's Wasm runtime
6. The plugin starts processing traffic

The key detail is that the Wasm binary is downloaded by the proxy-side Istio agent, not streamed through istiod. This means the proxy pod needs network access to wherever the binary is hosted.
...
Wasm plugins for Envoy can be written in several languages. The most common are Go, Rust, and C++.
  ...
  build:
    ...
    steps:
      ...
      - name: Build Wasm module
        run: env GOOS=wasip1 GOARCH=wasm go build -buildmode=c-shared -o plugin.wasm ./main.go
...
The Istio agent caches Wasm modules locally in each proxy pod. The `imagePullPolicy` controls when modules are refetched:

```yaml
# Only fetch if not cached - good for production with versioned tags
imagePullPolicy: IfNotPresent

# Always fetch when the WasmPlugin resource changes - good for development with mutable tags
imagePullPolicy: Always
```

With `IfNotPresent`, the module is fetched once and cached for the lifetime of the proxy pod unless the cache entry expires. If you update the binary behind the same tag, proxies may continue using the cached copy. This is why immutable version tags or digests are important for production.
...
When you update a WasmPlugin version, proxy-side Istio agents fetch the new module as the updated configuration is processed. This is not instantaneous - it happens over seconds to minutes depending on your mesh size.
...
# Check Istio agent stats for Wasm fetch and cache activity
kubectl exec deploy/my-service -c istio-proxy -- curl -s localhost:15020/stats/prometheus | grep "istio_agent_wasm"
...
env GOOS=wasip1 GOARCH=wasm go build -buildmode=c-shared -ldflags="-s -w" -o plugin.wasm ./main.go
