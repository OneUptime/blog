...
Once you enable orphaned resource monitoring in your ArgoCD projects, you will quickly discover that many resources in your cluster are technically "orphaned" but perfectly legitimate. Kubernetes auto-generates endpoint resources for Services, older clusters may have legacy ServiceAccount token Secrets, and operators spawn child resources that are not directly tracked by ArgoCD. Without proper exclusion rules, your orphan report becomes noisy and unusable.
...
All specified fields must match for a resource to be excluded. The `kind` and `name` fields match any value when omitted. The `group` field defaults to the core API group when omitted; use `group: "*"` if you need to match any API group.

```yaml
ignore:
  # Matches core-group Endpoints
  - kind: Endpoints

  # Matches Endpoints in any API group
  - group: "*"
    kind: Endpoints
...
orphanedResources:
  warn: true
  ignore:
    # Endpoints are auto-created for Services with selectors
    - group: ""
      kind: Endpoints

    # EndpointSlices are the current API for Service backends
...
```yaml
ignore:
  # ServiceAccount token secrets (legacy format, mainly pre-Kubernetes 1.24)
  - group: ""
    kind: Secret
    name: "*-token-*"
...
# Step 2: Check what shows up as orphaned for an application in the project
...
# Check the orphaned resources report for an application in the project
argocd app resources my-app --orphaned
