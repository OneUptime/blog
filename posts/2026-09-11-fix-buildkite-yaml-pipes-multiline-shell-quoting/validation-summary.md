...
- The v4.0.3 source confirms that dry-run output occurs without submitting a pipeline or requiring a job ID. Shared API configuration validation still requires a nonempty agent access token before the dry-run branch, so the example placeholder is needed for that local check; no authenticated upload occurs. JSON is the default dry-run format, so preserved newlines appear as escaped newline sequences in the output.
