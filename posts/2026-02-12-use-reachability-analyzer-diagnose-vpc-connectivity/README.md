...
- A Lambda function's VPC network interface can't reach the internet
...
Navigate to the Network Manager console and find "Reachability Analyzer" in the left sidebar. Then click "Create and analyze path."

You'll need to specify:

- **Source type**: Choose "Instance" and select your EC2 instance
- **Destination type**: Choose "Network interface" and select the network interface for your RDS instance
- **Protocol**: TCP
...
If the path is reachable, you'll see a green "Reachable" status with the path displayed. If there are multiple reachable paths, Reachability Analyzer displays the shortest path. The path shows the components the analyzed traffic uses - through subnets, route tables, security groups, and NACLs.

If the path is not reachable, this is where things get interesting. The analyzer will show you the component or combination of components that's blocking traffic and explains why. There might be additional blocking components that you'll uncover after fixing the first issue. For example, it might tell you:
...
The output is a JSON blob with all the path details. The key field to look at is `NetworkPathFound` - it's either `true` or `false`. If false, the `Explanations` array tells you where the analyzer found the blockage.
...
# Check if an EC2 instance can reach an RDS network interface on port 3306
check_connectivity('i-0123456789abcdef0', 'eni-0123456789abcdef0', 3306)
```
...
NAT gateways, load balancers, AWS Network Firewall, transit gateways, transit gateway attachments, and VPC peering connections can be included or excluded as intermediate components. This flexibility means you can test paths like "Can my EC2 instance reach the internet through this NAT gateway?" or "Is the transit gateway routing traffic correctly between VPCs?"

## Understanding the Path Components

When the analyzer returns a reachable path, it shows you the components in the analyzed path. Here's what a typical path might look like:
...
By default, you can store up to 1,000 network insights paths and 10,000 analyses per account per Region, and you can run up to 100 analyses concurrently. These quotas are adjustable.
