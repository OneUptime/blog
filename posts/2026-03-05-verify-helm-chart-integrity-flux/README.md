...
- A Helm chart signed with Cosign
- For OCI verification: a Cosign-signed chart in an OCI registry

## Understanding Verification Methods

Flux supports built-in signature verification for OCI Helm charts. Helm provenance files are a separate Helm mechanism that you can use before charts are published or mirrored, but Flux does not natively verify `.prov` files for HTTP Helm repositories.
...
2. **Helm provenance verification outside Flux** (for HTTP repositories): Verifies `.prov` provenance files generated by `helm package --sign` with Helm tooling before the chart reaches the repository.
...
This configuration verifies that the chart was signed during a GitHub Actions workflow run from the specified repository and branch. Flux treats the `issuer` and `subject` values as regular expressions. No key management is required.

## Method 2: Helm Provenance Verification Outside Flux

For charts stored in traditional HTTP Helm repositories, you can use Helm's built-in provenance mechanism before publishing or mirroring the chart.
...
### Configure Verification Before Flux

For HTTP repositories, Flux's `verify` field is not used for Helm provenance files. Add a verification step in your CI/CD pipeline before the chart reaches the repository, or mirror the chart into an OCI registry and sign the OCI artifact with Cosign.

Currently, Flux's native Helm chart verification support applies to OCI charts with signature verification. For HTTP repositories, you can implement provenance verification before publication and use admission controllers for policy enforcement.

## Using an Admission Controller for Additional Verification

For additional policy enforcement, deploy an admission controller like Kyverno or OPA Gatekeeper. The following example checks that HelmReleases include a Cosign verification configuration; it does not verify chart signatures itself.
        ...
        pattern:
          spec:
            chart:
              spec:
                verify:
                  provider: "cosign"    # Require Cosign verification to be configured
```
    ...
    steps:
      ...
      - name: Sign chart with Cosign (keyless)
        run: |
          ...
          cosign sign --yes ghcr.io/${{ github.repository_owner }}/charts/my-app:${VERSION}
```
...
4. **Keyless verification errors**: Check the source-controller logs for Fulcio, Rekor, or timestamp verification errors, and make sure you are using a Flux version that supports the signature format you used.

## Conclusion

Verifying Helm chart integrity in Flux is a critical step for securing your software supply chain. Cosign verification for OCI charts is the most robust native Flux approach, supporting both key-based and keyless signing. By configuring the `verify` field in your HelmRelease chart spec for OCI charts, you can ensure that only charts signed by trusted parties are deployed to your cluster. Combine this with CI/CD signing workflows and admission controller policies for a comprehensive defense-in-depth strategy.
