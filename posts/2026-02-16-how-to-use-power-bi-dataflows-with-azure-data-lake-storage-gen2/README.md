...
Power BI Dataflows give business analysts and data engineers a way to build reusable data transformation logic right inside the Power BI service. This guide applies to Power BI Dataflow Gen1 storage integration, which Microsoft now treats as a legacy capability. When you connect those dataflows to Azure Data Lake Storage Gen2, you unlock a whole new level of control over where your prepared data lands, who can access it, and how it integrates with the rest of your data platform.
...
- **Versioning**: ADLS Gen2 integrates with blob snapshots and versioning for data recovery scenarios.
...
- A Power BI workspace where you have permission to create dataflows
- Global admin or Power BI admin access for the tenant-level storage configuration
- Owner permission, Storage Blob Data Owner, and Storage Blob Data Reader on the storage account you want to connect
...
After the account is created, create a container (file system) inside it. Power BI writes dataflows to a filesystem named `powerbi`.
...
The Power BI service needs permission to write data to your ADLS Gen2 account. The account used to connect Azure storage to Power BI must have Owner permission at the storage account scope, plus Storage Blob Data Owner and Storage Blob Data Reader.

For the tenant-level approach, Power BI uses your signed-in account once to configure the connection and grant the Power BI service account the required read and write permissions. After that initial connection, the signed-in user account is not needed to keep the connection active.

In the Azure portal, go to your storage account, select "Access Control (IAM)," and add role assignments for the user who will configure the connection in Power BI.
...
# Assign the required roles to the user configuring the Power BI connection
  ...
  <workspace-name>/
    <dataflow-name>/
      ...
      model.json.snapshots/
        <timestamp>/
          model.json      # Previous versions of the dataflow metadata
      Orders/
...
orders_path = (
    "abfss://powerbi@pbidatalakestore.dfs.core.windows.net/"
    "SalesWorkspace/SalesDataflow/Orders/Orders.csv.snapshots/*/Orders.csv"
)
