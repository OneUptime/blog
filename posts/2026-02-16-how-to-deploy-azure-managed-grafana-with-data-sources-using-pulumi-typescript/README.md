...
The two available tiers are Essential and Standard, although Microsoft recommends Standard for new workspaces because Essential is a preview tier intended for evaluation and testing. Essential lacks features like zone redundancy, SMTP configuration, private endpoints, and enterprise plugins. For production monitoring, Standard is the right pick.
...
const grafana = new azure.dashboard.Grafana("grafana", {
...
Azure Managed Grafana uses Azure AD for authentication and has several built-in roles. The main roles you will assign most often are:
...
// Grant Grafana's managed identity Monitoring Data Reader on the Azure Monitor workspace
// This lets Grafana query Prometheus metrics stored in the workspace
const prometheusReaderRole = new azure.authorization.RoleAssignment("grafana-prometheus-reader", {
    scope: prometheusWorkspace.id,
    roleDefinitionId: pulumi.interpolate`/subscriptions/${currentClient.subscriptionId}/providers/Microsoft.Authorization/roleDefinitions/b0d8363b-8ddd-447d-831f-62ca05bff136`,
    principalId: grafana.identity.apply(id => id!.principalId!),
    principalType: "ServicePrincipal",
});
```
...
While Azure Managed Grafana includes an Azure Monitor data source by default, you may want to add additional data sources like external Prometheus servers or other databases. You can do this through the Azure CLI's Grafana commands after the instance is deployed.

```typescript
// User-assigned identity used by deployment scripts
const scriptIdentity = new azure.managedidentity.UserAssignedIdentity("grafana-script-identity", {
    resourceName: `id-grafana-script-${environment}`,
    resourceGroupName: resourceGroup.name,
    location: resourceGroup.location,
    tags: tags,
});

// Grant the script identity Grafana Admin so the CLI can configure Grafana
const scriptGrafanaAdminRole = new azure.authorization.RoleAssignment("script-grafana-admin", {
    scope: grafana.id,
    roleDefinitionId: pulumi.interpolate`/subscriptions/${currentClient.subscriptionId}/providers/Microsoft.Authorization/roleDefinitions/22926164-76b3-42b3-bc55-97df8dab3e41`,
...
// Helper function to configure Grafana data sources via Azure CLI
// This runs after the Grafana instance is fully provisioned
const configureDataSources = new azure.resources.AzureCliScript("configure-grafana-ds", {
    scriptName: `configure-grafana-ds-${environment}`,
    resourceGroupName: resourceGroup.name,
    location: resourceGroup.location,
    identity: {
        type: "UserAssigned",
        userAssignedIdentities: [scriptIdentity.id],
    },
    azCliVersion: "2.61.0",
    ...
    `,
    tags: tags,
}, { dependsOn: [grafana, logAnalyticsReaderRole, scriptGrafanaAdminRole] });
```
...
const deployDashboard = new azure.resources.AzureCliScript("deploy-dashboard", {
    scriptName: `deploy-dashboard-${environment}`,
    resourceGroupName: resourceGroup.name,
    location: resourceGroup.location,
    identity: {
        type: "UserAssigned",
