...
calicoctl get bgpfilter <filter-name> -o yaml --export > bgpfilter-backup.yaml
...
diff <(calicoctl get bgpfilter <filter-name> -o yaml --export) bgpfilter.yaml
```

Review each changed field and consider its impact:

- Will this change affect active connections?
- Does this change require a BGP session restart?
...
# Check BGP-related components for configuration reload messages
kubectl logs -n calico-system -l k8s-app=calico-node -c calico-node --tail=100 | grep -Ei "bird|confd|bgp|filter"
```
...
calicoctl get bgpfilter <filter-name> -o yaml
...
- Check if calico-node is crashlooping: `kubectl get pods -n calico-system`.
- Review calico-node logs for configuration errors.
...
- Check for typos in field names. On current Kubernetes versions, server-side field validation should warn or reject unknown fields, but older clusters or clients may still ignore them.
...
```bash
# Check whether your current user can update BGPFilter resources
kubectl auth can-i update bgpfilters.projectcalico.org --all-namespaces

# List allowed actions and filter for Calico resources
kubectl auth can-i --list --all-namespaces | grep -i projectcalico
