...
Also confirm that the 504 was generated by this NGINX instance. A CDN, load balancer, ingress controller, service mesh, another NGINX, or the application can return the same status. Compare `Server` and diagnostic headers cautiously because custom error pages can rewrite them, and use per-layer access logs.
