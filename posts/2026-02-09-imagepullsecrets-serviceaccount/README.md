...
Attaching imagePullSecrets to ServiceAccounts provides automatic registry authentication for pods using that ServiceAccount. Create docker-registry secrets with kubectl create secret docker-registry, add them to the ServiceAccount with the imagePullSecrets field, and new pods using that ServiceAccount get those pull secrets set automatically. This eliminates need to specify imagePullSecrets in every pod spec. Configure the default ServiceAccount to cover pods that use the namespace's default ServiceAccount, or create dedicated ServiceAccounts for different access levels. Simplifies private registry access management.
