...
- Client certificate and unencrypted key generated by the appropriate CA
...
# Create certificate extensions for TLS client authentication
...
Any repository added with a URL prefixed by `https://helm.internal.example.com` will inherit these TLS credentials, as long as the repository does not define its own credentials.
...
kubectl -n argocd label configmap argocd-tls-certs-cm \
...
ArgoCD mounts this ConfigMap into the `argocd-server` and `argocd-repo-server` pods. After updating it, allow time for Kubernetes to refresh the mounted ConfigMap in the pods.
