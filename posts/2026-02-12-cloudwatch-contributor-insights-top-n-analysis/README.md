...
7. Choose the aggregation (count or sum)
8. Set the number of top contributors to display when you view or query the report (typically 10 or 25)
...
If your application logs include IP addresses in structured JSON access logs, this rule finds the busiest clients:
...
aws cloudwatch put-insight-rule \
  ...
  --rule-definition '{
    ...
    "LogGroupNames": ["/app/my-service/access"],
    "LogFormat": "JSON",
    "Contribution": {
      "Keys": ["$.client_ip"],
      "Filters": []
...
That command creates Contributor Insights rules for the table. To enable it for a global secondary index, run the same command with `--index-name YourIndexName`. You'll see the most accessed and most throttled partition keys without writing any custom rules.
...
Contributor Insights integrates with CloudWatch dashboards through metric math. Add a widget that shows metric data generated by the rule:
  ...
  "widgets": [
    {
      ...
      "properties": {
        "metrics": [
          [
            {
              "expression": "INSIGHT_RULE_METRIC(\"TopIPAddresses\", \"Sum\")",
              "label": "Total requests matched by TopIPAddresses",
              "id": "e1"
            }
          ]
        ],
        "period": 300,
        "title": "Requests Matched by TopIPAddresses",
        "region": "us-east-1"
      }
    }
  ]
...
aws cloudwatch put-metric-alarm \
  ...
  --metrics '[{
    "Id": "e1",
    "Expression": "INSIGHT_RULE_METRIC(\"TopIPAddresses\", \"MaxContributorValue\")",
    "Label": "Top IP request count",
    "Period": 300
  }]' \
  --threshold 10000 \
...
- The CloudWatch free tier includes 1 Contributor Insights rule per month and the first 1 million matching log events per month

There are limits to be aware of: you can have up to 100 rules per account per region, and `get-insight-rule-report` can return 1 to 100 contributors per request. For most use cases, these limits are more than enough.
