...
If you have been using Flux with GitRepository sources, you may be experiencing unnecessary reconciliation in monorepo setups or wanting finer control over which file changes trigger deployments. The ArtifactGenerator resource in Flux provides path-based generation of ExternalArtifacts that solves these problems. This guide walks through migrating existing GitRepository-based Kustomizations and HelmReleases to use generated ExternalArtifacts, step by step.

## Prerequisites

- A Kubernetes cluster running Flux 2.7 or later with the source-watcher component installed and the ExternalArtifact feature gate enabled for the kustomize-controller and helm-controller
...
The GitRepository remains as the primary source. ArtifactGenerator sits between the GitRepository and the ExternalArtifacts used by your Kustomizations or HelmReleases. Do not delete the GitRepository.
...
spec:
  ...
  artifacts:
    - name: frontend-artifacts
      originRevision: "@repo"
      copy:
        - from: "@repo/apps/frontend/**"
          to: "@artifact/apps/frontend/"
---
apiVersion: source.extensions.fluxcd.io/v1beta1
kind: ArtifactGenerator
metadata:
...
spec:
  ...
  artifacts:
    - name: backend-artifacts
      originRevision: "@repo"
      copy:
        - from: "@repo/apps/backend/**"
          to: "@artifact/apps/backend/"
```
...
Now update each Kustomization to reference its corresponding generated ExternalArtifact instead of the GitRepository directly:
...
The key change is updating `sourceRef.kind` from `GitRepository` to `ExternalArtifact` and `sourceRef.name` to the corresponding generated ExternalArtifact name.
  ...
  artifacts:
    - name: frontend-artifacts
      ...
      copy:
        - from: "@repo/apps/frontend/**"
          to: "@artifact/apps/frontend/"
        - from: "@repo/common/base/**"
          to: "@artifact/common/base/"
        - from: "@repo/common/overlays/production/**"
          to: "@artifact/common/overlays/production/"
```
...
# Check generated ExternalArtifacts
...
If the migration causes issues, you can quickly revert by changing the Kustomization sourceRef back to GitRepository:
