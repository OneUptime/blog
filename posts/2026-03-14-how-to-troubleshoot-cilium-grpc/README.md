...
```bash
# Check endpoint status for failures on a Cilium agent
...
kubectl -n kube-system exec ds/cilium -- \
  ...
  jq '.[] | select(.status.labels.id | any(contains("app="))) | {
    ...
    ingress_l4_policy: .status.policy.realized.l4.ingress,
    egress_l4_policy: .status.policy.realized.l4.egress
  }'
```
...
- **Policy changes not propagating**: Check endpoint regeneration state with `kubectl -n kube-system exec ds/cilium -- cilium-dbg endpoint list` and review Cilium agent logs; Cilium regenerates endpoints automatically after policy, identity, or configuration changes.
