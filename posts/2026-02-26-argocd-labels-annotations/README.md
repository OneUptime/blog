...
In the ArgoCD web UI, use the application filters panel:
- Add `env=production` in the Labels filter
- Combine multiple filters by adding `env=production` and `team=payments`
...
Add human-readable links or ownership details that appear in the ArgoCD Application details tab:

```yaml
spec:
  info:
    - name: Description
      value: Payment processing API service
    - name: Owner
      value: payments-team@example.com
    - name: Runbook
      value: https://wiki.internal.com/runbooks/payment-api
```
...
### Refresh and Reconciliation Control Annotations
...
metadata:
  annotations:
    ...
    # Stop reconciliation for this Application until the annotation is removed or set to "false"
...
You might want labels on the Application CR to also appear on the resources it manages. ArgoCD does not automatically copy Application labels to generated Kubernetes resources; configure the manifest generator for each application source.
