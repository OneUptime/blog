...
This guide covers setting up Managed Microsoft AD and integrating it with GCP services like Compute Engine and Cloud SQL for SQL Server.

## What Managed Microsoft AD Provides

Managed Microsoft AD deploys actual Windows Server-based domain controllers in your VPC. You get standard AD features like domain join, Group Policy, Kerberos authentication, LDAP, and DNS. Google manages the infrastructure - the domain controllers run in a dedicated project, get patched automatically, replicate across zones for high availability, and are backed up automatically.

You get a standard AD forest for each Managed Microsoft AD domain, but you do not get Domain Admin or Enterprise Admin access. Instead, you get a delegated admin account that can manage users, groups, OUs, and Group Policies within your domain.
...
Managed Microsoft AD integrates with Cloud DNS so VMs in authorized networks can resolve the AD domain without per-client DNS changes. For on-premises resolvers that need to query Google Cloud DNS, create an inbound DNS policy:

```bash
# Create a DNS policy for inbound forwarding
gcloud dns policies create ad-dns-forwarding \
  --description="Allow inbound DNS queries for Managed AD discovery" \
  --networks=default \
  --enable-inbound-forwarding
```

If you need a private forwarding zone for a separate DNS namespace, create one with forwarding targets that can resolve that namespace:

```bash
# Create a forwarding zone for another internal domain
gcloud dns managed-zones create ad-forward-zone \
  --description="Forward internal domain queries" \
  --dns-name="onprem.mycompany.com." \
  --networks=default \
  --visibility=private \
...
Then RDP into the VM and join the domain. You can also automate this with the Managed Microsoft AD domain-join startup script. The VM service account needs the Google Cloud Managed Identities Domain Join role on the project that hosts the Managed Microsoft AD domain.

```bash
# Create a Windows VM and automatically join it to the domain
gcloud compute instances create windows-server-01 \
  --zone=us-central1-a \
  --machine-type=n2-standard-4 \
  --image-family=windows-2022 \
  ...
  --boot-disk-size=100GB \
  --network=default \
  --service-account=DOMAIN_JOIN_SERVICE_ACCOUNT \
  --scopes=https://www.googleapis.com/auth/cloud-platform \
  --metadata=windows-startup-script-url=https://raw.githubusercontent.com/GoogleCloudPlatform/managed-microsoft-activedirectory/main/domain_join.ps1,managed-ad-domain=projects/PROJECT_ID/locations/global/domains/corp.mycompany.com,managed-ad-domain-join-failure-stop=TRUE,enable-guest-attributes=TRUE
```
...
gcloud active-directory domains trusts create corp.mycompany.com \
  ...
  --handshake-secret="SHARED_SECRET"
```
...
gcloud beta sql instances create sql-server-01 \
...
After the instance is joined to Managed Microsoft AD, connect as the `sqlserver` user and create a SQL Server login for an AD user or group:

```sql
CREATE LOGIN [corp\dbuser] FROM WINDOWS;
```
...
gcloud monitoring policies create \
  ...
  --condition-filter='resource.type="microsoft_ad_domain" AND metric.type="managedidentities.googleapis.com/microsoft_ad/domain/health"' \
  --if="< 1" \
  --duration="300s" \
...
def audit_ad_logins(project_id, days_back=1):
    ...
    filter_str = (
        ...
        'resource.labels.fqdn="corp.mycompany.com" AND '
        '(jsonPayload.ID=4624 OR jsonPayload.ID=4625) AND '
...
Managed AD includes automatic scheduled backups every 12 hours, but you can also trigger manual backups before major changes.
...
gcloud active-directory domains backups create pre-migration-backup \
  --domain=corp.mycompany.com \
  --project=PROJECT_ID
