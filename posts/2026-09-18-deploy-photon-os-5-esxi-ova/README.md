...
On a Linux workstation, calculate the download's digest with `sha256sum`; on macOS, use `shasum -a 256`. Compare it with the checksum for that exact filename. Record the filename, checksum, and intended datastore in the deployment ticket so another operator can reproduce the VM.
...
Before power-on, inspect the NIC connection setting, disk provisioning, firmware, and resource allocation. Keep the image's intended firmware configuration unless you have verified a reason to change it. A boot failure after changing firmware is a different problem from a corrupt download.
...
A successful DNS lookup does not prove outbound HTTPS works, but it separates name resolution from transport failures. Check the clock as well: a badly incorrect date can make otherwise valid certificates fail validation.
...
If this VM will host Docker, check the installed package and service before adding anything:
...
If Docker is absent, install the `docker` package from the matching Photon repositories. Pull and run an approved test image, then test an application endpoint from another machine on the intended client network. Testing only from inside the VM misses port-group and upstream firewall problems.
