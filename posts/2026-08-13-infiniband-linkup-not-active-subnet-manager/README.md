...
| LID `0x0` or permissive `0xffff`, together with SM LID `0x0` | no usable LID/SM information is installed | by itself, where the SM failure is |
...
A useful response identifies an SM state such as `Master`, its priority, GUID, and activity count. A timeout does not prove that no SM process exists anywhere. It proves that this local management path did not get a response. Correlate it with an unusable local LID, a zero SM LID, and the logical port state.
...
~~~console
$ pgrep -a opensm
$ systemctl list-unit-files --type=service | grep -i opensm
...
`LinkUp` with `Init`, an unusable local LID, and no `sminfo` response strongly points to an absent or unreachable SM. Check whether the intended SM host is powered on, whether its own IB port is `LinkUp`, and whether OpenSM can access the corresponding management devices (`/dev/infiniband/umad*` and `/dev/infiniband/issm*`). Review the OpenSM log for bind failures, discovery timeouts, duplicate GUIDs, or a sweep that never completes.
...
~~~console
$ ibportstate -C mlx5_0 -P 1 SWITCH_LID SWITCH_PORT
~~~
...
The expected endpoint is `State: Active`, `Physical state: LinkUp`, a usable unicast LID (neither `0x0` nor permissive `0xffff`), and an SM LID that corresponds to the elected master. `iblinkinfo` adds the fabric-side view once management queries work.
...
- For IPoIB, confirm the correct interface, including any required P_Key child interface, IP address, route, and MTU on both peers, then test the selected source and destination addresses.
...
- [Linux kernel: userspace MAD and SM device access](https://docs.kernel.org/infiniband/user_mad.html)
