...
Pre-DNAT policies in Calico are applied before Destination NAT translation, meaning they operate on the original destination IP of incoming traffic rather than the translated pod IP. This is critical for protecting Kubernetes NodePort and NodePort-backed LoadBalancer services because traffic arrives at the node IP before being forwarded to pods.
...
This guide covers migrate pre-DNAT policies in Calico for controlling external access to NodePort and NodePort-backed LoadBalancer services.

## Prerequisites

- Kubernetes cluster with Calico v3.26+
- `calicoctl` and `kubectl` installed
- Host endpoints configured for the target nodes and labeled to match the policy selector
