...
sequenceDiagram
    ...
    Dex-->>ArgoCD: Return auth code
    ArgoCD->>Dex: Exchange code for tokens
    Dex-->>ArgoCD: Return ID token
      ...
      containers:
        - name: dex
          command:
            - /shared/argocd-dex
            - rundex
          # These environment variables are usually populated from argocd-cmd-params-cm
          env:
      ...
      - type: microsoft
        ...
        config:
          ...
          groups:
            - ArgoCD-Admins
            - ArgoCD-Developers
          # Include all group types, not only security groups
...
ArgoCD generates the final Dex configuration and sets Dex storage automatically for the bundled Dex server. In current ArgoCD releases this generated storage setting is in-memory, so you normally do not configure `storage` in `dex.config`.

```yaml
# Generated by ArgoCD for the bundled Dex server
storage:
  type: memory
```
...
kubectl exec -n argocd deployment/argocd-server -- \
  curl -ks https://argocd-dex-server:5556/api/dex/.well-known/openid-configuration | jq .
```
