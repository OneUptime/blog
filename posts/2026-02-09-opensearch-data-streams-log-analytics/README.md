...
OpenSearch data streams provide the optimal architecture for time-series log data from Kubernetes clusters. Unlike traditional indices, data streams simplify rollover, can be managed with lifecycle policies, and optimize storage for append-only workloads. This guide shows you how to deploy OpenSearch with data streams configured specifically for Kubernetes log analytics.

## Understanding OpenSearch Data Streams

Data streams offer several advantages for log management:

- **Automatic rollover** - New indices created automatically when size or age thresholds are met through ISM
- **Index State Management** - Automatic transition from hot to warm to cold storage
...
Traditional index approach requires manual rollover and alias management. Data streams simplify this process.
...
curl -X PUT "http://opensearch:9200/_index_template/kubernetes-logs" \
...
Create an ISM policy for automatic index management:

```bash
# Create lifecycle policy
curl -X PUT "http://opensearch:9200/_plugins/_ism/policies/kubernetes-logs-policy" \
  -H 'Content-Type: application/json' \
  -d '{
  "policy": {
    ...
    "ism_template": {
      "index_patterns": [".ds-kubernetes-logs-*"],
      "priority": 200
    }
  }
}'
...
data:
  fluent-bit.conf: |
    ...
    [FILTER]
        Name           lua
        Match          kube.*
        code           function add_timestamp(tag, timestamp, record) record["@timestamp"] = os.date("!%Y-%m-%dT%H:%M:%SZ", timestamp) return 2, timestamp, record end
        call           add_timestamp
...
curl -X PUT "http://opensearch:9200/_data_stream/kubernetes-logs"

# Verify data stream
curl -X GET "http://opensearch:9200/_data_stream/kubernetes-logs"
```
...
```bash
# Simple search
curl -X GET "http://opensearch:9200/kubernetes-logs/_search" \
  -H 'Content-Type: application/json' \
  -d '{
  "query": {
    "bool": {
...
curl -X GET "http://opensearch:9200/_data_stream/kubernetes-logs/_stats"

# Check backing indices
curl -X GET "http://opensearch:9200/_data_stream/kubernetes-logs"

# Monitor index sizes
curl -X GET "http://opensearch:9200/_cat/indices/.ds-kubernetes-logs-*?v&s=index"

# Check shard allocation
curl -X GET "http://opensearch:9200/_cat/shards/.ds-kubernetes-logs-*?v"
```
...
curl -X PUT "http://opensearch:9200/.ds-kubernetes-logs-*/_settings" \
  -H 'Content-Type: application/json' \
  -d '{
  "index": {
...
curl -X POST "http://opensearch:9200/.ds-kubernetes-logs-000001/_forcemerge?max_num_segments=1"

# Clear cache
curl -X POST "http://opensearch:9200/kubernetes-logs/_cache/clear"
```

## Conclusion

OpenSearch data streams provide an optimized architecture for Kubernetes log analytics. With ISM-managed rollover, lifecycle management, and efficient querying across time periods, data streams simplify log management while improving performance and reducing storage costs. Deploy with proper lifecycle policies, configure Fluent Bit for efficient log shipping, and use OpenSearch Dashboards for powerful log visualization and analysis.
