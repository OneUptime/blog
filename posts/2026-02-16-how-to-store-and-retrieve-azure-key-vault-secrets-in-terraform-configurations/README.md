...
Secrets in Terraform are a constant headache. Database passwords, API keys, connection strings - they all need to go somewhere, and hardcoding them in your Terraform files is obviously a terrible idea. Azure Key Vault is the natural place to store secrets in an Azure environment, and Terraform integrates with it through data sources and ephemeral resources (reading secrets) and resources (writing secrets). But getting this right without accidentally exposing secrets in your state file or logs takes some care.
...
Before getting into Key Vault specifics, it is worth understanding why secrets in Terraform are tricky. Terraform stores everything it knows about your resources in state. When you create a secret with a regular resource argument or read one with a data source, the value ends up in the state file in plain text. This means your state file is effectively a secrets store, which is why remote state with encryption and access controls is so important.

You cannot eliminate secrets from every Terraform workflow, but you can minimize exposure by using Key Vault references, ephemeral resources, write-only arguments, and only reading secret values when necessary.
...
resource "azurerm_key_vault" "main" {
  ...
  # Network restrictions - allow trusted Microsoft services and specific IPs
  network_acls {
    default_action = "Deny"
...
You can create secrets in Key Vault directly from Terraform. This is useful for generated values like random passwords. The write-only argument pattern below requires Terraform 1.11 or later and a current AzureRM provider:

```hcl
# secrets.tf - Generate and store secrets in Key Vault

# Generate a random password for the database without storing it in state
...
resource "azurerm_key_vault_secret" "api_key" {
  ...
  value_wo         = var.external_api_key  # Passed via TF_VAR or tfvars
  value_wo_version = 1
  key_vault_id     = azurerm_key_vault.main.id
...
If the Key Vault firewall is enabled, make sure the app can reach the vault through allowed outbound IPs, virtual network integration with a service endpoint, or a private endpoint. The `AzureServices` bypass only covers trusted Microsoft service scenarios, not every App Service runtime request.
...
resource "azurerm_key_vault_secret" "connection_string" {
  ...
  value_wo         = "postgresql://dbadmin:${ephemeral.random_password.admin.result}@${azurerm_postgresql_flexible_server.main.fqdn}:5432/${var.database_name}?sslmode=require"
  value_wo_version = 1
...
# Increment the write-only version only when the rotation timestamp changes
resource "azurerm_key_vault_secret" "db_password" {
  name             = "db-admin-password"
  value_wo         = ephemeral.random_password.db_password.result
  value_wo_version = time_rotating.db_password.unix
  key_vault_id     = azurerm_key_vault.main.id
}
```
