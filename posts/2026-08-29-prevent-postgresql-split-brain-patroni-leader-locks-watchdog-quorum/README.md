...
2. **Role enforcement:** only the Patroni member that owns that lock may run PostgreSQL as the normal primary; loss of the ability to renew it triggers demotion.
...
The Patroni leader writes and periodically renews a TTL-backed leader key in the DCS. Other members observe that key and remain replicas. If the key expires, eligible replicas participate in a leader race; etcd's compare-and-set behavior and Raft quorum allow one winner to create the new leader key.
...
For three etcd members, two votes form quorum and one member may fail. Place the members across independent host, power, storage, and network failure domains. A two-member etcd cluster requires both votes and tolerates no failure. A single etcd endpoint or a single proxy in front of healthy members still creates a single access failure for Patroni.
...
When Patroni cannot update the leader lock, it normally stops or demotes PostgreSQL before the lock expires. But the same fault that interrupts DCS access can prevent that code from running:
...
After the old lock expires, another member can legitimately promote. If the old PostgreSQL process still accepts writes when its host resumes, two primaries exist. A watchdog provides a deadline outside the Patroni process.
...
First verify the device and permissions without opening it from an ad hoc program:
...
Use `required` only after proving the device works on every failover candidate. Patroni arms the watchdog before promotion, refreshes it after confirmed leader-lock updates, and disables it after demotion or while the cluster is paused.
...
There is a narrow suspension window with a fixed margin: if Patroni is paused at exactly the wrong moment after a keepalive, the watchdog could expire after the DCS lock. Patroni's documented strongest setting is:
...
`maximum_lag_on_failover` bounds how far behind a replica may be for automatic failover; it limits potential data loss but does not guarantee zero loss with asynchronous replication. `check_timeline` keeps automatic failover/switchover candidates from using a timeline older than the known cluster timeline.
...
- Patroni synchronous mode restricts eligible failover candidates and configures `synchronous_standby_names` so acknowledged transactions are durable on designated standbys.
- `synchronous_mode_strict: true` blocks writes if no eligible synchronous standby exists. It trades write availability for durability.
...
The primary reports `false` and `off`; replicas report `true` and are read-only.
...
2. Remove primary access to etcd but leave the host schedulable; verify Patroni demotes before lock expiry.
...
| --- | --- | --- |
| DCS quorum lost and primary demotes | Patroni cannot prove exclusive leadership | Restore quorum; do not bypass the lock |
...
| Old primary comes back on an earlier timeline | Its data diverged after failover | Keep fenced; use `pg_rewind` or a fresh base backup |
...
| Synchronous mode blocks writes | Required durable standby is absent | Restore a synchronous standby or make an explicit durability trade-off |
