...
Using the configured canonical hostname rather than reflecting an arbitrary client `Host` value also avoids generating redirects from an untrusted header. If another load balancer sits in front of NGINX, define which proxy is authoritative and configure each hop consistently rather than blindly preserving client-supplied forwarding headers.
...
Alternatively, enable Jetty's forwarded connector with this JVM setting:

```text
-Drundeck.jetty.connector.forwarded=true
```

The package places additional JVM options in `/etc/default/rundeckd` on Debian-family systems or `/etc/sysconfig/rundeckd` on RPM-family systems. Use the property or JVM approach appropriate to the deployment rather than assuming both are required. The Docker image's `RUNDECK_SERVER_FORWARDED=true` performs the equivalent container configuration.

## Keep Port 4440 Private

On a single host, bind or firewall the upstream so only the proxy can reach it. A Docker Compose deployment does not need to publish 4440 to every interface:
...
Here `expose` makes the port available on the Compose network but does not publish it on the host. Point NGINX at `http://rundeck:4440`. In a VM deployment, use loopback binding when supported or a host firewall rule that accepts 4440 only from the proxy.

The point-release tag above is current at the time of writing and illustrates the network layout. Substitute the release you tested, and use a digest as well when the deployment requires immutable image identity.
...
- A `Location` header containing `:4440` means `grails.serverURL` or `framework.server.url` still names the internal listener.
...
Configure Rundeck with its canonical public HTTPS URL, forward the original host, port, scheme, and client chain, and enable forwarded-request handling. Once the proxy can reach a private 4440 listener, expose only port 443 and validate every generated redirect through the public hostname.
